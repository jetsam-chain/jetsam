...
pub fn validate_mandatory_coinbase(
    ...
    // cheap preflight.
    jetsam_tx::validate_body_semantics_no_hash(&coinbase.body)
    ...
    match (allocation.payout_each, stream.has_development_payout) {
        ...
        (Some(expected_amount), true) => {
            ...
            jetsam_tx::validate_body_semantics_no_hash(&payout.body).map_err(|_| {
            ...
            if first.owner != crate::consensus::development_allocation::NETWORK_FUND_ADDRESS
                || second.owner != crate::consensus::development_allocation::LAB_FUND_ADDRESS
