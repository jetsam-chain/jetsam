...
//! JetsamReceipt — unforgeable proof of transaction inclusion.
//!
...
//!   Must match `jetsam_chain::block::compute_tx_root` exactly.
...
use jetsam_tx::{validate_paged_spend, PagedSpendIntent, Transaction, TxPage};
...
impl JetsamReceipt {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        bincode::serialize(self).expect("JetsamReceipt serialize")
    }
...
///
/// Uses Poseidon2b COMPRESS to match `compute_tx_root` in `jetsam_chain::block`.
pub fn verify_merkle_inclusion(receipt: &JetsamReceipt) -> bool {
...
pub fn verify_against_header(receipt: &JetsamReceipt, canonical_header: &BlockHeader) -> bool {
...
/// Mirrors `compute_tx_root` in `jetsam_chain::block`.
...
mod tests {
    ...
    fn all_255_user_positions_have_authenticated_fixed_paths() {
        ...
        for (index, transaction) in groups.iter().enumerate() {
            ...
            assert_eq!(
                JetsamReceipt::from_bytes(&receipt.to_bytes())
                    .unwrap()
                    .paged_spend,
                receipt.paged_spend
            );
