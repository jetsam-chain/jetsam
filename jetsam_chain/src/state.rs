...
pub struct ChainState {
    ...
    /// Exact sum of every live UTXO amount in μJTM.
    ///
...
impl ChainState {
    ...
    pub fn durable_metadata_clone(&self) -> Option<Self> {
        Some(Self {
            ...
            circulating_supply_micro_jtm: self.circulating_supply_micro_jtm,
    ...
    pub fn from_loaded_parts(
        ...
        let circulating_supply_micro_jtm = Self::loaded_circulating_supply(&state)?;
    ...
    pub fn from_sparse_utxos(
        ...
        state.circulating_supply_micro_jtm = slots
            .iter()
    ...
    pub(crate) fn supply_after_slot_updates(
        ...
        let mut supply = self.circulating_supply_micro_jtm;
    ...
    pub fn apply_verified_exact_transition(
        ...
        self.circulating_supply_micro_jtm = circulating_supply_micro_jtm;
        Ok(applied_root)
    }
}
...
mod tests {
    ...
    fn explicit_input_owner_matches_state_and_output_gets_new_incarnation() {
        ...
        assert_eq!(state.circulating_supply_micro_jtm, 10);
    }
    ...
    fn wrong_owner_and_occupied_output_fail_atomically() {
        ...
        let supply = state.circulating_supply_micro_jtm;
        assert_eq!(
            apply_tx(&mut state, &body(Address([9u8; 32]), 1, 2)),
            Err(ApplyError::UnknownOrSpentInput)
        );
        assert_eq!(state.state_root(), root);
        assert_eq!(state.circulating_supply_micro_jtm, supply);
    ...
    fn coinbase_mint_is_height_tagged_and_aba_binds_exact_creation_id() {
        ...
        assert_eq!(state.active_slot_count, 1);
        assert_eq!(state.circulating_supply_micro_jtm, 50);
        ...
        assert_eq!(state.circulating_supply_micro_jtm, 49);
    }
    ...
    fn development_payout_uses_the_normal_allocator_namespace() {
        ...
        assert_eq!(state.circulating_supply_micro_jtm, 65);
    }
    ...
    fn verified_exact_transition_updates_the_derived_supply_atomically() {
        ...
        assert_eq!(state.circulating_supply_micro_jtm, 8);
