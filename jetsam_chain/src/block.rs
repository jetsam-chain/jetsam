...
use jetsam_tx::{PagedSpendError, Transaction, TxPage};
...
pub fn try_compute_logical_txids(
    txs: &[Transaction],
) -> Result<Vec<jetsam_poseidon2b::primitives::TxBodyHash>, BlockPageStreamError> {
    if txs.is_empty() {
        return Ok(Vec::new());
    }
...
pub fn canonical_block_wire_len(tx_count: usize) -> Result<usize, WireError> {
    ...
    tx_count
        .checked_mul(jetsam_tx::wire::TRANSACTION_WIRE_SIZE)
