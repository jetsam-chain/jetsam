...
use elide_chain::consensus::validation::validate_block_checks_template;
use elide_chain::consensus::{
...
use elide_chain::exact_state_hash::{slot_leaf_hash, StateHash};
use elide_chain::sparse_merkle::{
    derive_structural_frontier_plan, evaluate_structural_frontier, SparseMerkleError,
};
use elide_chain::state::{ChainState, ExactFrontierError};
use elide_chain::{
...
use elide_gkr::{spine_inputs_from_body, MerklePathInputs, SlotLeafInputs, MAX_MERKLE_DEPTH};
use elide_poseidon2b::native::compress;
use elide_recursive::acceptance::history_step::{
...
impl<const TIER: usize> PreparedHistoryStepWitness<TIER> {
    ...
    pub fn user_page_count(&self) -> usize {
        usize::from(
            elide_chain::validate_block_page_stream(&self.template.transactions)
    ...
    ) -> Result<(Block, elide_recursive::BuiltHistoryStep, ChainAccumulator), HistoryStepWitnessError>
    {
        let Self {
            template,
            parent_header,
        ...
        elide_chain::consensus::validation::validate_block_checks_template(
            &template,
            &parent_header,
            &previous_timestamps,
            &finalized_active_counts,
...
fn prepare_native_history_step<const TIER: usize>(
    ...
    let actual_tier =
        elide_chain::consensus::paged_spend::BlockProofClass::for_page_count(effective_pages)
...
fn validate_nonce_independent_block(
    ...
    if block.header.prev_block_hash != elide_chain::hash_block_header(context.parent_header)
        || context
            .start_accumulator
            .height
            .checked_add(1)
    ...
    elide_chain::consensus::checks::validate_block_slot_conflicts(&block.transactions)?;
    ...
    let parent_block_id = elide_chain::hash_block_header(context.parent_header);
    let user_epoch_anchor = if context
        .start_accumulator
        .height
        .is_multiple_of(elide_chain::consensus::params::TX_EPOCH_BLOCKS)
    {
        parent_block_id
    } else {
...
fn build_history_step_components(
    ...
    for (group_index, group) in stream.groups.iter().enumerate() {
        ...
        let public =
            elide_gkr::OwnerAuthPublicInputs::new(tx_body_hash, group.spend.input_owner.as_fields());
...
fn slot_leaf_input(slot: elide_chain::SlotValue) -> SlotLeafInputs {
    SlotLeafInputs {
...
fn tx_root_merkle_inputs(
    ...
    let leaf_capacity = elide_chain::tx_tree::TX_TREE_LEAVES;
    let depth = elide_chain::tx_tree::TX_TREE_DEPTH;
    ...
    if elide_chain::tx_tree::bind_tx_count(root, logical_hashes.len()) != block.header.tx_root {
        return Err(HistoryStepWitnessError::TransactionRootMismatch);
    }
...
mod tests {
    ...
    use elide_chain::consensus::template::build_block_template;
    ...
    fn expansion_frontier_binds_parent_as_left_child_of_the_new_domain() {
        use elide_chain::block_id;
        use elide_chain::consensus::fees::required_fee_for_tx_body;
        use elide_chain::exact_state_hash::{state_node_hash, zero_slot_roots};
    ...
    fn tx_root_paths_bind_every_real_body_to_the_count_wrapped_root() {
        ...
        assert_eq!(paths[0].active_depth, elide_chain::tx_tree::TX_TREE_DEPTH);
    }
}
