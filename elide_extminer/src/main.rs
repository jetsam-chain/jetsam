...
use elide_poseidon2b::batch::FixedFieldNonceBatch;
use elide_poseidon2b::native::domain::TAG_POWHDR;
use rayon::prelude::*;
...
fn mine(cli: &Cli) -> Result<()> {
    ...
    eprintln!(
        "elide-miner  rpc={}  threads={}  backend={}  poll={}ms",
        cli.rpc,
        threads,
...
fn main() {
    ...
    if let Err(error) = elide_core::cpu::ensure_production_hardware() {
