...
const _: () =
    assert!(SNAPSHOT_BOUNDARY_INTERVAL <= jetsam_chain::consensus::params::CONSENSUS_FINALITY_DEPTH);
...
impl GetStateManifestHeader {
    ...
    pub fn has_canonical_page_shape(&self) -> bool {
        ...
        if segment_count > jetsam_chain::consensus::wire_limits::MAX_SNAPSHOT_MANIFEST_SEGMENTS {
            return false;
        }
...
pub struct GetStateSegmentResponse {
    ...
    pub status: DataResponseStatus,
    pub eff_log: u8,
    /// Column data encoded by `jetsam_chain::storage::serial::encode_segment`.
...
impl NetworkTopics {
    pub fn for_network_cfg(cfg: &jetsam_chain::consensus::NetworkConfig) -> Self {
        Self {
