...
use jetsam_chain::AcceptedBlockBundle;
...
enum PreparedSnapshotExport {
    ...
    GenerationError(jetsam_chain::storage::SnapshotGenerationError),
    InvalidManifest,
}
...
const _: () = assert!(
    MAX_PENDING_STATE_SEGMENT_REQUESTS >= jetsam_chain::consensus::wire_limits::MAX_INFLIGHT_SEGMENTS
);
...
// Hard caps on incoming response sizes are shared via jetsam_chain::consensus::wire_limits.
...
fn snapshot_suffix_is_retained(tip_height: u64, terminal_height: u64) -> bool {
    terminal_height <= tip_height
        && tip_height.saturating_sub(terminal_height)
            <= jetsam_chain::consensus::params::RECENT_BLOCK_RETENTION_DEPTH
}

fn retained_object_inventory_floor(tip_height: u64) -> u64 {
    tip_height.saturating_sub(jetsam_chain::consensus::params::RETAINED_BLOCK_SERVING_DEPTH)
}
...
fn deterministic_snapshot_boundary_window(
    ...
    let oldest = tip_height
        .saturating_sub(jetsam_chain::consensus::params::UNDO_RETENTION_DEPTH)
        .max(1);
...
fn local_history_step_boundary(store: &MdbxStore) -> Option<(u64, [u8; 32])> {
        ...
        .filter(|height| height % crate::protocol::SNAPSHOT_BOUNDARY_INTERVAL == 0)
    {
        ...
        let block_hash = jetsam_chain::hash_block_header(&header);
        ...
        let has_cached = store
            .has_any_history_step_proof_object(
                height,
                jetsam_chain::block_header::semantic_header_id(&header),
            )
            .ok()?;
    ...
    match object {
        ObjectId::BlockBody(expected) => {
            ...
            let bytes = match canonical {
                Some(bytes) => {
                    let matches = jetsam_chain::Block::from_bytes(&bytes)
                        .ok()
                        .is_some_and(|block| {
                            block.header.height == expected.claim.height
                                && jetsam_chain::block_header::block_id(&block.header)
            ...
            let block = jetsam_chain::Block::from_bytes(&bytes)
                .map_err(|error| format!("decode cached block object: {error:?}"))?;
            if block.header.height != expected.claim.height
                || jetsam_chain::block_header::block_id(&block.header) != expected.claim.block_hash
                || !expected.matches_bytes(&bytes)
            {
                return Ok(None);
            }
        ...
        ObjectId::Terminal(expected) => {
            ...
            let metadata =
                jetsam_chain::history_step::HistoryStepTerminalMetadata::decode_prefix(&bytes)
    ...
    exports
        ...
        .filter(|generation| {
            ...
            let boundary_matches = store
                ...
                .flatten()
                .is_some_and(|header| {
                    jetsam_chain::hash_block_header(&header) == manifest.target_hash
            ...
            let bridge_matches = store
                ...
                .flatten()
                .is_some_and(|header| {
                    jetsam_chain::hash_block_header(&header) == manifest.bridge_tip_hash
                });
...
fn local_history_step_terminal(
    ...
    let header = store.get_header(height).ok().flatten()?;
    if jetsam_chain::block_header::block_id(&header) != block_hash {
        return None;
    }
...
pub enum NetworkEvent {
    ...
    SnapshotHeadersBatch {
        ...
        start_height: u64,
        requested_count: u16,
        headers: Vec<jetsam_chain::block_header::BlockHeader>,
        snapshot_boundary: Option<crate::object_protocol::ChainPoint>,
    },
                    ...
                    match result {
                        ...
                        PreparedSnapshotExport::GenerationError(error) => {
                            let retry_after_tail_install = matches!(
                                error,
                                jetsam_chain::storage::SnapshotGenerationError::MissingBridgeTerminal(_)
                                    | jetsam_chain::storage::SnapshotGenerationError::MissingBoundaryTerminal(_)
                            );
                                        ...
                                        .then(|| {
                                            ...
                                            canonical.or_else(|| {
                                                store
                                                    .get_any_history_step_proof_object(
                                                        header.height,
                                                        jetsam_chain::block_header::semantic_header_id(
                                                            &header,
                                                        ),
                                                    )
                                                    .ok()
...
mod tests {
    ...
    fn snapshot_terminal_serving_requires_retained_suffix() {
        let retention = jetsam_chain::consensus::params::RECENT_BLOCK_RETENTION_DEPTH;
    ...
    fn header_inventory_serves_the_complete_retained_payload_window() {
        let serving = jetsam_chain::consensus::params::RETAINED_BLOCK_SERVING_DEPTH;
        let recent = jetsam_chain::consensus::params::RECENT_BLOCK_RETENTION_DEPTH;
        let tip = serving + 100;
    ...
    fn deterministic_snapshot_boundary_exists_for_every_bucket_residue() {
        ...
        for residue in 0..crate::protocol::SNAPSHOT_BOUNDARY_INTERVAL {
            ...
            assert!(tip_height - boundary <= jetsam_chain::consensus::params::UNDO_RETENTION_DEPTH);
            assert!(
    ...
    fn cached_boundary_proof_outlives_the_recent_suffix_window() {
        let tip_height = 100;
        let boundary_height =
            tip_height - jetsam_chain::consensus::params::RECENT_BLOCK_RETENTION_DEPTH - 5;
        assert!(!snapshot_suffix_is_retained(tip_height, boundary_height));
        assert!(
            tip_height - boundary_height <= jetsam_chain::consensus::params::UNDO_RETENTION_DEPTH,
    ...
    fn header_batch_shape_rejects_noncontiguity_without_rehashing_links() {
        let mut first = jetsam_chain::consensus::genesis::genesis_header();
        first.height = 77;
        let mut second = first;
        second.height = 78;
        second.prev_block_hash = jetsam_chain::hash_block_header(&first);
        assert_eq!(
