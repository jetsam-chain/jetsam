...
use jetsam_chain::consensus::NetworkConfig;
use jetsam_chain::storage::snapshot_staging::{
    AuthenticatedSnapshotMetadata, FinalizedSnapshotStaging, SnapshotStagingError,
    SnapshotStagingSession,
};
use jetsam_chain::storage::{MdbxChainContext, MdbxStore, SnapshotSegmentDescriptor};
use jetsam_mempool::{AsyncMempool, ChainView, MempoolConfig};
use jetsam_miner::{BlockMiner, MinerConfig};
use jetsam_node::snapshot_header_staging::{
...
use jetsam_p2p::{NetworkEvent, P2PNetwork};
use jetsam_rpc::{start_rpc_server, ExternalMiningAttemptInvalidator, WalletOperationGate};

struct AppliedCompactSuffix {
    height: u64,
    block_hash: [u8; 32],
    confirmed_tx_hashes: Vec<jetsam_poseidon2b::primitives::TxBodyHash>,
    view: ChainView,
    applied_blocks: u64,
    payload_bytes: u64,
...
struct AppliedReorg {
    result: jetsam_chain::consensus::ReorgResult,
    confirmed_tx_hashes: Vec<jetsam_poseidon2b::primitives::TxBodyHash>,
    view: ChainView,
}
...
fn try_dispatch_header_fetch(
    ...
    if p2p_cmd
        .try_send(jetsam_p2p::NetworkCommand::FetchHeaders {
            peer,
            start_height,
            count,
        })
...
fn finalized_header_search_floor(local_height: u64) -> u64 {
    local_height.saturating_sub(jetsam_chain::consensus::params::CONSENSUS_FINALITY_DEPTH)
}
...
fn unresolved_selected_tip_probe_range(
    ...
    committed_tip: jetsam_node::networking::ChainPoint,
    forward_headers: u16,
) -> Result<(u64, u16), jetsam_node::networking::header_dag::HeaderDagError> {
...
const CONNECTED_TIP_PROBE_HEADERS: u16 =
    jetsam_chain::consensus::params::RECENT_BLOCK_RETENTION_DEPTH as u16 + 2;
...
struct MiningPeerQuorum {
    ...
    readiness: jetsam_node::networking::mining_readiness::MiningReadiness,
    connected: std::collections::HashMap<libp2p::PeerId, jetsam_node::networking::FailureDomain>,
...
impl MiningPeerQuorum {
    fn new(
        ...
        let initial = jetsam_node::networking::ChainPoint::new(0, [0u8; 32]);
        let quorum = Self {
            isolated,
            origin,
            initial_sync_complete: isolated,
            unresolved_better_header: false,
            readiness: jetsam_node::networking::mining_readiness::MiningReadiness::new(
                isolated,
                MINING_PEER_QUORUM,
                initial,
            ),
    ...
    fn connect(
        &mut self,
        peer: libp2p::PeerId,
        failure_domain: jetsam_node::networking::FailureDomain,
    ) {
    ...
    fn set_canonical_tip_state(
        ...
        let tip = jetsam_node::networking::ChainPoint::new(height, hash);
    ...
    fn reconcile_canonical_tip(&mut self, height: u64, hash: [u8; 32], prev_hash: [u8; 32]) {
        ...
        let current = jetsam_node::networking::ChainPoint::new(height, hash);
        if previous == current {
            return;
        }
    ...
    fn confirm_tip_at(
        ...
        let point = jetsam_node::networking::ChainPoint::new(height, hash);
        let Some(failure_domain) = self.connected.get(&peer).copied() else {
            return;
        };
...
fn terminal_transport_can_retry_same_peer(kind: jetsam_p2p::RequestFailureKind) -> bool {
    matches!(
        kind,
        jetsam_p2p::RequestFailureKind::Timeout | jetsam_p2p::RequestFailureKind::Io
    )
}
...
fn embedded_history_step_cache_file(
    ...
    Some(
        history_step_cache_directory(data_dir, pack.runtime_metadata_digest()).join(
            jetsam_miner::history_step_runtime_image_file_name(class.class_id()),
        ),
    )
}
...
fn prepare_history_step_ghost_authorization() -> Result<
    ...
    String,
> {
    jetsam_miner::install_history_step_phase_cpu(|| {
        let proof = jetsam_gkr::ghost_tx::prove_selected_ghost_authorization()
            .map_err(|error| format!("canonical ghost authorization proof failed: {error}"))?;
        jetsam_recursive::acceptance::history_step::prepare_history_step_ghost_authorization(proof)
            .map(Arc::new)
...
impl HistoryStepCacheClass {
    fn class_id(self) -> jetsam_recursive::CanonicalHistoryStepClassId {
        jetsam_recursive::CanonicalHistoryStepClassId::new(match self {
            Self::B25 => 0,
            Self::B255 => 1,
        })
...
fn network_storage_epoch_bytes() -> Vec<u8> {
    ...
    let genesis_id = jetsam_chain::block_header::block_id(&genesis);
...
/// Record which network this directory was last used with. Informational only.
///
/// Until 2026-09-07 a marker that did not match deleted every entry of the data
/// directory — the wallet with it — and that ran on any invocation of the
/// binary, including `--export-wallet-secret`. The evidence was never worth the
/// verdict: a dotfile is dropped by `cp data/*`, skipped by backup tools and
/// lost to a disk repair, none of which means the directory belongs to another
/// network. And a wallet kept one level down, as a backup vault keeps several,
/// was deleted without even being copied aside.
///
/// The question the marker was pretending to answer is now asked of the only
/// witness that cannot be lost by copying files around — the genesis header the
/// database itself holds, see `refuse_foreign_chain_data`. This function writes
/// a record and deletes nothing, ever.
fn record_network_storage_epoch(data_dir: &Path) -> anyhow::Result<()> {
    if network_storage_epoch_is_current(data_dir)? {
        return Ok(());
    }
    tracing::info!(
        directory = %data_dir.display(),
        "recording this directory as belonging to the current network"
    );
    persist_network_storage_epoch_marker(data_dir)
}

/// Refuse to run against a database built for a different network.
///
/// Nothing is deleted either way. A directory that does not belong to this
/// network is the operator's to move; erasing it on our own initiative is what
/// this whole area used to do, and it is what cost people their coins upstream.
fn refuse_foreign_chain_data(data_dir: &Path) -> anyhow::Result<()> {
    // No database yet: a fresh install, or a directory holding only a wallet.
    // There is nothing to disagree with.
    if !data_dir.join("mdbx.dat").exists() {
        return Ok(());
    }
    let store = match MdbxStore::open(data_dir) {
        Ok(store) => store,
        // An unreadable database is a separate problem, handled where the chain
        // is actually opened. It is never a reason to delete anything here.
        Err(error) => {
            tracing::warn!(%error, "could not inspect the chain database before start");
            return Ok(());
        }
    };
    let stored = match store.get_header(0) {
        Ok(Some(header)) => header,
        _ => return Ok(()),
    };
    let expected = jetsam_chain::consensus::genesis::genesis_header();
    let (found, want) = (
        jetsam_chain::block_header::block_id(&stored),
        jetsam_chain::block_header::block_id(&expected),
    );
    if found != want {
        anyhow::bail!(
            "the chain database in {} was built for a different network \n\
             (its genesis is {}, this build expects {}).\n\
             Nothing has been deleted, and your wallet is still in that directory.\n\
             Point --data-dir at another directory, or move this one aside yourself.",
            data_dir.display(),
            hex::encode(found),
            hex::encode(want)
        );
...
async fn main() -> anyhow::Result<()> {
    ...
    // The database decides which network this directory belongs to, and nothing
    // is deleted on our own initiative. The configuration the operator wrote is
    // also left alone: the old branch threw it away on every first start, and
    // rewrote the default install's jetsam.toml to point at whichever data
    // directory happened to be starting.
    refuse_foreign_chain_data(&data_dir)?;
    record_network_storage_epoch(&data_dir)?;
    ...
    let cpu_budget_mode = if cli.mode != NodeMode::Node {
        jetsam_miner::ProcessCpuBudgetMode::InternalMiner
    } else {
        jetsam_miner::ProcessCpuBudgetMode::ProofOnly
    };
    let cpu_plan = jetsam_miner::configure_process_cpu_budget_with_threads(
        cpu_budget_mode,
    ...
    let initial_canonical_tip = {
        ...
        jetsam_p2p::object_protocol::ChainPoint::new(ctx.tip_height(), ctx.tip_hash())
    };
    ...
    let authorization_verification_executor: jetsam_mempool::AuthorizationVerificationExecutor =
        Arc::new(|task: jetsam_mempool::AuthorizationVerificationTask| {
            jetsam_miner::install_inbound_verifier_cpu(task).map_err(|error| {
    ...
    // --- P2P Network ---
    let topics = jetsam_p2p::protocol::NetworkTopics::for_network_cfg(&net);
    ...
    tokio::spawn(async move {
        loop {
            match mp_events.recv().await {
                Ok(jetsam_mempool::MempoolEvent::TxAdmitted { intent_bytes, .. }) => {
                    let _ = p2p_tx_relay
    ...
    let miner_handle = if cfg.mining.enabled {
        ...
        let miner_addr = if cfg.mining.miner_address.is_empty() {
            ...
            guard
                ...
                .unwrap_or(jetsam_poseidon2b::primitives::Address([0u8; 32]))
        } else {
        ...
        // Remote wallets use P2P block subscription independently.
        {
            ...
            miner.set_block_applied_hook(std::sync::Arc::new(move |block| {
                ...
                hook_canonical_tip_changes.send_replace(
                    jetsam_p2p::object_protocol::ChainPoint::new(
                        block.header.height,
                        jetsam_chain::block_id(&block.header),
                    ),
                );
            }));
        }
        ...
        tokio::spawn(async move {
            loop {
                match miner_rx.recv().await {
                    Ok(jetsam_miner::MinerEvent::BlockFound {
                        bundle,
                        height,
                        hash,
                        n_txs,
                        ...
                        let mut command = jetsam_p2p::NetworkCommand::AnnounceBlock { bundle };
                        loop {
                    ...
                    Ok(jetsam_miner::MinerEvent::ProveFailed { height, error }) => {
    ...
    // --- Startup Banner ---
    {
        ...
        use jetsam_chain::fri_state::LOG_SEGMENT_SIZE;
...
struct PendingSnapshotHeaderSync {
    ...
    snapshot: jetsam_node::networking::SnapshotId,
...
impl BoundaryProofTarget {
    ...
    fn block_hash(self) -> [u8; 32] {
        jetsam_chain::block_header::block_id(&self.header)
    }
}
...
fn missing_snapshot_boundary_proof(
    ...
    let interval = jetsam_p2p::protocol::SNAPSHOT_BOUNDARY_INTERVAL;
    ...
    let block_hash = jetsam_chain::block_header::block_id(&header);
    ...
    let semantic_id = jetsam_chain::block_header::semantic_header_id(&header);
    ...
    let epoch_anchor_height =
        jetsam_chain::consensus::tx_epoch_anchor_height_for_child(header.height);
...
struct ReadySnapshotHeaderRange {
    ...
    count: u16,
    attempted_peers: std::collections::HashSet<libp2p::PeerId>,
    headers: Vec<jetsam_chain::BlockHeader>,
}
...
impl SnapshotHeaderPipeline {
    ...
    fn failure_plan(
        ...
        count: u16,
        token: u64,
        kind: jetsam_p2p::RequestFailureKind,
        peers: &std::collections::HashSet<libp2p::PeerId>,
        ...
        if matches!(kind, jetsam_p2p::RequestFailureKind::Timeout)
...
fn dispatch_snapshot_header_plans(
    ...
    for request in plans {
        let dispatched = !data_lane_full
            && p2p_cmd
                .try_send(jetsam_p2p::NetworkCommand::FetchSnapshotHeaders {
                    generation,
                    token: request.token,
                    peer: request.peer,
...
fn highest_snapshot_header_boundary(
    store: &jetsam_chain::storage::MdbxStore,
    target_height: u64,
...
fn prepare_snapshot_header_sync(
    ...
    Ok(PendingSnapshotHeaderSync {
        snapshot: jetsam_node::networking::SnapshotId {
            boundary: jetsam_node::networking::ChainPoint::new(
                manifest.tip_height,
                manifest.tip_hash,
            ),
...
struct VerifiedHistoryStepSnapshot {
    height: u64,
    block_hash: [u8; 32],
    boundary: jetsam_chain::VerifiedSnapshotBoundary,
...
fn validate_snapshot_staged_header_boundary(
    ...
    if bridge_span > jetsam_chain::consensus::params::RECENT_BLOCK_RETENTION_DEPTH {
    ...
    let expected_epoch_height =
        jetsam_chain::consensus::tx_epoch_anchor_height_for_child(manifest.tip_height);
...
fn exact_suffix_context_error_is_body_peer_fault(
    ...
    match error {
        ...
        jetsam_chain::storage::MdbxContextError::Consensus(_) => true,
        jetsam_chain::storage::MdbxContextError::Corrupt(message) => matches!(
            *message,
        ...
        jetsam_chain::storage::MdbxContextError::Store(_)
        | jetsam_chain::storage::MdbxContextError::ResourceLimit { .. } => false,
    }
}
...
fn validate_history_step_tip_future_drift(
    ...
    jetsam_chain::consensus::validate_future_drift(boundary.tip_header.timestamp, local_time)
...
async fn apply_exact_suffix_offthread(
    ...
    let result = tokio::task::spawn_blocking(move || {
        ...
        for ((bytes, source), expected) in body_bytes
            ...
            .zip(plan.headers())
        {
            let block = jetsam_chain::Block::from_bytes(bytes).map_err(|error| {
        ...
        let tip_header = blocks
            ...
            .header;
        if jetsam_chain::block_id(&tip_header) != plan.target().hash {
            return Err(ExactSuffixApplyError::body(
                *body_sources
                    .last()
        ...
        let epoch_height =
            jetsam_chain::consensus::tx_epoch_anchor_height_for_child(tip_header.height);
        ...
        match plan.kind() {
            SyncPlanKind::LiveSuffix => {
                ...
                {
                    let txids = match jetsam_chain::try_compute_logical_txids(&block.transactions) {
                        Ok(txids) => txids,
                        Err(error) => {
...
enum HeaderInventoryPlan {
    Confirmed {
        tip: jetsam_node::networking::ChainPoint,
    },
    Behind,
    NeedOlder {
        start_height: u64,
    ...
    Candidate {
        headers: Vec<jetsam_node::networking::header_dag::ValidatedHeader>,
        records: Vec<jetsam_p2p::header_protocol::HeaderInventoryRecord>,
        old_tip: jetsam_node::networking::ChainPoint,
...
fn canonical_header_dag(
...
) -> Result<jetsam_node::networking::header_dag::HeaderDag, String> {
    use jetsam_node::networking::{header_dag::ValidatedHeader, ChainPoint};
    ...
    let mut dag = jetsam_node::networking::header_dag::HeaderDag::new(
...
fn reconcile_canonical_header_dag(
    context: &MdbxChainContext,
    dag: &mut jetsam_node::networking::header_dag::HeaderDag,
) -> Result<(), String> {
    use jetsam_node::networking::{header_dag::ValidatedHeader, ChainPoint};
    ...
    for height in dag.finalized().height.saturating_add(1)..=context.tip_height() {
        ...
        let hash = jetsam_chain::block_header::block_id(&header);
...
fn record_validated_headers(
    dag: &mut jetsam_node::networking::header_dag::HeaderDag,
    headers: &[jetsam_node::networking::header_dag::ValidatedHeader],
) -> Result<(), jetsam_node::networking::header_dag::HeaderDagError> {
...
fn advertise_inventory_for_known_headers(
    dag: &mut jetsam_node::networking::header_dag::HeaderDag,
    peer: libp2p::PeerId,
    records: &[jetsam_p2p::header_protocol::HeaderInventoryRecord],
) -> Result<usize, jetsam_node::networking::header_dag::HeaderDagError> {
    let known_inventory = records
        .iter()
        .filter(|record| record.body.is_some() || record.terminal.is_some())
        .filter(|record| {
            let hash = jetsam_chain::block_id(&record.header);
            dag.get(&hash)
...
fn header_inventory_validation_anchor(
    canonical: Option<jetsam_node::networking::ChainPoint>,
    validated_dag: Option<jetsam_node::networking::ChainPoint>,
) -> Option<jetsam_node::networking::ChainPoint> {
...
fn header_dag_has_unresolved_better_tip(
    dag: &jetsam_node::networking::header_dag::HeaderDag,
    canonical_tip: jetsam_node::networking::ChainPoint,
...
async fn plan_header_inventory(
    ...
    for header in competing_headers {
        cumulative_work = jetsam_chain::add_work(
            &cumulative_work,
            &jetsam_chain::block_work(&header.difficulty_target),
        );
...
fn source_independent_suffix_offer(
    ...
    preferred_peer: libp2p::PeerId,
    old_tip: jetsam_node::networking::ChainPoint,
    base: jetsam_node::networking::ChainPoint,
    headers: Vec<jetsam_node::networking::header_dag::ValidatedHeader>,
) -> Result<
    (
        libp2p::PeerId,
        jetsam_node::networking::suffix_sync::SuffixOffer,
        Vec<(
            libp2p::PeerId,
            Vec<jetsam_p2p::header_protocol::HeaderInventoryRecord>,
        )>,
    ),
    jetsam_node::networking::suffix_sync::SuffixSyncError,
> {
    use jetsam_node::networking::suffix_sync::{SuffixOffer, SuffixSyncError};
    ...
    let mut bootstrap = headers
        .iter()
        .map(|header| jetsam_p2p::header_protocol::HeaderInventoryRecord::header_only(header.header))
        .collect::<Vec<_>>();
    bootstrap
        .last_mut()
...
fn dispatch_exact_suffix_requests(
    sync: &mut jetsam_node::networking::suffix_sync::SuffixSync,
    p2p_cmd: &jetsam_p2p::NetworkCommandSender,
) {
    ...
    for request in sync.schedule(now_ms) {
        if p2p_cmd
            .try_send(jetsam_p2p::NetworkCommand::FetchObjects {
                token: request.token,
                peer: request.peer,
...
fn merge_active_suffix_inventory(
    active: &mut Option<jetsam_node::networking::suffix_sync::SuffixSync>,
    peer: libp2p::PeerId,
    failure_domain: jetsam_node::networking::FailureDomain,
    records: &[jetsam_p2p::header_protocol::HeaderInventoryRecord],
) -> Result<usize, jetsam_node::networking::suffix_sync::SuffixSyncError> {
...
fn admit_exact_suffix_offer(
    ...
    peer: libp2p::PeerId,
    failure_domain: jetsam_node::networking::FailureDomain,
    offer: jetsam_node::networking::suffix_sync::SuffixOffer,
) -> Result<SuffixAdmission, String> {
    use jetsam_chain::consensus::fork_choice::ChainChoice;
    use jetsam_node::networking::suffix_sync::SuffixSync;
...
mod tests {
    use super::{
        ...
        persist_network_storage_epoch_marker, record_network_storage_epoch,
    ...
    fn test_snapshot_id() -> jetsam_node::networking::SnapshotId {
        jetsam_node::networking::SnapshotId {
            boundary: jetsam_node::networking::ChainPoint::new(100, [1; 32]),
            state_root: [2; 32],
            manifest_digest: [3; 32],
            format_version: jetsam_p2p::protocol::SNAPSHOT_MANIFEST_FORMAT_VERSION,
        }
    }
    ...
    fn dag_selected_suffix_merges_objects_from_independent_sources() {
        ...
        let second_work = jetsam_chain::add_work(
            &first_work,
            &jetsam_chain::block_work(&second_header.difficulty_target),
        );
        ...
        assert_eq!(
            source_independent_suffix_offer(&dag, tip_peer, base, base, headers.clone())
                .unwrap_err(),
            jetsam_node::networking::suffix_sync::SuffixSyncError::MissingBodySource
        );
        assert_eq!(
    ...
    /// The bomb, disarmed. A marker that belongs to another network used to
    /// delete every entry of the data directory, wallet included. It now
    /// records and moves on: a dotfile is lost by `cp data/*`, skipped by
    /// backup tools and dropped by disk repair, and none of that is evidence
    /// worth destroying somebody's money over.
    #[test]
    fn a_foreign_marker_deletes_nothing_at_all() {
        ...
        .unwrap();
        std::fs::write(directory.path().join("wallet.key"), b"top level wallet").unwrap();
        std::fs::write(directory.path().join("peers.json"), b"peers").unwrap();
        std::fs::write(directory.path().join("mdbx.dat"), b"chain data").unwrap();
        std::fs::write(
            directory.path().join(".network-storage-epoch"),
            b"jetsam/some-other-network/v1/\n",
        )
        .unwrap();

        let before = tree_snapshot(directory.path());
        record_network_storage_epoch(directory.path()).unwrap();
        let after = tree_snapshot(directory.path());

        let untouched = |t: &Vec<(String, Vec<u8>)>| -> Vec<(String, Vec<u8>)> {
            t.iter()
                .filter(|(name, _)| name != ".network-storage-epoch")
                .cloned()
                .collect()
        };
        assert_eq!(
            untouched(&before),
            untouched(&after),
            "the data directory was modified; nothing but the marker may change"
        );
        assert!(network_storage_epoch_is_current(directory.path()).unwrap());
    }

    /// A directory that holds no chain database has nothing to disagree about.
    #[test]
    fn a_directory_without_a_database_is_never_refused() {
        let directory = tempfile::tempdir().unwrap();
        std::fs::write(directory.path().join("wallet.key"), b"only a wallet").unwrap();
        refuse_foreign_chain_data(directory.path()).expect("a wallet-only directory must be usable");
    }
    ...
    fn moving_tip_does_not_mutate_an_inflight_plan() {
        ...
        assert_eq!(
            admit_exact_suffix_offer(
                &mut active,
                first_peer,
                jetsam_node::networking::FailureDomain(1),
                pinned,
            )
            .unwrap(),
        ...
        assert_eq!(
            admit_exact_suffix_offer(
                &mut active,
                extension_peer,
                jetsam_node::networking::FailureDomain(2),
                extension,
            )
            .unwrap(),
    ...
    fn behind_header_range_still_supplies_the_pinned_suffix_objects() {
        use jetsam_node::networking::FailureDomain;
        use jetsam_p2p::{header_protocol::HeaderInventoryRecord, object_protocol::ObjectId};
        ...
        let records = vec![
            HeaderInventoryRecord::header_only(jetsam_chain::consensus::genesis_header()),
            HeaderInventoryRecord {
        ...
        let mut active = Some(
            jetsam_node::networking::suffix_sync::SuffixSync::from_offer(
                original_peer,
                FailureDomain(1),
                offer,
            )
    ...
    fn only_transient_terminal_transport_failures_retry_the_same_peer() {
        use jetsam_p2p::RequestFailureKind;
    ...
    fn mining_gate_accepts_one_confirmed_ordinary_peer() {
        ...
        quorum.set_canonical_tip(height, hash, false);
        quorum.set_sync_state(true, false);
        quorum.connect(first, jetsam_node::networking::FailureDomain(1));
    ...
    fn mining_quorum_reacquisition_prioritizes_unconfirmed_public_peers() {
        ...
        for (index, peer) in peers.iter().enumerate() {
            quorum.connect(*peer, jetsam_node::networking::FailureDomain(index as u64));
        }
        let height = 31;
        let hash = [0x31; 32];
        quorum.set_canonical_tip(height, hash, false);
    ...
    fn verified_exact_suffix_source_reauthorizes_the_committed_tip_immediately() {
        ...
        quorum.connect(source, jetsam_node::networking::FailureDomain(1));
        quorum.connect(second_announcer, jetsam_node::networking::FailureDomain(2));
    ...
    fn header_only_candidate_does_not_pause_until_exact_plan_exists() {
        ...
        quorum.connect(peer, jetsam_node::networking::FailureDomain(1));
        quorum.set_canonical_tip(height, hash, false);
        quorum.set_sync_state(true, false);
    ...
    fn sync_mode_uses_guaranteed_object_serving_window_boundary() {
        let retention = jetsam_chain::consensus::params::RETAINED_BLOCK_SERVING_DEPTH;
    ...
    fn shorter_peer_discovery_reads_only_the_complete_nonfinal_window() {
        let finality = jetsam_chain::consensus::params::CONSENSUS_FINALITY_DEPTH;
        assert_eq!(
    ...
    fn unresolved_reorg_probe_starts_at_the_common_ancestor() {
        ...
        let local_work = jetsam_chain::add_work(
            &base_work,
            &jetsam_chain::block_work(&local_header.difficulty_target),
        );
        ...
        let competing_tip_work = jetsam_chain::add_work(
            &local_work,
            &jetsam_chain::block_work(&competing_tip_header.difficulty_target),
        );
        let competing_tip =
    ...
    fn snapshot_segment_errors_separate_remote_bytes_from_local_state() {
        use jetsam_chain::storage::snapshot_staging::SnapshotStagingError;
        ...
        assert!(matches!(
            classify_snapshot_session_prepare_error(
                jetsam_chain::storage::snapshot_staging::SnapshotStagingError::EffectiveLogMismatch {
                    expected: 16,
                    actual: 15,
                }
            ),
        ...
        assert!(matches!(
            classify_snapshot_session_prepare_error(
                jetsam_chain::storage::snapshot_staging::SnapshotStagingError::Io {
                    operation: "test",
                    source: std::io::Error::other("disk failed"),
                }
            ),
        ...
        assert!(matches!(
            classify_snapshot_finalization_error(
                jetsam_chain::storage::snapshot_staging::SnapshotStagingError::StateRootMismatch {
                    expected: [1; 32],
                    actual: [2; 32],
                }
            ),
        ...
        assert!(matches!(
            classify_snapshot_finalization_error(
                jetsam_chain::storage::snapshot_staging::SnapshotStagingError::StagedFileLength {
                    segment_id: 7,
                    expected: 10,
                    actual: 9,
                }
    ...
    fn snapshot_header_pipeline_uses_one_ordered_same_peer_window() {
        ...
        let headers = |start: u64, count: u16| {
            (start..start + u64::from(count))
                .map(|height| {
                    let mut header = jetsam_chain::consensus::genesis::genesis_header();
    ...
    fn snapshot_header_transport_failure_retries_only_its_exact_range() {
        ...
        let retry = pipeline
            .failure_plan(
                ...
                SNAPSHOT_HEADER_BATCH as u16,
                failed.token,
                jetsam_p2p::RequestFailureKind::Io,
                &peers,
            )
    ...
    fn snapshot_header_timeout_reduces_only_the_slow_range() {
        ...
        let retry = pipeline
            .failure_plan(
                ...
                initial.count,
                initial.token,
                jetsam_p2p::RequestFailureKind::Timeout,
                &peers,
            )
            .unwrap();
    ...
    fn snapshot_header_failures_try_every_connected_peer_before_reuse() {
        ...
        let retry_one = pipeline
            .failure_plan(
                ...
                initial.count,
                initial.token,
                jetsam_p2p::RequestFailureKind::InvalidResponse,
                &peers,
            )
            .unwrap();
    ...
    fn test_coinbase_child(
        ...
        let template = jetsam_chain::consensus::build_block_template(
            ...
            vec![],
            jetsam_poseidon2b::primitives::Address([0x22; 32]),
            timestamp,
            difficulty_target,
        )
    ...
    fn snapshot_history_boundary_checks_staged_header_chainwork() {
        let state = jetsam_chain::ChainState::with_log_slots(
            jetsam_chain::consensus::params::LOG_SLOTS_GENESIS
                .try_into()
                .expect("genesis log_slots fits usize"),
        );
        let h0 = jetsam_chain::consensus::genesis_header();
        let h0_hash = jetsam_chain::hash_block_header(&h0);
        let high_start_work = jetsam_chain::consensus::block_work(&h0.difficulty_target);

        let block = test_coinbase_child(&h0, &state);
        let h1 = block.header;
        let h1_hash = jetsam_chain::hash_block_header(&h1);
        let h1_work = jetsam_chain::consensus::add_work(
            &high_start_work,
            &jetsam_chain::consensus::block_work(&h1.difficulty_target),
        );
        let mut manifest = jetsam_p2p::protocol::GetStateManifestResponse {
            tip_height: 1,
            tip_hash: h1_hash,
    ...
    fn snapshot_epoch_anchor_obeys_start_of_block_boundaries() {
        ...
        for (tip_height, expected_epoch_height) in [
            ...
            let mut tip_header = jetsam_chain::consensus::genesis_header();
            tip_header.height = tip_height;
            let tip_hash = jetsam_chain::hash_block_header(&tip_header);
            ...
            assert!(manifest.seal_manifest_digest());
            let mut epoch_anchor_header = jetsam_chain::consensus::genesis_header();
        ...
        let tip_height = jetsam_chain::consensus::params::TX_EPOCH_BLOCKS;
        let mut tip_header = jetsam_chain::consensus::genesis_header();
        tip_header.height = tip_height;
        let tip_hash = jetsam_chain::hash_block_header(&tip_header);
        ...
        assert!(manifest.seal_manifest_digest());
        let mut wrong_anchor = jetsam_chain::consensus::genesis_header();
    ...
    fn snapshot_history_step_tip_obeys_local_future_drift_admission() {
        ...
        let mut tip = jetsam_chain::consensus::genesis::genesis_header();
        tip.timestamp = local_time + jetsam_chain::consensus::params::MAX_FUTURE_DRIFT;
        let mut boundary = SnapshotHeaderBoundary {
            tip_header: tip,
            tip_hash: jetsam_chain::hash_block_header(&tip),
...
async fn handle_p2p_events(
    ...
    struct PendingManifest {
        ...
        rebase_base: Option<jetsam_node::networking::ChainPoint>,
        manifest: jetsam_p2p::protocol::VerifiedStateManifest,
        offer: jetsam_node::networking::snapshot_sync::SnapshotOffer,
    ...
    struct ExactSuffixApplyCompletion {
        plan_id: jetsam_node::networking::PlanId,
        target: jetsam_node::networking::ChainPoint,
        tip_announcement: jetsam_p2p::header_protocol::HeaderAnnouncement,
    ...
    let mut active_snapshot_sync: Option<jetsam_node::networking::snapshot_sync::SnapshotSync> = None;
    ...
    let mut active_suffix_sync: Option<jetsam_node::networking::suffix_sync::SuffixSync> = None;
    ...
    let mut exact_suffix_apply_inflight: Option<jetsam_node::networking::PlanId> = None;
    let mut peer_failure_domains: std::collections::HashMap<
        libp2p::PeerId,
        jetsam_node::networking::FailureDomain,
    > = std::collections::HashMap::new();
    ...
    let (mut header_dag, mut header_dag_canonical_tip) = {
        let ctx = chain.read().await;
        let canonical_tip =
            jetsam_node::networking::ChainPoint::new(ctx.tip_height(), ctx.tip_hash());
    ...
    let mut queued_segment_response: Option<(
        libp2p::PeerId,
        jetsam_p2p::protocol::GetStateSegmentResponse,
    )> = None;
    ...
    macro_rules! dispatch_snapshot_generation_advance {
        () => {{
            if p2p_snapshot_generation_announced != snapshot_sync_generation
                && p2p_cmd
                    .try_send(jetsam_p2p::NetworkCommand::AdvanceSnapshotGeneration {
    ...
    macro_rules! dispatch_pending_boundary_terminal_requests {
        () => {{
            if let Some(pending) = snapshot_boundary_terminal_inflight.as_mut() {
                ...
                for request in requests {
                    if p2p_cmd
                        .try_send(jetsam_p2p::NetworkCommand::RequestHistoryStepTerminal {
                            token: request.token,
                            peer: request.peer,
    ...
    macro_rules! retire_snapshot_plan {
        () => {{
            ...
            if let Some(request) = snapshot_boundary_terminal_inflight.take() {
                let _ = p2p_cmd
                    .send(jetsam_p2p::NetworkCommand::CancelHistoryStepTerminalRace {
        ...
        ($from:expr, $manifest:expr, $rebase_anchor:expr) => {{
            ...
            let rebase_base = snapshot_rebase_hint.and_then(|hint| {
                (hint.ancestor_height < terminal_height).then_some(
                    jetsam_node::networking::ChainPoint::new(
    ...
    macro_rules! mark_bootstrap_complete_if_caught_up {
        ($local_height:expr) => {{
            ...
            {
                if p2p_cmd
                    .send(jetsam_p2p::NetworkCommand::BootstrapComplete)
                    .await
                    .is_ok()
                {
    ...
    loop {
        ...
        match rx_item {
            Ok(NetworkEvent::HeaderAnnouncement {
                ...
                let exact_inventory = (source_has_objects && manifest_peers.contains(&from))
                    .then(|| {
                        jetsam_p2p::header_protocol::HeaderInventoryRecord::from_announcement(
                            announcement,
                        )
                    });
                ...
                let announced_hash = jetsam_chain::block_id(&announced_header);
                let (our_height, our_hash, finalized_height, canonical_hash_at_height) = {
                    ...
                    (
                        ...
                        ctx.header(height).map(jetsam_chain::block_id),
                    )
                };
                ...
                if gap_requires_snapshot_sync(our_height, height) {
                    ...
                    if let Err(e) = precheck {
                        if e == jetsam_chain::consensus::ConsensusError::BadParentHash {
                    ...
                    let cumulative_work = jetsam_chain::add_work(
                        &base_work,
                        &jetsam_chain::block_work(&announced_header.difficulty_target),
                    );
                    let validated = jetsam_node::networking::header_dag::ValidatedHeader::new_after_consensus_checks(
                        announced_header,
                        cumulative_work,
                    );
                    ...
                    {
                        match jetsam_node::networking::suffix_sync::SuffixOffer::live(
                            base,
                            vec![validated],
                            &[record],
                        ) {
                            Ok(offer) => {
                                let domain = peer_failure_domains.get(&from).copied().unwrap_or(
                                    jetsam_node::networking::FailureDomain(u64::MAX),
                                );
            ...
            Ok(NetworkEvent::ObjectsResponse {
                ...
                match sync.accept_response(
                    ...
                    Err(
                        jetsam_node::networking::suffix_sync::SuffixSyncError::UnknownToken,
                    ) => {
                        tracing::debug!(
                            token,
                            peer = %from,
                    ...
                    Err(
                        jetsam_node::networking::suffix_sync::SuffixSyncError::ContentMismatch,
                    ) => {
            ...
            Ok(NetworkEvent::ObjectsRequestFailed {
                ...
                match result {
                    Ok(()) => {
                        let rejected = matches!(kind, jetsam_p2p::RequestFailureKind::InvalidResponse);
            ...
            Ok(NetworkEvent::ObjectsRequestBusy {
                ...
                match sync.request_busy(
                    ...
                    Err(jetsam_node::networking::suffix_sync::SuffixSyncError::UnknownToken) => {}
                    Err(error) => {
            ...
            Ok(NetworkEvent::PeerConnected {
                ...
                peer_failure_domains.insert(
                    peer,
                    jetsam_node::networking::FailureDomain(failure_domain),
                );
                ...
                if request_mempool
                    ...
                    && p2p_cmd
                        .try_send(jetsam_p2p::NetworkCommand::RequestMempoolSync { peer })
                        .is_ok()
                {
            ...
            Ok(NetworkEvent::SnapshotHeadersRequestFailed {
                ...
                if kind == jetsam_p2p::RequestFailureKind::LocalCapacity {
            ...
            Ok(NetworkEvent::HeaderInventoryBatch {
                ...
                if !rejected_suffix_object_peers.contains(&from) {
                    let domain = peer_failure_domains
                        .get(&from)
                        .copied()
                        .unwrap_or(jetsam_node::networking::FailureDomain(u64::MAX));
                    ...
                    Ok(HeaderInventoryPlan::Candidate {
                        ...
                        let needs_snapshot = if base == old_tip {
                            ...
                            old_tip.height.saturating_sub(base.height)
                                > jetsam_chain::consensus::params::CONSENSUS_FINALITY_DEPTH
                                || headers.len()
                                    > jetsam_chain::consensus::params::CONSENSUS_FINALITY_DEPTH
                                        as usize
                                        * 2
                        ...
                        match source_independent_suffix_offer(
                            ...
                            Err(
                                jetsam_node::networking::suffix_sync::SuffixSyncError::MissingTipTerminal,
                            ) => {
                                let domain = peer_failure_domains.get(&from).copied().unwrap_or(
                                    jetsam_node::networking::FailureDomain(u64::MAX),
                                );
                ...
                if manifest.tip_height > 0 {
                        ...
                        .is_some_and(|pending| pending.manifest.as_ref() == manifest.as_ref())
                    {
                        ...
                        if let Some(sync) = active_snapshot_sync.as_mut() {
                            ...
                            let offer = jetsam_node::networking::snapshot_sync::SnapshotOffer::from_verified_manifest(
                                manifest.clone(),
                            );
            ...
            Ok(NetworkEvent::StateSegment { from, response }) => {
                ...
                if snapshot_staging_inflight.is_some() && queued_segment_response.is_some() {
                    ...
                    let request =
                        jetsam_node::networking::snapshot_sync::SnapshotSegmentRequest {
                            peer: from,
                            segment,
                        };
                ...
                if let Err(error) = sync.accept_response(from, segment, data_len) {
                    ...
                    if matches!(
                        error,
                        jetsam_node::networking::snapshot_sync::SnapshotSyncError::ResponseLengthMismatch
                    ) {
            ...
            Ok(NetworkEvent::HistoryStepTerminal {
                ...
                if let Some(maintenance_key) = maintenance_key {
                    ...
                    let _ = p2p_cmd.try_send(
                        jetsam_p2p::NetworkCommand::CancelHistoryStepTerminalRace { token },
                    );
            ...
            Ok(NetworkEvent::HistoryStepTerminalRequestFailed {
                ...
                let source_exhausted = match kind {
                    ...
                    jetsam_p2p::RequestFailureKind::Unavailable
                    | jetsam_p2p::RequestFailureKind::UnsupportedProtocol => {
                        manifest_terminal_capabilities.remove(&from);
                        snapshot_terminal_exhausted.insert(source_key);
                        true
                    }
                    jetsam_p2p::RequestFailureKind::Timeout
                    | jetsam_p2p::RequestFailureKind::Io
                    | jetsam_p2p::RequestFailureKind::Dial
            ...
            Err(jetsam_p2p::NetworkEventRecvError::Lagged(n)) => {
                tracing::warn!(n, "P2P gossip receiver lagged — recoverable gossip events dropped");
            }
            Err(jetsam_p2p::NetworkEventRecvError::Closed) => {
            ...
            match completed.result {
                Ok(AppliedExactSuffix::Live(mut applied)) => {
                    ...
                    if complete {
                        ...
                        tokio::spawn(async move {
                            if relay
                                .send(jetsam_p2p::NetworkCommand::AnnounceAvailability {
                                    announcement,
                                })
                                .await
                                .is_err()
        ...
        completed = snapshot_install_completion_rx.recv() => {
            ...
            if let Some(token) = completed.key.terminal_request_token {
                let _ = p2p_cmd
                    .send(jetsam_p2p::NetworkCommand::CancelHistoryStepTerminalRace { token })
                    .await;
            }
            match completed.result {
                Ok(applied) => {
        ...
        completed = history_step_verification_rx.recv() => {
            ...
            sync_phase_telemetry.observe_header_scale(
                ...
                completed
                    .staged_header_count
                    .saturating_mul(jetsam_chain::BLOCK_HEADER_WIRE_SIZE as u64),
            );
            ...
            let semantic_header_id = jetsam_chain::block_header::semantic_header_id(&boundary_header);
            let mut snapshot_sync = match jetsam_node::networking::snapshot_sync::SnapshotSync::new(
                initial_peer,
                initial_domain,
        ...
        _ = heartbeat.tick() => {
            ...
            let (our_height, our_hash, our_prev_hash, our_work, reconciled_header_dag) = {
                let ctx = chain.read().await;
                let canonical_tip = jetsam_node::networking::ChainPoint::new(
                    ctx.tip_height(),
                    ctx.tip_hash(),
                );
            ...
            if let Some(reconciled) = reconciled_header_dag {
                ...
                match reconciled {
                    Ok(()) => {
                        header_dag_canonical_tip = jetsam_node::networking::ChainPoint::new(
                            our_height,
                            our_hash,
                        );
            ...
            let unresolved_header_waiting_for_inventory = active_suffix_sync.is_none()
                ...
                && header_dag_has_unresolved_better_tip(
                    &header_dag,
                    jetsam_node::networking::ChainPoint::new(our_height, our_hash),
                    our_work,
                );
                    ...
                    >= EXACT_INVENTORY_PROBE_INTERVAL
            {
                ...
                let committed_tip = jetsam_node::networking::ChainPoint::new(our_height, our_hash);
                ...
                .is_some_and(|pending| pending.requests.deadline_due(now))
            {
                ...
                let _ = p2p_cmd.try_send(
                    jetsam_p2p::NetworkCommand::CancelHistoryStepTerminalRace {
...
fn create_snapshot_staging_session(
    ...
    if jetsam_chain::block_header::block_id(&header) != manifest.tip_hash
...
fn dispatch_exact_snapshot_segments(
    sync: &mut jetsam_node::networking::snapshot_sync::SnapshotSync,
    p2p_cmd: &jetsam_p2p::NetworkCommandSender,
    ...
    for request in sync.schedule(now_ms, 1) {
        let command = jetsam_p2p::NetworkCommand::RequestStateSegment {
            peer: request.peer,
...
fn update_wallet_for_block(wallet: &SharedWallet, block: &jetsam_chain::block::Block) {
...
fn print_startup_banner(
    ...
    let bar: String = (0..bar_w)
        ...
        .collect();
    let effective_log = log_slots.min(jetsam_chain::fri_state::LOG_SEGMENT_SIZE as u32) as u8;
    let max_segment_bytes = jetsam_chain::storage::max_encoded_segment_len_for_eff_log(effective_log)
