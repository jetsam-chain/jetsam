...
use jetsam_chain::consensus::receipt::generate_receipt;
...
fn derive_output_creation_ids(
    ...
    jetsam_chain::validate_block_page_stream(&block.transactions)
    ...
    for (tx_index, tx) in block.transactions.iter().enumerate() {
        for (output_index, _output) in tx.body.outputs.iter().enumerate() {
            ...
            ids[tx_index][output_index] = Some(if tx.body.is_primary_coinbase_shape() {
                jetsam_chain::consensus::params::coinbase_creation_id(block.header.height)
            } else {
                counter
            });
        }
...
pub fn update_wallet_artifacts_from_block(
    ...
    active_address: jetsam_poseidon2b::primitives::Address,
    active_index: u32,
    block: &Block,
) {
    let block_hash = jetsam_chain::block_id(&block.header);
    ...
    let block_tx_hashes: Vec<[u8; 32]> = jetsam_chain::try_compute_logical_txids(&block.transactions)
...
pub fn update_active_wallet_from_block(
    ...
    let stream = jetsam_chain::validate_block_page_stream(&block.transactions).map_err(|error| {
        ...
        if !already_pending {
            if sent_from_wallet > 0 {
                ...
                history.push(TxHistoryEntry {
                    ...
                    direction: TxDirection::Received,
                    is_coinbase: false,
                    amount_micro_jtm: received_by_wallet,
                    peer_address: None,
                    timestamp,
...
mod tests {
    ...
    fn block(transactions: Vec<Transaction>, alloc_counter: u64) -> Block {
        let tx_root = jetsam_chain::try_compute_tx_root(&transactions).unwrap_or([0; 32]);
        Block {
            header: BlockHeader {
    ...
    fn logical_txid(transaction: &Transaction) -> [u8; 32] {
        jetsam_tx::validate_paged_spend(&[jetsam_tx::TxPage {
    ...
    fn incremental_scan_assigns_coinbase_then_user_creation_ids() {
        ...
        assert_eq!(
            ...
            jetsam_chain::consensus::params::coinbase_creation_id(7),
        ...
        let expected_block_hash = jetsam_chain::block_id(&block.header);
        assert!(history
            .iter()
    ...
    fn incremental_scan_assigns_normal_ids_to_development_payout_outputs() {
        let owner = jetsam_chain::consensus::NETWORK_FUND_ADDRESS;
        let block = block(
            vec![
        ...
        assert_eq!(
            ids[0][0],
            Some(jetsam_chain::consensus::params::coinbase_creation_id(7))
        );
