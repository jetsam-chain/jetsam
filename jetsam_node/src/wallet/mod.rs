...
pub fn update_for_accepted_block(
    wallet: &SharedWallet,
    block: &jetsam_chain::block::Block,
) -> Result<(), String> {
    ...
    wallet.active_snapshot = Some(state::ActiveWalletSnapshot {
        ...
        tip_hash: jetsam_chain::consensus::pow::block_id(&block.header),
    ...
    let confirmed_block_hash = jetsam_chain::block_id(&block.header);
    for txid in jetsam_chain::try_compute_logical_txids(&block.transactions)
...
fn recover_outgoing_receipts_from_block(
    ...
    let tx_hashes = jetsam_chain::try_compute_logical_txids(&block.transactions)
    ...
    for (group_index, group) in stream.groups.iter().enumerate() {
        ...
        if let std::collections::hash_map::Entry::Vacant(entry) = wallet.receipts.entry(tx_hash) {
            entry.insert(
                jetsam_chain::consensus::receipt::generate_receipt(
                    &block.header,
                    pages,
                    tx_index,
                    &tx_hashes,
...
pub fn reconcile_receipts_at_startup(
    wallet: &mut WalletState,
    chain: &jetsam_chain::storage::MdbxChainContext,
    ...
    if first <= tip {
        for height in first..=tip {
            ...
            let block = jetsam_chain::block::Block::from_bytes(bundle.block_bytes())
...
pub fn install_reorg_snapshot_and_artifacts(
    ...
    reclaimed_tx_hashes: &[jetsam_poseidon2b::primitives::TxBodyHash],
    replacement_blocks: &[&jetsam_chain::block::Block],
) -> Result<(), String> {
    let mut guard = wallet
        .lock()
        .map_err(|_| "wallet state lock is poisoned".to_string())?;
...
impl WalletOps for WalletHandle {
    ...
    fn on_accepted_block(&self, block: &jetsam_chain::block::Block) -> Result<(), String> {
    ...
    fn plan_send(
        ...
        available.sort_by_key(|utxo| {
            (
                ...
                utxo.slot_index >> jetsam_chain::consensus::params::LOG_SEGMENT_SIZE,
                utxo.slot_index,
            )
        });
        ...
        for input_count in 1..=MAX_PAGED_SPEND_INPUTS {
            ...
            let one_output_breakdown = jetsam_chain::consensus::fee_breakdown(
                input_count as u64,
                1,
                active_slot_count,
                log_slots,
                    ...
                    entry.tx_hash == tx_hash
                        ...
                        && entry.peer_address.is_some_and(|peer| {
                            entry.own_address.as_deref().is_some_and(|own| {
                                jetsam_poseidon2b::primitives::Address(peer).to_bech32() == own
                            })
                        })
                });
...
mod tests {
    ...
    fn retained_body_rebuilds_missing_outgoing_receipt_without_history_marker() {
        ...
        let header = BlockHeader {
            ...
            state_root: [0; 32],
            tx_root: jetsam_chain::block::compute_tx_root(&transactions),
            timestamp: 123,
            height: 7,
    ...
    fn automatic_fee_path_selects_more_than_one_physical_page() {
        ...
        assert!(plan.input_count > jetsam_tx::TX_INPUTS);
    ...
    fn building_is_side_effect_free_until_pending_reservation_is_installed() {
        ...
        let intent =
            jetsam_tx::PagedSpendIntent::from_bytes(&intent_bytes).expect("decode built intent");
    ...
    fn reorg_installs_exact_snapshot_instead_of_replaying_on_old_cache() {
        ...
        let snapshot = VerifiedOwnerSnapshot {
            ...
            active_slot_count: 1,
            alloc_counter: 8,
            utxos: vec![jetsam_chain::storage::VerifiedOwnerUtxo {
                slot_index: 99,
                amount: 777,
                creation_id: 8,
            }],
    ...
    fn reorg_removes_receipts_bound_to_orphaned_blocks() {
        ...
        install_reorg_snapshot_and_artifacts(
            ...
            &std::collections::HashSet::new(),
            &std::collections::HashSet::new(),
            &[jetsam_poseidon2b::primitives::TxBodyHash(orphan_hash)],
            &[],
        )
        .unwrap();
    ...
    fn reorg_preserves_displaced_coinbase_as_mining_history() {
        ...
        install_reorg_snapshot_and_artifacts(
            ...
            &std::collections::HashSet::new(),
            &std::collections::HashSet::new(),
            &[jetsam_poseidon2b::primitives::TxBodyHash(coinbase_txid)],
            &[],
        )
        .unwrap();
    ...
    fn rejected_incremental_update_does_not_advance_snapshot_or_cache() {
        ...
        let mut outputs = [jetsam_tx::TxOutput::dummy(); jetsam_tx::TX_OUTPUTS];
        outputs[0] = jetsam_tx::TxOutput {
            slot_index: 9,
            amount: 50,
            owner,
        };
        let body = jetsam_tx::TxBody {
            epoch_anchor: [0; 32],
            fee: 0,
            input_owner: jetsam_poseidon2b::primitives::Address([0; 32]),
            inputs: [jetsam_tx::TxInput::dummy(); jetsam_tx::TX_INPUTS],
            outputs,
            validity_bitmap: jetsam_tx::output_bitmap_bit(0),
            is_coinbase: true,
        };
        let malformed = jetsam_chain::block::Block {
