...
| 组件 | 职责 |
|---|---|
| `jetsam_tx` | 逻辑 `PagedSpend`、交易标识与授权绑定 |
| `jetsam_mempool` | 交易意图准入、冲突预留与转发策略 |
| `jetsam_miner` | 交易选择、State 见证数据、区块准备与 PoW |
| `jetsam_recursive` | `HistoryStep` 关系、终端验证与递归 |
| `jetsam_chain` | 共识规则、MDBX State、区块头、重组与快照 |
| `jetsam_p2p` | GossipSub 转发、发现与同步协议 |
| `jetsam_node` | 运行时编排、钱包集成与关闭流程 |
| `jetsam_rpc` | 本地节点、钱包与外部挖矿接口 |
