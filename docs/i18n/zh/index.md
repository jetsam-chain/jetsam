...
Jetsam 消除的是对历史执行的依赖，并非不保存 State。Live State 传输量仍随其中的 UTXO 数量增长；不再随链龄增长的，是证明“该 State 为何有效”所需的执行量。
...
Jetsam 是透明系统，并非隐私链。金额、所有者和网络中转发的交易都是公开的。零知识保护的是支出见证数据。协议存储机制会减少节点在正常运行中保留的交易体，但无法阻止第三方归档公开交易。
