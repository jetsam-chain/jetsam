...
- Поля с плавающей точкой `*_eld` предназначены для отображения. Учётный код
  должен использовать соответствующее целое поле `*_micro_jtm`.
...
`waitBlockTemplate` возвращает ответ, как только `template_seq` отличается от
`known_seq`, поэтому пул узнаёт о новой работе без опроса. Передайте
`known_seq = 0`, чтобы получить ответ немедленно. `timeout_s` ограничивается
диапазоном 1…120; тот же самый номер после истечения тайм-аута означает «ничего
не изменилось», и вызывающая сторона просто повторяет вызов.
...
Внешний вычислитель заменяет поле с индексом `nonce_field_index` — читайте
этот индекс из ответа, не задавайте его константой. В этой сети он равен **0**.

Семь полей после пустой строки позволяют пулу ответить на вопрос «это та же
работа, которую я уже раздал, и сколько времени у меня осталось?» без сравнения
hex-строк. У них есть `serde(default)`, поэтому клиент, написанный для более
ранней ноды, продолжает разбирать эти ответы без изменений.

| Поле | Значение |
|---|---|
| `template_seq` | Строго возрастает в пределах процесса ноды. Два ответа с одинаковым номером описывают одну и ту же работу. |
| `parent_id` | ID родительского блока. Позволяет обнаружить смену вершины, не разбирая поля 1 и 2. |
| `miner_address` | Адрес bech32m, фактически записанный в coinbase. Проверьте, что он ваш, прежде чем раздавать задание. |
| `timestamp` | Отметка времени заголовка. Два шаблона на одном родителе различаются по ней. |
| `state` | `"preview"`, пока доказательство ещё строится, `"ready"`, когда отправка обслуживается сразу. |
| `ttl_remaining_ms` | Оставшееся время жизни, измеренное при построении этого ответа. |
| `difficulty` | `ceil(2^256 / target)` в десятичном виде — число, которое показывает панель. |

`expires_in_seconds` — константа и всегда сообщает полный TTL. Пул, повторно
раздающий шаблон, которому уже 40 секунд, обязан читать `ttl_remaining_ms`, а не
`expires_in_seconds`, иначе он раздаёт работу, которая умрёт раньше, чем майнеры
её закончат.
...
Каждый отказ внешнего майнинга несёт стабильный код, поэтому пул может
разветвлять логику по коду, а не разбирать текст сообщения:

| Код | Значение | Что делать вызывающей стороне |
|---|---|---|
| `-32020` | Шаблон неизвестен или удалён | Запросить новый шаблон |
| `-32021` | Шаблон истёк | Запросить новый шаблон |
| `-32022` | Уже занят другой отправкой | Отбросить долю, продолжать майнить |
| `-32024` | Родитель больше не является вершиной | Запросить новый шаблон |
| `-32025` | Дайджест не меньше цели | Nonce неверен; проверьте свою реализацию |
| `-32026` | Nonce не удалось декодировать | Отправьте 32 строчных hex-символа |
| `-32027` | Нода ещё не выдаёт работу | Прочитайте `data.reason`, подождите, повторите |

Каждый из них прикрепляет один и тот же объект данных:
...
`tip_height` и `tip_id` включены потому, что проигравшему гонку вызывающему
нужно знать, какой стала вершина, а повторный запрос стоит ещё одного круга.
`reason` задаётся только для `-32027` и принимает значения `"sync"`,
`"no_peer"` или `"better_header"`, чтобы оператор мог отличить «ещё
синхронизируется» от «нет пиров», не читая логи.

Неверный nonce отклоняется за микросекунды и не затрагивает чужой шаблон.
Верный nonce, отправленный, пока нода ещё строит доказательство, удерживается,
а не отклоняется: выигранный блок не выбрасывается из-за момента отправки.
