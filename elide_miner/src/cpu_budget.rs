...
impl ProcessCpuBudget {
    fn build(plan: ProcessCpuBudgetPlan) -> Result<Self, ProcessCpuBudgetError> {
        let phase_pool = rayon::ThreadPoolBuilder::new()
            ...
            .thread_name(|index| format!("elide-cpu-{index}"))
            .start_handler(|_| {
                ...
                elide_ivc_core::verifier::set_budgeted_large_stack_worker(true);
            })
            .exit_handler(|_| {
                elide_ivc_core::verifier::set_budgeted_large_stack_worker(false);
