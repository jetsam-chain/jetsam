...
use elide_mempool::{AsyncMempool, MempoolEvent};
...
impl Default for MinerConfig {
    fn default() -> Self {
        Self {
            ...
            refresh_interval_secs: elide_chain::consensus::params::BLOCK_TIME * 5,
        }
    }
}
...
pub type BlockAppliedHook = Arc<dyn Fn(&elide_chain::block::Block) + Send + Sync>;
...
pub struct BlockMiner {
    ...
    history_step_runtime: Arc<elide_recursive::acceptance::history_step::HistoryStepRuntime>,
    ghost_authorization:
                ...
                pow_res = &mut pow_handle => {
                    match pow_res {
                        Ok(Ok(Some(sol))) => {
                            ...
                            let diff_lz = elide_chain::consensus::target_leading_zero_bits(
    ...
    async fn apply_found_block(
        ...
        let (new_view, committed) = {
            ...
            tokio::task::spawn_blocking(move || {
                ...
                Ok::<_, elide_chain::storage::MdbxContextError>((view, committed))
            })
            .await
...
mod tests {
    ...
    fn payout_resolver_is_dynamic_while_configured_address_is_fixed() {
        let configured = elide_poseidon2b::primitives::Address([0x11; 32]);
        ...
        let resolver: MiningPayoutResolver = Arc::new(move || {
            elide_poseidon2b::primitives::Address([resolver_marker.load(Ordering::Relaxed); 32])
        });
        assert_eq!(
        ...
        assert_eq!(
            resolve_mining_payout(configured, Some(&resolver)),
            elide_poseidon2b::primitives::Address([0x33; 32])
        );
    }
    ...
    async fn previous_height_confirmation_does_not_cancel_fresh_pow() {
        ...
        sender
            .send(MempoolEvent::TxConfirmed {
                hash: elide_poseidon2b::primitives::TxBodyHash([0x11; 32]),
                block_height: 1,
            })
            .unwrap();
        ...
        sender
            .send(MempoolEvent::TxAdmitted {
                hash: elide_poseidon2b::primitives::TxBodyHash([0x22; 32]),
                fee: 1,
