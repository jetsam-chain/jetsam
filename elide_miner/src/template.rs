...
use elide_chain::consensus::difficulty::next_target;
use elide_chain::consensus::pow::block_id;
use elide_chain::consensus::template::BlockTemplate as ChainTemplate;
use elide_chain::consensus::AnchorInfo;
use elide_chain::state::ChainState;
use elide_chain::storage::{MdbxChainContext, MdbxContextError, MdbxStore};
...
pub struct BlockTemplate {
    ...
    pub(crate) prepared_state_commit: elide_chain::consensus::template::PreparedBlockStateCommit,
}
...
fn template_epoch_anchor_heights(parent_height: u64) -> Option<TemplateEpochAnchorHeights> {
    ...
    Some(TemplateEpochAnchorHeights {
        parent_terminal: elide_chain::consensus::tx_epoch_anchor_height_for_child(parent_height),
        child_transactions: elide_chain::consensus::tx_epoch_anchor_height_for_child(child_height),
    })
}
...
impl TemplateBuilder {
    ...
    pub async fn build_from_snapshot(
        ...
        self.build_from_snapshot_with_limit(
            ...
            miner_address,
            now_unix,
            elide_chain::consensus::paged_spend::BlockProofClass::B25.page_capacity(),
        )
        .await
    }
    ...
    pub async fn build_from_snapshot_with_limit(
        ...
        use elide_chain::consensus::median_time_past;
        ...
        //   See validate_timestamp in elide_chain::consensus::timestamps.
        ...
        // elide_chain::consensus::header::validate_header_inner. The two sites
        ...
        let consensus_max = elide_chain::consensus::params::BLOCK_MAX_USER_PAGES;
        ...
        let (authorization_bytes, groups): (Vec<_>, Vec<_>) = authorization_bytes
            ...
            .unzip();
        let mut proof_by_hash: HashMap<elide_poseidon2b::primitives::TxBodyHash, Option<Vec<u8>>> =
            authorization_bytes
                .into_iter()
        ...
        let txs: Vec<_> = groups
            ...
            .map(|page| elide_tx::Transaction::new(page.body))
            .collect();
        ...
        let template_cpu_result = install_history_step_phase_cpu(|| {
            match elide_chain::consensus::template::build_node_owned_block_template(
                &parent,
                &state,
        ...
        let selected_stream =
            elide_chain::consensus::validate_paged_spend_transaction_stream(&inner.txs)
...
mod tests {
    ...
    fn template_separates_parent_and_child_anchors_at_transaction_epoch_boundary() {
        ...
        let e = elide_chain::consensus::params::TX_EPOCH_BLOCKS;
