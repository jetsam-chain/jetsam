...
//! and atomically commit one [`elide_chain::AcceptedBlockBundle`].

use elide_chain::block::Block;
use elide_chain::consensus::pow::{block_id, validate_pow};
use elide_chain::storage::{MdbxChainContext, MdbxContextError};

use crate::template::BlockTemplate;

type HistoryStepRuntime = elide_recursive::acceptance::history_step::HistoryStepRuntime;
type PreparedGhost =
    elide_recursive::acceptance::history_step::PreparedHistoryStepGhostAuthorization;
type PreparedStateCommit = elide_chain::consensus::template::PreparedBlockStateCommit;
type LocallyProvedStateCommit = elide_chain::consensus::template::LocallyProvedBlockCommit;

enum PreparedWitness {
    B25(elide_block::PreparedHistoryStepWitness<25>),
    B255(elide_block::PreparedHistoryStepWitness<255>),
}
...
pub struct CommittedBlock {
    block: Block,
    bundle: elide_chain::AcceptedBlockBundle,
}

fn decode_template_authorizations(
    authorization_bytes: Vec<Option<Vec<u8>>>,
) -> Result<Vec<elide_gkr::zk_authorization::ZkAuthorizationProof>, String> {
    authorization_bytes
        .into_iter()
        .enumerate()
        .map(|(index, encoded)| {
            ...
            elide_gkr::WalletAuthorizationBundle::from_bytes(&encoded)
...
fn accumulator_from_header_boundary(
    ...
    elide_recursive::ChainAccumulator {
        ...
        tip_semantic_id: elide_chain::block_header::semantic_header_id(header),
...
impl PreparedBlockAttempt {
    ...
    fn prepare_inner(
        ...
        let parent_terminal = match (parent.height, parent_history_step_terminal_bytes) {
            ...
            (_, Some(bytes)) => Some(
                elide_recursive::acceptance::history_step::decode_history_step_terminal(
                    runtime, &bytes,
                )
        ...
        let witness = match proof_class {
            elide_chain::consensus::paged_spend::BlockProofClass::B25 => {
                elide_block::prepare_history_step_witness::<25>(
                    block,
                    context,
            ...
            elide_chain::consensus::paged_spend::BlockProofClass::B255 => {
                elide_block::prepare_history_step_witness::<255>(
                    block,
                    context,
                    authorization_proofs,
                    ghost,
        ...
        macro_rules! finish_and_prove {
            ($witness:expr) => {{
                ...
                let terminal = match cancellation {
                    Some(cancellation) => {
                        match elide_recursive::acceptance::history_step::prove_built_history_step_terminal_cancellable(
                            runtime,
                            &built,
                            cancellation,
                        ) {
                            Ok(terminal) => terminal,
                            Err(elide_recursive::acceptance::history_step::HistoryStepError::Cancelled) => {
                                return Ok(None);
                            }
                            Err(error) => return Err(error.to_string()),
                        }
                    }
                    None => elide_recursive::acceptance::history_step::prove_built_history_step_terminal(
                        runtime,
                        &built,
                    )
                    .map_err(|error| error.to_string())?,
                ...
                let terminal_bytes =
                    elide_recursive::acceptance::history_step::encode_history_step_terminal(
                        runtime, &terminal,
                    )
                    .map_err(|error| error.to_string())?;
                if cancelled() {
    ...
    pub fn pow_header(&self, nonce: u128) -> elide_chain::BlockHeader {
    ...
    pub fn user_page_count(&self) -> usize {
        usize::from(
            elide_chain::validate_block_page_stream(&self.block.transactions)
                .expect("prepared block has a canonical body")
                .page_count,
        )
    }

    pub fn proof_class(&self) -> elide_chain::consensus::paged_spend::BlockProofClass {
        elide_chain::validate_block_page_stream(&self.block.transactions)
            .expect("prepared block has a canonical body")
            .proof_class
    }
...
impl ProvedBlock {
    ...
    pub const fn bundle(&self) -> &elide_chain::AcceptedBlockBundle {
...
impl CommittedBlock {
    ...
    pub fn into_parts(self) -> (Block, elide_chain::AcceptedBlockBundle) {
...
mod tests {
    use super::*;
    use elide_chain::consensus::params::GENESIS_TARGET;
    use elide_poseidon2b::primitives::Address;

    fn header(height: u64, prev_block_hash: [u8; 32]) -> elide_chain::BlockHeader {
        elide_chain::BlockHeader {
            prev_block_hash,
    ...
    fn production_boundary_uses_parent_anchor_before_advancing_child_anchor() {
        ...
        assert!(matches!(
            ...
            Err(
                elide_recursive::ChainAccumulatorLocalBoundaryError::EpochAnchorHeight {
                    expected: 0,
                    actual,
                }
            ) if actual == epoch
