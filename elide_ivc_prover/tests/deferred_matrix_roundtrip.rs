...
use elide_ivc_core::field_r1cs::{FieldR1cs, SparseFieldMatrix};
...
use elide_ivc_core::verifier::{verify_field_deferred_matrix, verify_field_with_public_io};
...
fn free_instance(m: usize, k_log: usize, seed: u64) -> (FieldR1cs, Vec<F128>) {
    ...
    let r1cs = FieldR1cs {
        m,
        k_log,
        k_skip: elide_ivc_core::zerocheck::K_SKIP.min(k_log),
        useful_rows: k,
        a_0,
        b_0,
        const_pin: Some(0),
...
fn tiny_spec(k_log: usize) -> (PublicIoSpec, elide_ivc_core::public_io::WitnessSlice) {
    let io_slice = elide_ivc_core::public_io::WitnessSlice {
        log2_len: 1,
        index: 1,
    };
