...
use jetsam_ivc_core::field_circuit::{FsChannelOps, FsChannelUnionRecorder};
use jetsam_ivc_core::pcs::C1QuirkyDirectClaim;
...
const RECORDING_DUPLEX_LAYOUT_DIGEST_DOMAIN: &[u8] =
    b"JTM/REGION-SIDECAR/RECORDING-DUPLEX-LAYOUT/V3";
const RECORDING_DUPLEX_VK_DIGEST_DOMAIN: &[u8] = b"JTM/REGION-SIDECAR/RECORDING-DUPLEX-VK/V3";
...
impl RecordingDuplexRegionVk {
    ...
    fn substitution_terms(
        ...
        selector: F128,
        alpha: F128,
    ) -> Vec<jetsam_ivc_core::deep_chain::relations::RelationTerm> {
        if self.is_selected() {
