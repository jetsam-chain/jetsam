...
use jetsam_ivc_core::deep_chain::capsule_leaf::{C1_CAPSULE_MID_SLOTS, C1_CAPSULE_SOURCE_SLOTS};
use jetsam_ivc_core::deep_chain::schedule::{compile_duplex, DuplexLayout, LaneSource, TranscriptOp};
use jetsam_poseidon2b::native::poseidon2b_hash_byte_slices;

pub use jetsam_gkr::zk_authorization::{
...
const _: () =
    assert!(ZK_AUTH_MLECHECK_VARS == jetsam_gkr::zk_auth_capsule::ZK_AUTH_CAPSULE_BANK_VARS);
const _: () =
    assert!(ZK_AUTH_MLECHECK_ROUND_FIELDS == jetsam_gkr::zk_mlecheck::ZK_MLECHECK_ROUND_PROOF_COEFFS);
...
const ZK_AUTH_REGION_SIDECAR_PURPOSE_DOMAIN: &[u8] = b"JTM/REGION-SIDECAR/ZK-AUTH-SPLIT/V3";
...
mod tests {
    ...
    use jetsam_gkr::evaluate_permutation;
    use jetsam_gkr::zk_auth_capsule::{ZkAuthCapsuleStateTable, ZK_AUTH_CAPSULE_STATE_LEN};
    ...
    use jetsam_poseidon2b::native::domain::{capacity_iv, TAG_ADDRFIX, TAG_KSCH256};
    ...
    fn affine_blend_rejects_both_erasing_endpoints() {
        assert!(!affine_blend_gamma_is_admissible(jetsam_core::Block256::ZERO));
        assert!(!affine_blend_gamma_is_admissible(jetsam_core::Block256::ONE));
        assert!(affine_blend_gamma_is_admissible(jetsam_core::Block256::new(
            Block128::from(2u128),
            Block128::ONE,
        )));
    }
