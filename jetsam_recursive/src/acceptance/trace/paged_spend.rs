...
use jetsam_poseidon2b::native::domain::{capacity_iv, DomainTag};
...
fn native_remaining_counts(
    ...
    while cursor < surfaces.len() && native_bool(b, &page_live[cursor]) {
        ...
        assert!(count <= jetsam_tx::MAX_PAGED_SPEND_PAGES);
...
pub fn bind_paged_spend_stream(
    ...
    let cap = const_block(Block128::from((jetsam_tx::MAX_PAGED_SPEND_PAGES + 1) as u128));
    let cap_bits = range_check_bits(b, &cap, PAGE_COUNT_BITS);

    let [iv0, iv1] = capacity_iv(TAG_PAGED_SPEND);
    let version = const_block(Block128::from(jetsam_tx::PAGED_SPEND_VERSION as u128));
    let pad = [
    ...
    for index in 0..tier {
        ...
        if index != 0 {
            let crosses_input_gap = mul(
                ...
                &surfaces[index - 1].raw_inputs[jetsam_tx::TX_INPUTS - 1].add_const(F128::ONE),
            );
            ...
            let crosses_output_gap = mul(
                ...
                &surfaces[index - 1].raw_outputs[jetsam_tx::TX_OUTPUTS - 1].add_const(F128::ONE),
            );
        ...
        let no_full_input = surface.raw_inputs[jetsam_tx::TX_INPUTS - 1].add_const(F128::ONE);
        let no_full_output = surface.raw_outputs[jetsam_tx::TX_OUTPUTS - 1].add_const(F128::ONE);
        ...
        let spines = bodies
            ...
            .map(|body| {
                SpineInputsTrace::alloc(
                    &mut b,
                    &jetsam_gkr::spine_statement::spine_inputs_from_body(body),
                )
            })
            .collect::<Vec<_>>();
        let hashes = bodies
