...
use jetsam_chain::exact_state_hash::{slot_leaf_hash, state_node_hash, zero_slot_roots, StateHash};
...
use jetsam_chain::SlotValue;
use jetsam_core::Block128;
use jetsam_gkr::state_leaf_killshot::SlotLeafInputs;
use jetsam_ivc_core::deep_chain::leaf_hash::flat_sponge_leaf_hash;
use jetsam_ivc_core::field_circuit::Wire;
use jetsam_poseidon2b::native::domain::{capacity_iv, TAG_EXSTNOD};
...
fn empty_slot_leaf_input() -> SlotLeafInputs {
    let empty = jetsam_chain::SlotValue::EMPTY;
    SlotLeafInputs {
        packed_value: empty.value,
        owner_hi: empty.owner_hi,
        owner_lo: empty.owner_lo,
        expected_leaf: digest_to_fields(jetsam_chain::exact_state_hash::slot_leaf_hash(empty)),
    }
}
...
mod tests {
    ...
    use jetsam_chain::exact_state_hash::{slot_leaf_hash, state_node_hash};
    use jetsam_chain::sparse_merkle::{
        derive_structural_frontier_plan, evaluate_structural_frontier, expected_sibling_count,
        SparseMerkleError,
    };
    use jetsam_chain::SlotValue;
    use jetsam_ivc_core::field::F128 as Field;
    ...
    fn action_leaf_case(
        ...
        let mut actions = vec![
            ...
            ActionRowTrace {
                ...
                value: alloc_block(&mut b, jetsam_tx::pack_amount_creation_id(11, 7)),
    ...
    fn spend_and_mint_leaf_semantics_follow_the_sorted_action_tuple() {
        ...
        let (r1cs, witness) = action_leaf_case(
            jetsam_tx::pack_amount_creation_id(11, 7),
            Block128::from(0u128),
            jetsam_tx::pack_amount_creation_id(13, COINBASE_TEST_ID),
    ...
    fn action_leaf_recombination_rejects_component_mixing() {
        ...
        let cases = [
            action_leaf_case(
                jetsam_tx::pack_amount_creation_id(12, 7),
                Block128::from(0u128),
                jetsam_tx::pack_amount_creation_id(13, COINBASE_TEST_ID),
                Block128::from(13u128),
                mint_owner,
            ...
            action_leaf_case(
                jetsam_tx::pack_amount_creation_id(11, 7),
                Block128::from(0u128),
                jetsam_tx::pack_amount_creation_id(13, COINBASE_TEST_ID + 1),
                Block128::from(13u128),
                mint_owner,
            ),
            action_leaf_case(
