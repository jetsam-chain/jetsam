...
use jetsam_chain::consensus::fees::{PRESSURE_EXTREME_BPS, PRESSURE_HIGH_BPS, PRESSURE_LOW_BPS};
use jetsam_chain::consensus::params::{
    BASE_REWARD_MICRO, BLOCK_MAX_USER_PAGES, FEE_PER_INPUT, FEE_PER_OUTPUT, MIN_FEE_BASE,
    STATE_GROWTH_FEE_BASE,
};
use jetsam_core::{hardware::flat_to_tower_u128, Block128};
...
const MAX_CONSERVATIVE_MIN_FEE: u128 = MIN_FEE_BASE as u128
    + FEE_PER_INPUT as u128 * jetsam_tx::MAX_PAGED_SPEND_INPUTS as u128
    + FEE_PER_OUTPUT as u128 * jetsam_tx::MAX_PAGED_SPEND_OUTPUTS as u128
    + STATE_GROWTH_FEE_BASE as u128 * 8 * jetsam_tx::MAX_PAGED_SPEND_OUTPUTS as u128;
const MAX_BLOCK_COINBASE_CEILING: u128 =
    BLOCK_MAX_USER_PAGES as u128 * u64::MAX as u128 + BASE_REWARD_MICRO as u128;
...
fn bind_pressure(
    ...
    let native = jetsam_chain::consensus::fees::pressure_multiplier(native_active, native_depth);
...
fn bind_group_fee(
    ...
    let input_cap = range_check_bits(
        ...
        &const_block(Block128::from(
            (jetsam_tx::MAX_PAGED_SPEND_INPUTS + 1) as u128,
        )),
        11,
    );
    let output_cap = range_check_bits(
        b,
        &const_block(Block128::from(
            (jetsam_tx::MAX_PAGED_SPEND_OUTPUTS + 1) as u128,
        )),
        9,
    );
...
mod tests {
    ...
    fn required(body: &TxBody, active: u64, depth: u32) -> u64 {
        jetsam_chain::consensus::fees::required_fee_for_tx_body(body, active, depth)
    }

    fn claimable(body: &TxBody, active: u64, depth: u32) -> u64 {
        jetsam_chain::consensus::fees::claimable_fee_for_tx_body(body, active, depth)
    }
    ...
    fn build_case_with_liveness(
        ...
        let subsidy = alloc_block(
            &mut b,
            Block128::from(jetsam_chain::consensus::emission::block_reward(TEST_HEIGHT) as u128),
        );
    ...
    fn honest_fee_burn_claimable_and_underclaimed_coinbase() {
        ...
        let reward = jetsam_chain::consensus::emission::block_reward(TEST_HEIGHT);
    ...
    fn coinbase_reward_overclaim_is_rejected() {
        ...
        let maximum =
            jetsam_chain::consensus::emission::block_reward(TEST_HEIGHT) + claimable(&body, 0, depth);
    ...
    fn exact_pressure_boundaries_match_native_integer_basis_points() {
        for depth in MIN_EXACT_STATE_DEPTH..=MAX_EXACT_STATE_DEPTH {
            for (bps, before, at) in [
                ...
                assert_eq!(
                    jetsam_chain::consensus::fees::pressure_multiplier(threshold - 1, depth as u32),
                    before
                );
                assert_eq!(
                    jetsam_chain::consensus::fees::pressure_multiplier(threshold, depth as u32),
                    at
                );
            }
        }
    ...
    fn matrix_is_invariant_across_counts_pressure_and_depth() {
        ...
        let sparse_max =
            jetsam_chain::consensus::emission::block_reward(TEST_HEIGHT) + claimable(&sparse, 0, 24);
        let dense_max =
            jetsam_chain::consensus::emission::block_reward(TEST_HEIGHT) + claimable(&dense, dense_active, 32);
        let (a, aw, _) = build_case(&[sparse], 0, 24, 24, sparse_max);
        let (c, cw, _) = build_case(&[dense], dense_active, 32, 32, dense_max);
        let ghost = jetsam_gkr::ghost_tx::ghost_tx_body();
        let (g, gw, gt) = build_case_with_liveness(
            ...
            32,
            32,
            jetsam_chain::consensus::emission::block_reward(TEST_HEIGHT),
        );
