...
use jetsam_ivc_core::deep_chain::ff_merkle::FfMerkleFamilyRefs;
...
use jetsam_ivc_core::deep_chain::relations::{ColRef, FixedPattern, RelationColumns};
use jetsam_ivc_core::deep_chain::schedule::{DuplexFamilyRefs, MerkleFamilyRefs};
...
fn c1_sponge_leaf_terms(
    refs: &jetsam_ivc_core::deep_chain::leaf_hash::SpongeLeafRefs,
    alpha: F256,
) -> Vec<C1RelationTerm> {
    let weights = c1_mds_weights(alpha);
    let mut terms = Vec::with_capacity(2 * STATE_SIZE);
...
fn c1_walk_a_union_terms(
    leaf_refs: &[(jetsam_ivc_core::deep_chain::leaf_hash::SpongeLeafRefs, usize)],
    split_tails: &[SplitDuplexTailRefs],
    es_sponge: Option<&(jetsam_ivc_core::deep_chain::leaf_hash::SpongeLeafRefs, usize)>,
...
mod tests {
    ...
    fn assert_lifted(
        base: &[jetsam_ivc_core::deep_chain::relations::RelationTerm],
        wide: &[C1RelationTerm],
    ) {
