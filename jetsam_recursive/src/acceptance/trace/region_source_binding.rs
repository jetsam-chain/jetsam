...
use jetsam_poseidon2b::native::domain::{capacity_iv, DomainTag, TAG_CAPSNODE, TAG_EXSTNOD};
#[cfg(test)]
use jetsam_poseidon2b::native::domain::{TAG_FRICHANL, TAG_KSCH256, TAG_KSCHANNL};
...
use jetsam_ivc_core::deep_chain::capsule_leaf::CAPSULE_LEAF_STRIDE;
#[cfg(test)]
use jetsam_ivc_core::deep_chain::ff_merkle::{
...
use jetsam_ivc_core::deep_chain::leaf_hash::{
...
use jetsam_ivc_core::deep_chain::relations::{
...
use jetsam_ivc_core::deep_chain::schedule::{
...
#[cfg(test)]
use jetsam_ivc_core::deep_chain::schedule::{compile_duplex, merkle_fixed_patterns};
use jetsam_ivc_core::deep_chain::source_tree::{
    compress_iv_flat, mds_weights_pub, source_tree_substitution_terms, SourceTreeRefs,
};
use jetsam_ivc_core::deep_chain::spine::{
...
fn build_auth_pcs_meta_region_draft(
    ...
    let (ghost_s0, ghost_out) =
        jetsam_ivc_core::deep_chain::source_tree::run_perm([F128::ZERO; STATE_SIZE]);
...
fn preflight_selected_zk_meta_inputs(
    ...
    if txr.depth != 8
        || txr.paths.len() != 1 << jetsam_chain::tx_tree::TX_TREE_DEPTH
        || txr
            .paths
            .iter()
...
mod selected_zk_common_allocator_tests {
    use jetsam_ivc_core::deep_chain::schedule::{duplex_family_refs, duplex_fixed_patterns};
...
mod column_slice_values_only_tests {
    ...
    fn values_only_allocators_match_legacy_slices_wires_rows_and_values() {
        ...
        for (offset, expression) in column_wires.iter().enumerate() {
            assert_eq!(
                expression,
                &LinExpr::from_wire(jetsam_ivc_core::field_circuit::Wire(
...
mod split_walk_a_layout_tests {
    ...
    fn wallet_ff_root_pin_uses_composite_only_at_power_of_two_depth() {
        let iv = iv_flat_of_tag(jetsam_poseidon2b::native::domain::TAG_CAPSNODE);
        ...
        let allocate =
            |b: &mut FieldR1csBuilder,
             cols: &jetsam_ivc_core::deep_chain::ff_merkle::FfMerklePathColumns| {
...
fn flat_mds_entry(e: usize, j: usize) -> F128 {
    jetsam_ivc_core::deep_chain::flat_mds(true)[e][j]
}
...
pub(crate) fn build_recording_only_duplex_union(recordings: &[RecordingSpec<'_>]) -> DuplexUnion {
    ...
    let fill_carry = |committed: &mut [Vec<F128>; 6],
                      ...
                      to: usize| {
        for slot in from..to {
            let (g0, gout) = jetsam_ivc_core::deep_chain::source_tree::run_perm(*carry);
            for lane in 0..STATE_SIZE {
...
fn duplex_mds_weights(alpha: F128) -> [F128; STATE_SIZE] {
    ...
    std::array::from_fn(|j| {
        ...
        for e in 0..STATE_SIZE {
            acc += alphas[e] * flat(jetsam_poseidon2b::native::permutation::MDS_FULL[e][j]);
        }
        acc
    })
}
...
mod stage1_duplex_union_tests {
    ...
    use jetsam_poseidon2b::native::domain::{capacity_iv, TAG_FRICHANL};
    ...
    fn duplex_union_slot_end_to_end() {
        use jetsam_ivc_core::challenger::FsLaneChallenger;
        use jetsam_ivc_core::pcs::{self, PcsParams};
        use jetsam_ivc_core::public_io::{IoClaimSpec, PublicIoSpec};
        use jetsam_ivc_core::verifier::verify_field_with_public_io;
    ...
    fn duplex_union_raw_read_binding() {
        ...
        let chal = LinExpr::from_wire(jetsam_ivc_core::field_circuit::Wire(
...
mod combined_duplex_union_tests {
    ...
    use jetsam_poseidon2b::native::domain::DomainTag;
