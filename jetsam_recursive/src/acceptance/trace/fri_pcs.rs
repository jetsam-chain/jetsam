...
pub fn packed_queries_from_seeds_with_bound_bits_for_count(
    ...
    for (query, bits) in all_bits.iter().enumerate() {
        let reconstructed = bits.iter().enumerate().fold(0usize, |acc, (bit, wire)| {
            acc | (usize::from(wire.eval(b.values()) != jetsam_ivc_core::field::F128::ZERO) << bit)
        });
...
/// Trace twin of `jetsam_core::ntt::forward_ntt` over expressions. The
...
mod tests {
    ...
    use super::*;
    use jetsam_core::AdditiveNTT;
    use jetsam_fri_binius::capsule::{capsule_queries_from_seeds, capsule_query_seed_count};
    ...
    fn queries_from_squeezes_match_native_mask_rule() {
        ...
        for (q, e) in squeezed.iter().enumerate() {
            ...
            for (i, bit) in bits[q].iter().enumerate() {
                ...
                assert_eq!(
                    v != jetsam_ivc_core::field::F128::ZERO,
                    expect,
                    "query {q} bit {i}"
                );
            }
    ...
    fn packed_capsule_seed_bits_match_native_query_mapping() {
        let log_max_len = 9 + jetsam_fri_binius::capsule::CAPSULE_LOG_RATE;
        ...
        bad[first_bit_wire] += jetsam_ivc_core::field::F128::ONE;
        assert!(
            !r1cs.satisfies(&bad),
