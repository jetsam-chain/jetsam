...
//! (`jetsam_gkr::batch_eval`): `verify_linear_eval_prebound` and
...
pub fn squeeze_alphas_trace(
    ...
    use jetsam_gkr::batch_eval::{rlc_levels, RLC_LEVEL_BASE};
    if m == 0 {
        return Vec::new();
    }
...
impl BatchEvalProofTrace {
    pub fn alloc(
        b: &mut FieldR1csBuilder,
        native: &jetsam_gkr::batch_eval::BatchEvalProof,
        n: usize,
    ) -> Self {
        assert_eq!(
            native.rounds.len(),
