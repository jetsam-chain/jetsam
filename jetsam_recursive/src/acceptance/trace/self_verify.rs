...
//! The self-verification slot: the `jetsam_ivc_core` FieldR1cs verifier
...
//! the verified proof's field elements are `jetsam_ivc_core::F128` values
...
//! primitives are the flat-basis constructions of `jetsam_ivc_core::merkle`.
...
use jetsam_ivc_core::field::PHI_8_TABLE;
use jetsam_ivc_core::field_circuit::{f128_from_u128, FsChannelOps};
use jetsam_ivc_core::field_r1cs::FieldR1cs;
use jetsam_ivc_core::merkle::{self, Hash};
use jetsam_ivc_core::ntt::AdditiveNttF128;
use jetsam_ivc_core::pcs::{self, compute_fri_arities, default_fri_queries, PcsParams, LOG_PACKING};
use jetsam_ivc_core::public_io::POST_COMMIT_CLASS_BINDING_LABEL;
use jetsam_ivc_core::zerocheck::{self, K_SKIP};
...
/// The `IVCPCSL_` / `IVCPCSN_` tags of `jetsam_ivc_core::merkle`, duplicated
...
// ---------------------------------------------------------------------------
// Merkle primitives (trace twins of jetsam_ivc_core::merkle)
// ---------------------------------------------------------------------------
...
/// Trace twin of `jetsam_ivc_core::merkle::hash_pair` — ONE feed-forward
...
/// Trace twin of `jetsam_ivc_core::merkle::hash_leaf` for a lane-aligned leaf
...
/// Trace twin of `jetsam_ivc_core::proof::bind_statement_field`. The instance
...
pub fn bind_statement_field_trace(
    ...
    ch.observe_bytes_const(
        b,
        &jetsam_ivc_core::proof::pcs_params_statement_bytes(pcs_params),
    );
...
impl LincheckProofTrace {
    pub fn alloc(
        b: &mut FieldR1csBuilder,
        native: &jetsam_ivc_core::lincheck::LincheckProof,
        k_log: usize,
        k_skip: usize,
    ) -> Self {
...
impl C1LincheckProofTrace {
    pub fn alloc(
        b: &mut FieldR1csBuilder,
        native: &jetsam_ivc_core::lincheck::c1::C1LincheckProof,
        k_log: usize,
        k_skip: usize,
    ) -> Self {
        assert_eq!(native.rounds.len(), k_log - k_skip, "rounds off shape");
...
impl FieldR1csProofTrace {
    pub fn alloc(
        b: &mut FieldR1csBuilder,
        native: &jetsam_ivc_core::proof::FieldR1csProof,
        r1cs: &FieldR1cs,
        pcs_params: &PcsParams,
    ) -> Self {
        Self::alloc_shape(
            b,
            native,
            &jetsam_ivc_core::proof::FieldShape::of(r1cs),
            pcs_params,
        )
    }
    ...
    pub fn alloc_shape(
        b: &mut FieldR1csBuilder,
        native: &jetsam_ivc_core::proof::FieldR1csProof,
        shape: &jetsam_ivc_core::proof::FieldShape,
        pcs_params: &PcsParams,
    ) -> Self {
...
impl C1FieldR1csProofTrace {
    pub fn alloc(
        b: &mut FieldR1csBuilder,
        native: &jetsam_ivc_core::proof::C1FieldR1csProof,
        r1cs: &FieldR1cs,
        pcs_params: &PcsParams,
    ) -> Self {
...
pub fn bind_public_io_trace(
    ...
    let prefix = |slice: &jetsam_ivc_core::public_io::WitnessSlice| -> Vec<LinExpr> {
        slice
...
pub fn bind_public_io_c1_trace(
    ...
    let prefix = |slice: &jetsam_ivc_core::public_io::WitnessSlice| {
        slice
            .prefix_coords(total_vars)
            .into_iter()
...
fn verify_field_trace_inner(
    ...
    commitment_root: &FlatDigestExpr,
    proof: &FieldR1csProofTrace,
    public_io: Option<(&jetsam_ivc_core::public_io::PublicIoSpec, &[LinExpr])>,
) -> R1csClaimTrace {
    assert_eq!(
        pcs_params.m,
        r1cs.m + LOG_PACKING,
...
pub(crate) fn shape_only_field_r1cs_proof_c1(
    shape: &jetsam_ivc_core::proof::FieldShape,
    pcs_params: &PcsParams,
) -> (jetsam_ivc_core::proof::C1FieldR1csProof, Hash) {
    ...
    let proof = jetsam_ivc_core::proof::C1FieldR1csProof {
        ...
        lincheck: jetsam_ivc_core::lincheck::c1::C1LincheckProof {
...
pub(crate) fn patch_shape_only_query_positions_c1(
    proof: &mut jetsam_ivc_core::proof::C1FieldR1csProof,
    pcs_params: &PcsParams,
    ...
    for lane in query_lane_values {
        let raw = jetsam_ivc_core::field_circuit::f128_to_u128(*lane);
...
mod tests {
    use super::*;
    use jetsam_ivc_core::challenger::{fs_pack_bytes_lanes, Challenger, FsLaneChallenger};
    use jetsam_ivc_core::deep_chain::schedule::{compile_duplex, DuplexLayout};
    use jetsam_ivc_core::field_circuit::{f128_to_u128, FsChannelUnionRecorder};
    ...
    use jetsam_ivc_core::field_circuit::FsChannelTrace;
    ...
    /// The module's private tag copies match the ones `jetsam_ivc_core::merkle`
    ...
    fn duplicated_tags_match_native_merkle() {
        use jetsam_poseidon2b::native::{compress_flat_feed_forward_with_tag, Poseidon2bFlatSponge};
    ...
    fn lagrange_windows_match_native() {
        use jetsam_ivc_core::zerocheck::multilinear::{
    ...
    fn lincheck_fixture(
        ...
        jetsam_ivc_core::lincheck::LincheckProof,
    ) {
        use jetsam_ivc_core::field_r1cs::synthetic_satisfiable;
        use jetsam_ivc_core::lincheck::{self, QuirkyPoint};
    ...
    fn c1_lincheck_fixture(
        ...
        jetsam_ivc_core::lincheck::c1::C1LincheckProof,
    ) {
        use jetsam_ivc_core::field_r1cs::synthetic_satisfiable;
        use jetsam_ivc_core::lincheck::c1::{self, C1QuirkyPoint};
    ...
    fn pcs_fixture(
        ...
        use jetsam_ivc_core::lincheck::build_eq_table;
        use jetsam_ivc_core::zerocheck::multilinear::lagrange_weights_naive;
    ...
    fn field_proof_fixture(
        ...
        PcsParams,
        pcs::Commitment,
        jetsam_ivc_core::proof::FieldR1csProof,
        jetsam_ivc_core::proof::R1csClaim,
    ) {
        use jetsam_ivc_core::field_r1cs::synthetic_satisfiable;
    ...
    fn c1_field_proof_fixture(
        ...
        jetsam_ivc_core::proof::C1R1csClaim,
    ) {
        use jetsam_ivc_core::field_r1cs::synthetic_satisfiable;
        use jetsam_ivc_prover::field_prover::prove_field_c1;
    ...
    fn build_field_verify_trace(
        ...
        params: &PcsParams,
        commitment: &pcs::Commitment,
        proof: &jetsam_ivc_core::proof::FieldR1csProof,
    ) -> (
        FieldR1cs,
        Vec<F128>,
        std::ops::Range<usize>,
        ...
        jetsam_ivc_core::verifier::verify_field(r1cs, commitment, proof, &mut ch_native)
    ...
    fn verify_field_c1_replay_lockstep_e2e() {
        ...
        jetsam_ivc_core::verifier::verify_field_c1(&r1cs, &commitment, &proof, &mut native)
    ...
    fn verify_field_c1_deferred_capture_lockstep_e2e() {
        use jetsam_ivc_core::field_r1cs::synthetic_satisfiable;
        use jetsam_ivc_prover::field_prover::prove_field_c1_capturing_fresh;
    ...
    fn verify_field_c1_production_envelope_lockstep_e2e() {
        ...
        use jetsam_ivc_core::verifier::VerifyError;
        use jetsam_ivc_prover::field_prover::prove_field_c1_with_public_io_and_post_commit_context;
            ...
            prove_field_c1_with_public_io_and_post_commit_context(
                ...
                |context| {
                    ...
                    let value = context
                        .witness()
                        .iter()
                        .zip(jetsam_ivc_core::lincheck::build_eq_table(&point))
        ...
        let (_, native_fresh) =
            jetsam_ivc_core::verifier::verify_field_c1_deferred_matrix_with_post_commit_context(
                &shape,
    ...
    fn verify_field_replay_with_public_io_lockstep_and_mutations() {
        ...
        use jetsam_ivc_core::public_io::{IoClaimSpec, PublicIoSpec, WitnessSlice};
        use jetsam_ivc_prover::field_prover::prove_field_with_public_io;
        ...
        let eq = jetsam_ivc_core::lincheck::build_eq_table(&p);
        let mut v = F128::ZERO;
        ...
        jetsam_ivc_core::verifier::verify_field_with_public_io(
            &inner,
            &commitment,
            &proof,
            &spec,
    ...
    fn verify_field_deferred_lockstep_and_mutations() {
        use jetsam_ivc_core::field_r1cs::SparseFieldMatrix;
        use jetsam_ivc_core::matrix_claim::{
        ...
        let (_, fresh_native) = jetsam_ivc_core::verifier::verify_field_deferred_matrix(
            &shape,
            &digest,
            &commitment,
            &proof,
    ...
    fn verify_field_deferred_post_commit_auxiliary_lockstep() {
        ...
        use jetsam_ivc_core::proof::FieldShape;
        use jetsam_ivc_core::public_io::{PublicIoSpec, WitnessSlice};
        use jetsam_ivc_core::verifier::{verify_field_deferred_matrix_with_post_commit, VerifyError};
        use jetsam_ivc_prover::field_prover::prove_field_with_public_io_and_post_commit;
    ...
    fn verify_field_deferred_post_commit_context_lockstep() {
        ...
        use jetsam_ivc_core::proof::FieldShape;
        use jetsam_ivc_core::public_io::{PublicIoSpec, WitnessSlice};
        use jetsam_ivc_core::verifier::{
            verify_field_deferred_matrix_with_post_commit_context, VerifyError,
        };
        use jetsam_ivc_prover::field_prover::prove_field_with_public_io_and_post_commit_context;
