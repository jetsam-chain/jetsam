...
const _: () = assert!(
    HISTORY_STEP_WIRE_VERSION == jetsam_chain::history_step::HISTORY_STEP_TERMINAL_VERSION,
...
fn prepare_history_step_assembly<const TIER: usize>(
    ...
    // header, pinned through both derived ids.
    {
        ...
        let genesis_semantic =
            digest_lanes(&jetsam_chain::block_header::semantic_header_id(&genesis));
        for lane in 0..2 {
    ...
    Ok(match r1cs {
        ...
        None => {
            ...
            let built = BuiltHistoryStep {
                ...
                useful_rows: used,
                class_id: current_class,
                semantic_id: jetsam_chain::block_header::semantic_header_id(sealed_header),
...
mod tests {
    ...
    fn seal_fixture_parent(height: u64) -> jetsam_chain::BlockHeader {
    ...
    fn parent_seal_replay_rejects_wrong_prev_block_hash() {
        ...
        let link = jetsam_chain::hash_block_header(&parent);
        let tip = jetsam_chain::block_header::semantic_header_id(&parent);
        ...
        assert!(seal_glue_satisfies(
            &renonced,
            jetsam_chain::hash_block_header(&renonced),
            tip,
        ));
        ...
        assert!(!seal_glue_satisfies(
            &tampered,
            jetsam_chain::hash_block_header(&tampered),
            tip,
        ));
    }
    ...
    fn epoch_lane_updates_from_derived_parent_id_at_boundary() {
        ...
        for parent_height in [jetsam_chain::consensus::params::TX_EPOCH_BLOCKS - 1, jetsam_chain::consensus::params::TX_EPOCH_BLOCKS] {
            ...
            let parent_id = jetsam_chain::hash_block_header(&parent);
            ...
            let satisfies = |end: &ChainAccumulator| {
                ...
                let child_wires = DirectChildWires {
                    semantic_id: digest_lanes(&jetsam_chain::block_header::semantic_header_id(
                        &child,
                    ))
                    .map(|lane| {
                        LinExpr::from_wire(
                    ...
                    height: LinExpr::from_wire(builder.alloc_f128(
                        crate::acceptance::trace::flat_of(jetsam_core::Block128::from(
    ...
    fn base_case_selector_rejects_non_genesis_parent() {
        ...
        let base_pins_satisfy = |parent: &jetsam_chain::BlockHeader| {
            let genesis = jetsam_chain::consensus::genesis_header();
            let genesis_id = digest_lanes(&jetsam_chain::hash_block_header(&genesis));
            let genesis_semantic =
                digest_lanes(&jetsam_chain::block_header::semantic_header_id(&genesis));
        ...
        assert!(base_pins_satisfy(&jetsam_chain::consensus::genesis_header()));
        assert!(!base_pins_satisfy(&seal_fixture_parent(0)));
        let mut renonced_genesis = jetsam_chain::consensus::genesis_header();
