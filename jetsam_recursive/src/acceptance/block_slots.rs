...
use jetsam_poseidon2b::native::compression::compress;
use jetsam_poseidon2b::native::domain::{capacity_iv, TAG_BLOCKHDR, TAG_SEMHDR, TAG_TXROOT};
...
mod selected_zk_capability_tests {
    ...
    fn b255_capability_is_zero_row_exact_and_pad_is_constant_metadata() {
        let ghost_body = jetsam_gkr::ghost_tx::ghost_tx_body();
        let ghost_statement = jetsam_gkr::zk_authorization::ZkAuthCapsuleOwnerStatement {
            tx_body_hash: jetsam_gkr::ghost_tx::ghost_tx_body_hash(),
...
const _: () =
    assert!(header_fields::FIELDS + 1 == jetsam_chain::consensus::pow::POW_HEADER_FIELD_COUNT);
...
fn tx_root_region_capacity_handoff(
    b: &mut FieldR1csBuilder,
    tx_root_inputs: &[jetsam_gkr::merkle_circuit::MerklePathInputs],
...
fn tx_root_region_capacity_data_from_wires(
    ...
    assert_eq!(depth, jetsam_chain::tx_tree::TX_TREE_DEPTH);
...
fn exact_state_region_capacities(
    ...
    let segments = structural
        .touched_indices
        .iter()
        .map(|slot| slot >> jetsam_chain::consensus::params::LOG_SEGMENT_SIZE)
...
pub(in crate::acceptance) struct CanonicalSelectedZkAuthorizationSlot {
    ...
    liveness: LinExpr,
    native_statement: jetsam_gkr::zk_authorization::ZkAuthCapsuleOwnerStatement,
...
impl CanonicalSelectedZkAuthorizationSlot {
    ...
    pub(in crate::acceptance) fn native_statement(
        &self,
    ) -> jetsam_gkr::zk_authorization::ZkAuthCapsuleOwnerStatement {
...
fn mint_canonical_selected_zk_authorization_capability(
    ...
    let ghost_body = jetsam_gkr::ghost_tx::ghost_tx_body();
    let ghost_hash = jetsam_gkr::ghost_tx::ghost_tx_body_hash();
    ...
    for index in 0..body_auth_slots {
        ...
        let native_statement = jetsam_gkr::zk_authorization::ZkAuthCapsuleOwnerStatement {
...
impl DirectHeaderTrace {
    fn alloc(b: &mut FieldR1csBuilder, header: &BlockHeader) -> Self {
        let fields = jetsam_chain::consensus::pow::pow_header_fields(header)
            .into_iter()
            .enumerate()
            .filter(|(index, _)| *index != jetsam_chain::consensus::pow::POW_NONCE_FIELD_INDEX)
            .map(|(_, field)| alloc_block(b, field))
            .collect();
        let semantic_id = digest_lanes(&jetsam_chain::block_header::semantic_header_id(header));
        Self {
            fields,
...
fn sponge_lanes_trace(
    b: &mut FieldR1csBuilder,
    tag: jetsam_poseidon2b::native::domain::DomainTag,
    fields: &[LinExpr],
) -> [LinExpr; 2] {
...
impl ParentSealTrace {
    pub(in crate::acceptance) fn alloc(b: &mut FieldR1csBuilder, parent: &BlockHeader) -> Self {
        let fields: Vec<LinExpr> = jetsam_chain::consensus::pow::pow_header_fields(parent)
            .into_iter()
...
fn build_selected_zk_block_slots_core(
    ...
    if cap_txs > n_real_txs {
        let ghost_body = jetsam_gkr::ghost_tx::ghost_tx_body();
        let ghost_spine = jetsam_gkr::spine_statement::spine_inputs_from_body(&ghost_body);
        let ghost_hash = jetsam_gkr::ghost_tx::ghost_tx_body_hash();
    ...
    let user_action_slots = tier.saturating_mul(jetsam_tx::TX_ACTIONS);
    let mut action_candidates = Vec::with_capacity(user_action_slots + 3);
    let mut selected_input_bits = Vec::with_capacity(tier.saturating_mul(jetsam_tx::TX_INPUTS));
    let mut selected_output_bits = Vec::with_capacity(tier.saturating_mul(jetsam_tx::TX_OUTPUTS) + 3);
    ...
    let user_spine_inputs = (0..tier)
        ...
        .collect::<Vec<_>>();
    let ghost_hash_native = jetsam_gkr::ghost_tx::ghost_tx_body_hash();
    let ghost_hash = [
...
mod tx_epoch_anchor_tests {
    ...
    fn tail_parent_header(height: u64) -> BlockHeader {
        BlockHeader {
            ...
            timestamp: height * 10,
            height,
            miner_address: jetsam_poseidon2b::primitives::Address([0x66; 32]),
    ...
    fn tail_start(parent: &BlockHeader) -> ChainAccumulator {
        ChainAccumulator {
            ...
            tip_semantic_id: jetsam_chain::block_header::semantic_header_id(parent),
    ...
    fn tail_child(parent: &BlockHeader, nonce: u128) -> BlockHeader {
        BlockHeader {
            prev_block_hash: jetsam_chain::hash_block_header(parent),
            state_root: [0x44; 32],
            tx_root: [0x55; 32],
    ...
    fn start_accumulator() -> ChainAccumulator {
        ChainAccumulator {
            height: jetsam_chain::consensus::params::TX_EPOCH_BLOCKS - 1,
    ...
    const MARKER_LEAF: usize = jetsam_tx::body_hash::TX8X2_LEAF_FEE;

    fn selected_suffix_case(
        payout_due: bool,
    ) -> (jetsam_ivc_core::field_r1cs::FieldR1cs, Vec<F128>, [F128; 3]) {
    ...
    fn bodies(start: &ChainAccumulator) -> Vec<SpineInputs> {
        ...
        coinbase.leaves[jetsam_tx::body_hash::TX8X2_LEAF_EPOCH_ANCHOR] =
    ...
    fn scheduled_system_anchor_satisfies(payout_anchor: [u8; 32]) -> bool {
        ...
        payout.leaves[jetsam_tx::body_hash::TX8X2_LEAF_EPOCH_ANCHOR] = digest_lanes(&payout_anchor);
        let ghost =
            jetsam_gkr::spine_statement::spine_inputs_from_body(&jetsam_gkr::ghost_tx::ghost_tx_body());
    ...
    fn b255_body_liveness_excludes_the_256th_authorization_pad() {
        ...
        while natives.len() < 1 + jetsam_chain::consensus::params::BLOCK_MAX_USER_PAGES {
        ...
        let auth_capacity = super::tier_auth_slot_count(
            Some(jetsam_chain::consensus::params::BLOCK_MAX_USER_PAGES),
            1,
        );
