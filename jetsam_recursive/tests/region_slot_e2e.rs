...
use jetsam_ivc_core::deep_chain::family::{
    self, build_family, g0_terms, g1_terms, run_family, substitution_terms, FamilyProofs, W_LOG,
};
use jetsam_ivc_core::deep_chain::relations::{ColRef, RelationTerm};
use jetsam_ivc_core::field::F128;
use jetsam_ivc_core::field_circuit::{FieldR1csBuilder, FsChannelTrace, LinExpr};
...
use jetsam_recursive::acceptance::trace::deep_chain::{
...
fn region_slot_end_to_end() {
    ...
    for (terms, proof) in [
        ...
        let refs = jetsam_ivc_core::deep_chain::relations::distinct_refs(&terms);
        let rho = ch.sample_f128_vec(&mut b, W_LOG);
    ...
    let out_refs = jetsam_ivc_core::deep_chain::relations::distinct_refs(&out_terms);
    ...
    for e in 0..STATE_SIZE {
        alpha_pow = jetsam_recursive::acceptance::trace::mul(&mut b, &alpha_pow, &alpha);
        alpha_powers.push(alpha_pow.clone());
        target = target.add(&jetsam_recursive::acceptance::trace::mul(
            &mut b,
            &alpha_pow,
            &terminal.values[e],
        ));
    ...
    let mds = jetsam_ivc_core::deep_chain::flat_mds(true);
    ...
    let sub_terms_e: Vec<RelationTermTrace> = native_sub_terms
        ...
        .collect();
    let sub_refs = jetsam_ivc_core::deep_chain::relations::distinct_refs(&native_sub_terms);
    ...
    for (ci, claim) in pending.iter().enumerate() {
        ...
        for (k, p) in claim.point.iter().enumerate() {
            jetsam_recursive::acceptance::trace::pin_eq(&mut b, p, &io_wires[base + k]);
        }
        jetsam_recursive::acceptance::trace::pin_eq(&mut b, &claim.value, &io_wires[base + W_LOG]);
    }
    ...
    let (proof, commitment, _) = jetsam_ivc_prover::field_prover::prove_field_with_public_io(
    ...
    let mut ch_p = FsLaneChallenger::new(OUTER_DOMAIN);
    let (bad_proof, bad_commitment, _) = jetsam_ivc_prover::field_prover::prove_field_with_public_io(
