...
use elide_poseidon2b::native::poseidon2b_hash_bytes;
...
const _: () =
    assert!(SNAPSHOT_BOUNDARY_INTERVAL <= elide_chain::consensus::params::CONSENSUS_FINALITY_DEPTH);
...
pub const MAX_SNAPSHOT_MANIFEST_PAGES: usize =
    elide_chain::consensus::wire_limits::MAX_SNAPSHOT_MANIFEST_SEGMENTS
...
impl GetStateManifestHeader {
    ...
    pub fn has_canonical_page_shape(&self) -> bool {
        ...
        if segment_count > elide_chain::consensus::wire_limits::MAX_SNAPSHOT_MANIFEST_SEGMENTS {
            return false;
        }
...
pub struct GetStateSegmentResponse {
    ...
    pub status: DataResponseStatus,
    pub eff_log: u8,
    /// Column data encoded by `elide_chain::storage::serial::encode_segment`.
...
impl NetworkTopics {
    pub fn for_network_cfg(cfg: &elide_chain::consensus::NetworkConfig) -> Self {
        Self {
