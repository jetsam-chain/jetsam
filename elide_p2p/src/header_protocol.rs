...
impl HeaderInventoryRecord {
    ...
    pub fn from_retained_objects(
        ...
        if body_bytes.is_some_and(|bytes| {
            elide_chain::Block::from_bytes(bytes)
...
mod tests {
    use super::*;
    use elide_chain::{history_step::HISTORY_STEP_TERMINAL_VERSION, Block};

    fn bundle() -> AcceptedBlockBundle {
        let mut header = elide_chain::consensus::genesis_header();
        header.height = 1;
        header.prev_block_hash = block_id(&elide_chain::consensus::genesis_header());
        let block = Block {
            header,
