...
//!
//! Quick start:
//!   elide-cli status            — node health at a glance
//!   elide-cli balance           — wallet balance
//!   elide-cli send <addr> 10.5  — send 10.5 ELD
//!   elide-cli help              — full command list

#![allow(clippy::format_in_format_args, clippy::print_literal)]

use anyhow::{bail, Context};
use clap::{Parser, Subcommand};
use elide_chain::consensus::params::RETAINED_BLOCK_SERVING_DEPTH;
...
QUICK START:
  ...
  elide-cli balance             Wallet balance
  elide-cli send <addr> 10.5   Send 10.5 ELD to address
  elide-cli history             Transaction history
  elide-cli mempool             Pending transactions
  elide-cli help                All commands
...
enum Command {
    ...
    ///
    /// Examples:
    ///   elide-cli send f784...b61e 10.5
    ///   elide-cli send f784...b61e 10.5 --fee 0.01
    Send {
    ...
    /// Redirect output to a file: elide-cli receipt <hash> > receipt.hex
    Receipt {
...
async fn main() {
    ...
    // Without this, `elide-cli utxos | head -5` prints a Rust panic traceback.
...
async fn run(cli: Cli) -> anyhow::Result<()> {
    let client = reqwest::Client::builder()
        .user_agent(concat!("elide-cli/", env!("CARGO_PKG_VERSION")))
...
fn print_error(msg: &str) {
    ...
    {
        "Invalid address.\nUse a bech32m address (o1…).\nExample: elide-cli send o1q9gnyj0zwhqj9tm5sf… 10.5".to_string()
    } else {
        msg.to_string()
    };
...
async fn cmd_tx(ctx: &Ctx<'_>, txhash: &str) -> anyhow::Result<()> {
    ...
    if result.is_null() {
        ...
        println!("  Use 'elide-cli mempool-tx {txhash}' to check if it is pending.");
        return Ok(());
    }
    section("Transaction");
...
async fn cmd_mining(ctx: &Ctx<'_>) -> anyhow::Result<()> {
    ...
    kv2(
        "Block reward",
        &format!("{} ELD/block", elide_str(reward_micro)),
...
async fn cmd_mempool_tx(ctx: &Ctx<'_>, txhash: &str) -> anyhow::Result<()> {
    ...
    if result.is_null() {
        ...
        println!("  It may have been confirmed. Use 'elide-cli tx {txhash}' to check.");
        return Ok(());
    }
...
async fn cmd_address(
    ...
    if let Some(idx) = use_index {
        ...
        let bal = rpc(ctx, "walletGetBalance", &[])
            .await
            .ok()
            .and_then(|balance| balance["balance_eld"].as_f64())
            .unwrap_or(0.0);
...
async fn cmd_balance(ctx: &Ctx<'_>) -> anyhow::Result<()> {
    ...
    if pending_out > 0 {
        if is_tty() {
            ...
            println!("  Spendable:         {} ELD", elide_str(spendable));
        }
    }
...
async fn cmd_utxos(ctx: &Ctx<'_>) -> anyhow::Result<()> {
    ...
    if utxos.is_empty() {
        println!(
            "  {}",
            c!(DIM, "(no UTXOs — run 'elide-cli scan' to discover)")
        );
        return Ok(());
    }
    ...
    if is_tty() {
        ...
        println!("  {:<8}  {:>20}  {:>14}", "TOTAL", "", elide_str(total));
    }
...
async fn cmd_send(
    ...
    let amount_micro =
        parse_elide_amount(amount).with_context(|| format!("invalid amount {amount:?}"))?;
    ...
    let fee_micro = match fee {
        Some(f) => parse_elide_amount(f).with_context(|| format!("invalid fee {f:?}"))?,
        None => 0, // auto
    };
    ...
    match result {
        ...
        Err(e) => {
            ...
            let human = if msg.contains("Insufficient") || msg.contains("insufficient") {
                ...
                "No UTXOs available. Run 'elide-cli scan' to discover your coins.".into()
...
async fn cmd_history(
    ...
    for e in &filtered {
        ...
        let amount_str = format!("{:>14}{}", elide_str(micro), sign);
    ...
    if is_tty() {
        println!(
            ...
            "",
            GRN,
            format!("+ {} ELD received", elide_str(received)),
            RED,
            format!("  - {} ELD sent", elide_str(sent)),
            RST
        );
    } else {
        println!(
...
async fn cmd_receipt(ctx: &Ctx<'_>, txhash: &str) -> anyhow::Result<()> {
    ...
    if is_tty() {
        ...
        eprintln!();
        eprintln!(
            "  {} Redirect to a file: elide-cli receipt {} > receipt.hex",
            c!(DIM, "Tip:"),
            txhash
        );
        eprintln!(
            "  {} Verify:              elide-cli verify $(cat receipt.hex)",
            c!(DIM, "Tip:")
        );
    }
...
async fn cmd_block_template(ctx: &Ctx<'_>, miner_addr: &str) -> anyhow::Result<()> {
    ...
    println!();
    println!(
        "  {} Search the nonce, then run `elide-cli submit-block {template_id} <32-char-lowercase-nonce-hex>`.",
        c!(DIM, "PoW:")
    );
    if !pow_fields.is_empty() {
...
fn input_limit_exceeded_message(error: &anyhow::Error) -> Option<String> {
    ...
    if rpc.code != i64::from(elide_rpc::types::WALLET_INPUT_LIMIT_EXCEEDED_CODE) {
        return None;
    }
    let max_inputs = rpc
        .data
...
async fn rpc(ctx: &Ctx<'_>, method: &str, params: &[Value]) -> anyhow::Result<Value> {
    let method_full = format!("paraelide_{method}");
...
#[cfg(test)]
mod amount_tests {
    use super::{input_limit_exceeded_message, elide_str, parse_elide_amount, RpcCallError};

    #[test]
    fn decimal_amounts_are_exact_and_checked() {
        assert_eq!(parse_elide_amount("10.5").unwrap(), 10_500_000);
        assert_eq!(parse_elide_amount(".000001").unwrap(), 1);
        assert_eq!(parse_elide_amount("1.").unwrap(), 1_000_000);
        assert_eq!(elide_str(u64::MAX), "18446744073709.551615");
        assert!(parse_elide_amount("0.0000001").is_err());
        assert!(parse_elide_amount("NaN").is_err());
        assert!(parse_elide_amount("inf").is_err());
        assert!(parse_elide_amount("18446744073710").is_err());
    }

    #[test]
    fn input_limit_error_is_concise_and_uses_the_structured_limit() {
        let error = anyhow::Error::new(RpcCallError {
            code: i64::from(elide_rpc::types::WALLET_INPUT_LIMIT_EXCEEDED_CODE),
            message: elide_rpc::types::WALLET_INPUT_LIMIT_EXCEEDED_MESSAGE.to_string(),
