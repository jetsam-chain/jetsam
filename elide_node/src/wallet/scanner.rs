...
use elide_chain::consensus::receipt::generate_receipt;
...
fn derive_output_creation_ids(
    ...
    elide_chain::validate_block_page_stream(&block.transactions)
    ...
    for (tx_index, tx) in block.transactions.iter().enumerate() {
        for (output_index, _output) in tx.body.outputs.iter().enumerate() {
            ...
            ids[tx_index][output_index] = Some(if tx.body.is_primary_coinbase_shape() {
                elide_chain::consensus::params::coinbase_creation_id(block.header.height)
            } else {
                counter
            });
        }
...
pub fn update_wallet_artifacts_from_block(
    ...
    active_address: elide_poseidon2b::primitives::Address,
    active_index: u32,
    block: &Block,
) {
    let block_hash = elide_chain::block_id(&block.header);
    ...
    let block_tx_hashes: Vec<[u8; 32]> = elide_chain::try_compute_logical_txids(&block.transactions)
...
pub fn update_active_wallet_from_block(
    ...
    let stream = elide_chain::validate_block_page_stream(&block.transactions).map_err(|error| {
        ...
        if !already_pending {
            if sent_from_wallet > 0 {
                ...
                history.push(TxHistoryEntry {
                    ...
                    direction: TxDirection::Received,
                    is_coinbase: false,
                    amount_micro_eld: received_by_wallet,
                    peer_address: None,
                    timestamp,
...
mod tests {
        ...
        Transaction::new(TxBody {
            ...
            outputs: [
                TxOutput {
                    slot_index,
                    amount: 500,
                    owner: elide_chain::consensus::NETWORK_FUND_ADDRESS,
                },
                TxOutput {
                    slot_index: slot_index + 1,
                    amount: 500,
                    owner: elide_chain::consensus::LAB_FUND_ADDRESS,
                },
            ],
    ...
    fn block(transactions: Vec<Transaction>, alloc_counter: u64) -> Block {
        let tx_root = elide_chain::try_compute_tx_root(&transactions).unwrap_or([0; 32]);
        Block {
            header: BlockHeader {
    ...
    fn logical_txid(transaction: &Transaction) -> [u8; 32] {
        elide_tx::validate_paged_spend(&[elide_tx::TxPage {
    ...
    fn incremental_scan_assigns_coinbase_then_user_creation_ids() {
        ...
        assert_eq!(
            ...
            elide_chain::consensus::params::coinbase_creation_id(7),
        ...
        let expected_block_hash = elide_chain::block_id(&block.header);
        assert!(history
            .iter()
    ...
    fn incremental_scan_assigns_normal_ids_to_development_payout_outputs() {
        let owner = elide_chain::consensus::NETWORK_FUND_ADDRESS;
        let block = block(
            vec![
        ...
        assert_eq!(
            ids[0][0],
            Some(elide_chain::consensus::params::coinbase_creation_id(7))
        );
