...
pub struct WalletUtxo {
    ...
    /// This must be copied into every spending [`elide_tx::TxInput`].
    pub creation_id: u64,
    ...
    pub fn select_utxos(&self, target: u64, fee: u64) -> Option<(Vec<&WalletUtxo>, u64)> {
        ...
        available.sort_by_key(|u| {
            (
                ...
                u.slot_index >> elide_chain::consensus::params::LOG_SEGMENT_SIZE,
                u.slot_index,
            )
        });
        ...
        let mut total = 0u64;
        for utxo in available.into_iter().take(elide_tx::MAX_PAGED_SPEND_INPUTS) {
            selected.push(utxo);
...
mod tests {
    ...
    fn snapshot(owner: [u8; 32], amount: Option<u64>) -> VerifiedOwnerSnapshot {
        VerifiedOwnerSnapshot {
            ...
            alloc_counter: 9,
            utxos: amount
                .map(|amount| elide_chain::storage::VerifiedOwnerUtxo {
                    slot_index: 5,
                    amount,
                    creation_id: 8,
                })
