...
//!
use elide_gkr::OwnerAuthWitness;
use elide_poseidon2b::primitives::{derive_address, Address};
use elide_rpc::types::WALLET_CONSOLIDATION_INPUT_LIMIT;
use elide_tx::{
    output_bitmap_bit,
...
mod tests {
    ...
    fn multi_page_builder_keeps_secret_only_in_owner_witness() {
        ...
        let bundle = elide_gkr::WalletAuthorizationBundle::from_bytes(&intent.authorization_bytes)
            .expect("decode standard authorization bundle");
        elide_gkr::verify_paged_spend_authorization(&intent.pages, &bundle)
