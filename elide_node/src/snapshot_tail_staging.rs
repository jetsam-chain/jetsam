...
use elide_chain::consensus::wire_limits::{MAX_BLOCK_BYTES, MAX_HISTORY_STEP_TERMINAL_BYTES};
use elide_chain::{
...
impl SnapshotTailStaging {
    ...
    pub fn append(mut self, block_bytes: Vec<u8>) -> Result<Self, String> {
        ...
        let block_hash = elide_chain::hash_block_header(&block.header);
        ...
        self.tip_chainwork = elide_chain::add_work(
            &self.tip_chainwork,
            &elide_chain::block_work(&block.header.difficulty_target),
        );
    ...
    pub fn append_batch(mut self, block_bodies: Vec<Vec<u8>>) -> Result<Self, String> {
        if block_bodies.is_empty()
            || block_bodies.len()
                > elide_chain::consensus::params::RECENT_BLOCK_RETENTION_DEPTH as usize
        {
    ...
    pub fn finalize(
        ...
        if metadata.terminal_height() != tip_header.height
            || metadata.terminal_hash() != elide_chain::block_header::semantic_header_id(&tip_header)
        {
            return Err(SnapshotTailFinalizeError::Terminal(
...
impl SnapshotTailReader {
    pub fn next_block(&mut self) -> Result<Option<Vec<u8>>, String> {
        ...
        self.previous_hash = elide_chain::hash_block_header(&block.header);
        self.remaining -= 1;
...
mod tests {
    ...
    fn linked_block(parent: &Block, height: u64) -> Block {
        ...
        header.prev_block_hash = elide_chain::hash_block_header(&parent.header);
    ...
    fn terminal_for(block: &Block) -> Vec<u8> {
        let mut terminal = elide_chain::HistoryStepTerminalMetadata::new(
            block.header.height,
            elide_chain::block_header::semantic_header_id(&block.header),
            0,
        )
        .unwrap()
        .encode_prefix()
    ...
    fn empty_tail_cannot_claim_a_recursive_suffix() {
        ...
        let hash = elide_chain::hash_block_header(&boundary);
    ...
    fn finalized_tail_rejects_trailing_bytes() {
        ...
        let hash = elide_chain::hash_block_header(&genesis.header);
    ...
    fn tail_rejects_unlinked_bodies_and_a_terminal_for_another_tip() {
        ...
        let boundary_hash = elide_chain::hash_block_header(&genesis.header);
    ...
    fn linked_tail_round_trips_height_hash_and_chainwork() {
        ...
        let boundary_hash = elide_chain::hash_block_header(&genesis.header);
        let boundary_work = elide_chain::block_work(&genesis.header.difficulty_target);
        let first = linked_block(&genesis, 1);
        let second = linked_block(&first, 2);
        let expected_work = elide_chain::add_work(
            &elide_chain::add_work(
                &boundary_work,
                &elide_chain::block_work(&first.header.difficulty_target),
            ),
            &elide_chain::block_work(&second.header.difficulty_target),
        );
        ...
        assert_eq!(
            staged.tip_hash(),
            elide_chain::hash_block_header(&second.header)
        );
