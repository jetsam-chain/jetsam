...
use elide_chain::consensus::wire_limits::MAX_TX_INTENT_BYTES_GLOBAL;
use elide_chain::consensus::NetworkConfig;
use elide_chain::storage::snapshot_staging::{
    AuthenticatedSnapshotMetadata, FinalizedSnapshotStaging, SnapshotStagingError,
    SnapshotStagingSession,
};
use elide_chain::storage::{MdbxChainContext, MdbxStore, SnapshotSegmentDescriptor};
use elide_mempool::{AsyncMempool, ChainView, MempoolConfig};
use elide_miner::{BlockMiner, MinerConfig};
use elide_node::snapshot_header_staging::{
...
use elide_p2p::{NetworkEvent, P2PNetwork};
use elide_rpc::{start_rpc_server, ExternalMiningAttemptInvalidator, WalletOperationGate};

struct AppliedCompactSuffix {
    height: u64,
    block_hash: [u8; 32],
    confirmed_tx_hashes: Vec<elide_poseidon2b::primitives::TxBodyHash>,
    view: ChainView,
    applied_blocks: u64,
    payload_bytes: u64,
...
struct AppliedReorg {
    result: elide_chain::consensus::ReorgResult,
    confirmed_tx_hashes: Vec<elide_poseidon2b::primitives::TxBodyHash>,
    view: ChainView,
}
...
fn gap_requires_snapshot_sync(local_height: u64, peer_height: u64) -> bool {
    peer_height
        > local_height.saturating_add(elide_chain::consensus::params::RETAINED_BLOCK_SERVING_DEPTH)
}

fn validate_rebase_snapshot_selection(
    header_dag: &elide_node::networking::header_dag::HeaderDag,
    hint: SnapshotRebaseHint,
    manifest: &elide_p2p::protocol::GetStateManifestResponse,
) -> Result<(), String> {
    use elide_node::networking::ChainPoint;
...
fn try_dispatch_header_fetch(
    ...
    if p2p_cmd
        .try_send(elide_p2p::NetworkCommand::FetchHeaders {
            peer,
            start_height,
            count,
        })
...
fn finalized_header_search_floor(local_height: u64) -> u64 {
    local_height.saturating_sub(elide_chain::consensus::params::CONSENSUS_FINALITY_DEPTH)
}
...
fn competing_suffix_wins(
    ...
    matches!(
        elide_chain::choose_chain_by_work(
            competing_work,
            competing_tip_hash,
            local_work,
            local_tip_hash,
        ),
        elide_chain::consensus::fork_choice::ChainChoice::A
    )
}
...
fn unresolved_selected_tip_probe_range(
    dag: &elide_node::networking::header_dag::HeaderDag,
    committed_tip: elide_node::networking::ChainPoint,
    forward_headers: u16,
) -> Result<(u64, u16), elide_node::networking::header_dag::HeaderDagError> {
...
struct MiningPeerQuorum {
    ...
    readiness: elide_node::networking::mining_readiness::MiningReadiness,
    connected: std::collections::HashMap<libp2p::PeerId, elide_node::networking::FailureDomain>,
...
impl MiningPeerQuorum {
    fn new(
        ...
        let initial = elide_node::networking::ChainPoint::new(0, [0u8; 32]);
        let quorum = Self {
            isolated,
            origin,
            initial_sync_complete: isolated,
            unresolved_better_header: false,
            readiness: elide_node::networking::mining_readiness::MiningReadiness::new(
                isolated,
                MINING_PEER_QUORUM,
                initial,
            ),
    ...
    fn connect(
        &mut self,
        peer: libp2p::PeerId,
        failure_domain: elide_node::networking::FailureDomain,
    ) {
    ...
    fn set_canonical_tip_state(
        ...
        let tip = elide_node::networking::ChainPoint::new(height, hash);
    ...
    fn reconcile_canonical_tip(&mut self, height: u64, hash: [u8; 32], prev_hash: [u8; 32]) {
        ...
        let current = elide_node::networking::ChainPoint::new(height, hash);
        if previous == current {
            return;
        }
    ...
    fn confirm_tip_at(
        ...
        let point = elide_node::networking::ChainPoint::new(height, hash);
        let Some(failure_domain) = self.connected.get(&peer).copied() else {
            return;
        };
...
fn terminal_transport_can_retry_same_peer(kind: elide_p2p::RequestFailureKind) -> bool {
    matches!(
        kind,
        elide_p2p::RequestFailureKind::Timeout | elide_p2p::RequestFailureKind::Io
    )
}
...
fn embedded_history_step_cache_file(
    ...
    Some(
        history_step_cache_directory(data_dir, pack.runtime_metadata_digest()).join(
            elide_miner::history_step_runtime_image_file_name(class.class_id()),
        ),
    )
}
...
fn embedded_history_step_runtime(
    data_dir: &Path,
) -> Result<Option<Arc<elide_recursive::acceptance::history_step::HistoryStepRuntime>>, String> {
    ...
    let runtime = elide_recursive::acceptance::history_step::HistoryStepRuntime::new(
        bank,
...
fn prepare_history_step_ghost_authorization() -> Result<
    ...
    String,
> {
    elide_miner::install_history_step_phase_cpu(|| {
        let proof = elide_gkr::ghost_tx::prove_selected_ghost_authorization()
            .map_err(|error| format!("canonical ghost authorization proof failed: {error}"))?;
        elide_recursive::acceptance::history_step::prepare_history_step_ghost_authorization(proof)
            .map(Arc::new)
...
impl HistoryStepCacheClass {
    fn class_id(self) -> elide_recursive::CanonicalHistoryStepClassId {
        elide_recursive::CanonicalHistoryStepClassId::new(match self {
            Self::B25 => 0,
            Self::B255 => 1,
        })
...
const NODE_LOG_FILE: &str = "elide-node.log";

fn network_storage_epoch_bytes() -> Vec<u8> {
    let genesis = elide_chain::consensus::genesis_header();
    let genesis_id = elide_chain::block_header::block_id(&genesis);
...
async fn main() -> anyhow::Result<()> {
    ...
    if cli.check_hardware {
        let report = elide_core::cpu::ProductionHardwareReport::detect();
        print!("{report}");
        if report.ready() {
            return Ok(());
        }
    ...
    let production_hardware = elide_core::cpu::ensure_production_hardware()?;
    ...
    if prepare_network_storage_epoch(&data_dir)? {
        ...
        let gui_supervised = expand_tilde(&config_path) == data_dir.join("elide-gui.toml");
    ...
    let cpu_budget_mode = if cli.mode != NodeMode::Node {
        elide_miner::ProcessCpuBudgetMode::InternalMiner
    } else {
        elide_miner::ProcessCpuBudgetMode::ProofOnly
    };
    let cpu_plan = elide_miner::configure_process_cpu_budget_with_threads(
        cpu_budget_mode,
    ...
    let initial_canonical_tip = {
        ...
        elide_p2p::object_protocol::ChainPoint::new(ctx.tip_height(), ctx.tip_hash())
    };
    ...
    let authorization_verification_executor: elide_mempool::AuthorizationVerificationExecutor =
        Arc::new(|task: elide_mempool::AuthorizationVerificationTask| {
            elide_miner::install_inbound_verifier_cpu(task).map_err(|error| {
    ...
    // --- P2P Network ---
    let topics = elide_p2p::protocol::NetworkTopics::for_network_cfg(&net);
    let p2p_background_capacity = if cli.mode == NodeMode::Node {
        elide_p2p::BackgroundCapacity::Full
    } else {
        elide_p2p::BackgroundCapacity::MiningReserved
    };
    ...
    tokio::spawn(async move {
        loop {
            match mp_events.recv().await {
                Ok(elide_mempool::MempoolEvent::TxAdmitted { intent_bytes, .. }) => {
                    let _ = p2p_tx_relay
                        .send(elide_p2p::NetworkCommand::BroadcastTx { intent_bytes })
                        .await;
                }
                Ok(_) => {}
    ...
    let (rpc_handle, rpc_stop_rx) = start_rpc_server(
        ...
        cli.genesis,
        cfg.mining.enabled,
        elide_core::cpu::selected_backend().to_string(),
    ...
    let miner_handle = if cfg.mining.enabled {
        ...
        let miner_addr = if cfg.mining.miner_address.is_empty() {
            ...
            guard
                ...
                .unwrap_or(elide_poseidon2b::primitives::Address([0u8; 32]))
        } else {
        ...
        // Remote wallets use P2P block subscription independently.
        {
            ...
            miner.set_block_applied_hook(std::sync::Arc::new(move |block| {
                ...
                hook_canonical_tip_changes.send_replace(
                    elide_p2p::object_protocol::ChainPoint::new(
                        block.header.height,
                        elide_chain::block_id(&block.header),
                    ),
                );
            }));
        }
        ...
        tokio::spawn(async move {
            loop {
                match miner_rx.recv().await {
                    Ok(elide_miner::MinerEvent::BlockFound {
                        bundle,
                        height,
                        hash,
                        n_txs,
                    ...
                    Ok(elide_miner::MinerEvent::ProveFailed { height, error }) => {
    ...
    // --- Shutdown ---
    // Wait for either Ctrl-C or a `paraelide_stop` RPC call.
...
struct BoundaryProofTarget {
    ...
    epoch_anchor_header: elide_chain::BlockHeader,
}
...
impl BoundaryProofTarget {
    ...
    fn block_hash(self) -> [u8; 32] {
        elide_chain::block_header::block_id(&self.header)
    }
}
...
fn missing_snapshot_boundary_proof(
    ...
    let interval = elide_p2p::protocol::SNAPSHOT_BOUNDARY_INTERVAL;
    ...
    let block_hash = elide_chain::block_header::block_id(&header);
    ...
    let semantic_id = elide_chain::block_header::semantic_header_id(&header);
    ...
    let epoch_anchor_height =
        elide_chain::consensus::tx_epoch_anchor_height_for_child(header.height);
...
struct ReadySnapshotHeaderRange {
    ...
    count: u16,
    attempted_peers: std::collections::HashSet<libp2p::PeerId>,
    headers: Vec<elide_chain::BlockHeader>,
}
...
impl SnapshotHeaderPipeline {
    ...
    fn failure_plan(
        ...
        count: u16,
        token: u64,
        kind: elide_p2p::RequestFailureKind,
        peers: &std::collections::HashSet<libp2p::PeerId>,
        ...
        if matches!(kind, elide_p2p::RequestFailureKind::Timeout)
...
fn dispatch_snapshot_header_plans(
    ...
    for request in plans {
        let dispatched = !data_lane_full
            && p2p_cmd
                .try_send(elide_p2p::NetworkCommand::FetchSnapshotHeaders {
                    generation,
                    token: request.token,
                    peer: request.peer,
...
fn highest_snapshot_header_boundary(
    store: &elide_chain::storage::MdbxStore,
    target_height: u64,
...
fn prepare_snapshot_header_sync(
    ...
    Ok(PendingSnapshotHeaderSync {
        snapshot: elide_node::networking::SnapshotId {
            boundary: elide_node::networking::ChainPoint::new(
                manifest.tip_height,
                manifest.tip_hash,
            ),
...
struct VerifiedHistoryStepSnapshot {
    height: u64,
    block_hash: [u8; 32],
    boundary: elide_chain::VerifiedSnapshotBoundary,
...
fn verify_terminal_against_validated_snapshot_headers(
    ...
    runtime: &elide_recursive::acceptance::history_step::HistoryStepRuntime,
    authority: RetainedSnapshotHeaderAuthority,
...
fn validate_snapshot_staged_header_boundary(
    ...
    if bridge_span > elide_chain::consensus::params::RECENT_BLOCK_RETENTION_DEPTH {
    ...
    let expected_epoch_height =
        elide_chain::consensus::tx_epoch_anchor_height_for_child(manifest.tip_height);
...
fn verify_history_step_terminal(
    claim: &elide_chain::storage::HistoryStepTerminalClaim<'_>,
    runtime: Option<&elide_recursive::acceptance::history_step::HistoryStepRuntime>,
) -> Result<(), String> {
    let Some(runtime) = runtime else {
        return Err("embedded HistoryStep verifier unavailable".to_string());
    };
    elide_miner::install_inbound_verifier_cpu(|| {
        elide_recursive::acceptance::history_step::decode_verify_history_step_terminal(
            runtime,
...
fn history_step_context_error_is_terminal_peer_fault(
    error: &elide_chain::storage::MdbxContextError,
) -> bool {
    match error {
        elide_chain::storage::MdbxContextError::Consensus(
            elide_chain::consensus::ConsensusError::BadHistoryStepTerminal(message),
        ) => {
...
fn exact_suffix_context_error_is_body_peer_fault(
    ...
    match error {
        ...
        elide_chain::storage::MdbxContextError::Consensus(_) => true,
        elide_chain::storage::MdbxContextError::Corrupt(message) => matches!(
            *message,
        ...
        elide_chain::storage::MdbxContextError::Store(_)
        | elide_chain::storage::MdbxContextError::ResourceLimit { .. } => false,
    }
}

fn quarantine_exact_suffix_sources(
    header_dag: &mut elide_node::networking::header_dag::HeaderDag,
...
fn validate_history_step_tip_future_drift(
    ...
    elide_chain::consensus::validate_future_drift(boundary.tip_header.timestamp, local_time)
...
async fn apply_exact_suffix_offthread(
    ...
    wallet: &SharedWallet,
    fetched: elide_node::networking::suffix_sync::FetchedSuffix,
    ...
    use elide_node::networking::sync_plan::SyncPlanKind;
    ...
    let result = tokio::task::spawn_blocking(move || {
        ...
        for ((bytes, source), expected) in body_bytes
            ...
            .zip(plan.headers())
        {
            let block = elide_chain::Block::from_bytes(bytes).map_err(|error| {
        ...
        let tip_header = blocks
            ...
            .header;
        if elide_chain::block_id(&tip_header) != plan.target().hash {
            return Err(ExactSuffixApplyError::body(
                *body_sources
                    .last()
        ...
        let epoch_height =
            elide_chain::consensus::tx_epoch_anchor_height_for_child(tip_header.height);
        ...
        let terminal_started = Instant::now();
        let verified_terminal = elide_chain::storage::verify_history_step_terminal_candidate(
            tip_header,
            epoch_anchor_header,
            terminal_bytes,
        ...
        match plan.kind() {
            SyncPlanKind::LiveSuffix => {
                ...
                {
                    let txids = match elide_chain::try_compute_logical_txids(&block.transactions) {
                        Ok(txids) => txids,
                        Err(error) => {
                    ...
                    if let Err(error) = ctx.apply_verified_recursive_suffix_block(
                        ...
                        unix_now(),
                        |block, state| {
                            elide_chain::materialize_accepted_block_state(state, block)
            ...
            SyncPlanKind::Reorg => {
                ...
                let confirmed_tx_hashes = blocks
                    .iter()
                    .flat_map(|block| {
                        elide_chain::try_compute_logical_txids(&block.transactions)
...
enum HeaderInventoryPlan {
    Confirmed {
        tip: elide_node::networking::ChainPoint,
    },
    Behind,
    NeedOlder {
        start_height: u64,
    ...
    Candidate {
        headers: Vec<elide_node::networking::header_dag::ValidatedHeader>,
        records: Vec<elide_p2p::header_protocol::HeaderInventoryRecord>,
        old_tip: elide_node::networking::ChainPoint,
        target: elide_node::networking::ChainPoint,
    },
    FinalizedDivergence,
}
...
fn canonical_header_dag(
...
) -> Result<elide_node::networking::header_dag::HeaderDag, String> {
    use elide_node::networking::{header_dag::ValidatedHeader, ChainPoint};
    ...
    let mut dag = elide_node::networking::header_dag::HeaderDag::new(
...
fn reconcile_canonical_header_dag(
    context: &MdbxChainContext,
    dag: &mut elide_node::networking::header_dag::HeaderDag,
) -> Result<(), String> {
    use elide_node::networking::{header_dag::ValidatedHeader, ChainPoint};
    ...
    for height in dag.finalized().height.saturating_add(1)..=context.tip_height() {
        ...
        let hash = elide_chain::block_header::block_id(&header);
...
fn record_validated_headers(
    dag: &mut elide_node::networking::header_dag::HeaderDag,
    headers: &[elide_node::networking::header_dag::ValidatedHeader],
) -> Result<(), elide_node::networking::header_dag::HeaderDagError> {
...
fn advertise_inventory_for_known_headers(
    dag: &mut elide_node::networking::header_dag::HeaderDag,
    peer: libp2p::PeerId,
    records: &[elide_p2p::header_protocol::HeaderInventoryRecord],
) -> Result<usize, elide_node::networking::header_dag::HeaderDagError> {
    let known_inventory = records
        .iter()
        .filter(|record| record.body.is_some() || record.terminal.is_some())
        .filter(|record| {
            let hash = elide_chain::block_id(&record.header);
            dag.get(&hash)
...
fn header_inventory_validation_anchor(
    canonical: Option<elide_node::networking::ChainPoint>,
    validated_dag: Option<elide_node::networking::ChainPoint>,
) -> Option<elide_node::networking::ChainPoint> {
...
fn header_dag_has_unresolved_better_tip(
    dag: &elide_node::networking::header_dag::HeaderDag,
    canonical_tip: elide_node::networking::ChainPoint,
...
async fn plan_header_inventory(
    ...
    use elide_chain::consensus::params::CONSENSUS_FINALITY_DEPTH;
    use elide_node::networking::{header_dag::ValidatedHeader, ChainPoint};
    ...
    for header in competing_headers {
        cumulative_work = elide_chain::add_work(
            &cumulative_work,
            &elide_chain::block_work(&header.difficulty_target),
        );
...
fn source_independent_suffix_offer(
    ...
    preferred_peer: libp2p::PeerId,
    old_tip: elide_node::networking::ChainPoint,
    base: elide_node::networking::ChainPoint,
    headers: Vec<elide_node::networking::header_dag::ValidatedHeader>,
) -> Result<
    (
        libp2p::PeerId,
        elide_node::networking::suffix_sync::SuffixOffer,
        Vec<(
            libp2p::PeerId,
            Vec<elide_p2p::header_protocol::HeaderInventoryRecord>,
        )>,
    ),
    elide_node::networking::suffix_sync::SuffixSyncError,
> {
    use elide_node::networking::suffix_sync::{SuffixOffer, SuffixSyncError};
    ...
    let mut bootstrap = headers
        .iter()
        .map(|header| elide_p2p::header_protocol::HeaderInventoryRecord::header_only(header.header))
        .collect::<Vec<_>>();
    bootstrap
        .last_mut()
...
fn dispatch_exact_suffix_requests(
    sync: &mut elide_node::networking::suffix_sync::SuffixSync,
    p2p_cmd: &elide_p2p::NetworkCommandSender,
) {
    ...
    for request in sync.schedule(now_ms) {
        if p2p_cmd
            .try_send(elide_p2p::NetworkCommand::FetchObjects {
                token: request.token,
                peer: request.peer,
...
fn merge_active_suffix_inventory(
    active: &mut Option<elide_node::networking::suffix_sync::SuffixSync>,
    peer: libp2p::PeerId,
    failure_domain: elide_node::networking::FailureDomain,
    records: &[elide_p2p::header_protocol::HeaderInventoryRecord],
) -> Result<usize, elide_node::networking::suffix_sync::SuffixSyncError> {
...
fn admit_exact_suffix_offer(
    ...
    peer: libp2p::PeerId,
    failure_domain: elide_node::networking::FailureDomain,
    offer: elide_node::networking::suffix_sync::SuffixOffer,
) -> Result<SuffixAdmission, String> {
    use elide_chain::consensus::fork_choice::ChainChoice;
    use elide_node::networking::suffix_sync::SuffixSync;
...
mod tests {
    ...
    fn test_snapshot_id() -> elide_node::networking::SnapshotId {
        elide_node::networking::SnapshotId {
            boundary: elide_node::networking::ChainPoint::new(100, [1; 32]),
            state_root: [2; 32],
            manifest_digest: [3; 32],
            format_version: elide_p2p::protocol::SNAPSHOT_MANIFEST_FORMAT_VERSION,
        }
    }
    ...
    fn test_exact_suffix_offer(
        ...
        use elide_chain::block_header::{block_id, semantic_header_id};
        use elide_node::networking::{header_dag::ValidatedHeader, ChainPoint};
        use elide_p2p::{
        ...
        elide_node::networking::suffix_sync::SuffixOffer::live(base, headers, &records).unwrap()
    }

    #[test]
    fn dag_selected_suffix_merges_objects_from_independent_sources() {
        use elide_chain::block_header::{block_id, semantic_header_id};
        use elide_node::networking::{
        ...
        let first_work = elide_chain::add_work(
            &base_work,
            &elide_chain::block_work(&first_header.difficulty_target),
        );
        ...
        let second_work = elide_chain::add_work(
            &first_work,
            &elide_chain::block_work(&second_header.difficulty_target),
        );
        ...
        assert_eq!(
            source_independent_suffix_offer(&dag, tip_peer, base, base, headers.clone())
                .unwrap_err(),
            elide_node::networking::suffix_sync::SuffixSyncError::MissingBodySource
        );
        assert_eq!(
    ...
    fn mainnet_reset_removes_every_previous_entry_and_runs_once() {
        ...
        std::fs::write(directory.path().join("elide-node.log"), b"old log").unwrap();
        ...
        assert_eq!(
            ...
            vec![
                ...
                std::ffi::OsString::from("elide-node.log")
            ]
        );
        assert_eq!(
            std::fs::read(directory.path().join("elide-node.log")).unwrap(),
            b"old log"
        );
    ...
    fn default_install_reset_discards_legacy_preferences_but_preserves_new_gui_settings() {
        ...
        std::fs::write(root.join("elide.toml"), b"legacy Core settings").unwrap();
        reset_install_preferences_at_root(&root, &data, &data.join("elide-gui.toml"), true)
            .unwrap();
        assert_eq!(
    ...
    fn moving_tip_does_not_mutate_an_inflight_plan() {
        ...
        assert_eq!(
            admit_exact_suffix_offer(
                &mut active,
                first_peer,
                elide_node::networking::FailureDomain(1),
                pinned,
            )
            .unwrap(),
        ...
        assert_eq!(
            admit_exact_suffix_offer(
                &mut active,
                extension_peer,
                elide_node::networking::FailureDomain(2),
                extension,
            )
            .unwrap(),
    ...
    fn behind_header_range_still_supplies_the_pinned_suffix_objects() {
        use elide_node::networking::FailureDomain;
        use elide_p2p::{header_protocol::HeaderInventoryRecord, object_protocol::ObjectId};
        ...
        let records = vec![
            HeaderInventoryRecord::header_only(elide_chain::consensus::genesis_header()),
            HeaderInventoryRecord {
        ...
        let mut active = Some(
            elide_node::networking::suffix_sync::SuffixSync::from_offer(
                original_peer,
                FailureDomain(1),
                offer,
            )
    ...
    fn only_transient_terminal_transport_failures_retry_the_same_peer() {
        use elide_p2p::RequestFailureKind;
    ...
    fn mining_gate_accepts_one_confirmed_ordinary_peer() {
        ...
        quorum.connect(first, elide_node::networking::FailureDomain(1));
    ...
    fn mining_quorum_reacquisition_prioritizes_unconfirmed_public_peers() {
        ...
        for (index, peer) in peers.iter().enumerate() {
            quorum.connect(*peer, elide_node::networking::FailureDomain(index as u64));
        }
        let height = 31;
        let hash = [0x31; 32];
        quorum.set_canonical_tip(height, hash, false);
    ...
    fn verified_exact_suffix_source_reauthorizes_the_committed_tip_immediately() {
        ...
        quorum.connect(source, elide_node::networking::FailureDomain(1));
        quorum.connect(second_announcer, elide_node::networking::FailureDomain(2));
    ...
    fn header_only_candidate_does_not_pause_until_exact_plan_exists() {
        ...
        quorum.connect(peer, elide_node::networking::FailureDomain(1));
        quorum.set_canonical_tip(height, hash, false);
        quorum.set_sync_state(true, false);
    ...
    fn unresolved_reorg_probe_starts_at_the_common_ancestor() {
        use elide_chain::block_header::block_id;
        use elide_node::networking::{
            header_dag::{HeaderDag, ValidatedHeader},
            ChainPoint,
        };
        ...
        let local_work = elide_chain::add_work(
            &base_work,
            &elide_chain::block_work(&local_header.difficulty_target),
        );
        ...
        let competing_tip_work = elide_chain::add_work(
            &local_work,
            &elide_chain::block_work(&competing_tip_header.difficulty_target),
        );
        let competing_tip =
    ...
    fn snapshot_segment_errors_separate_remote_bytes_from_local_state() {
        use elide_chain::storage::snapshot_staging::SnapshotStagingError;
        ...
        assert!(matches!(
            classify_snapshot_session_prepare_error(
                elide_chain::storage::snapshot_staging::SnapshotStagingError::EffectiveLogMismatch {
                    expected: 16,
                    actual: 15,
                }
            ),
        ...
        assert!(matches!(
            classify_snapshot_session_prepare_error(
                elide_chain::storage::snapshot_staging::SnapshotStagingError::Io {
                    operation: "test",
                    source: std::io::Error::other("disk failed"),
                }
            ),
        ...
        assert!(matches!(
            classify_snapshot_finalization_error(
                elide_chain::storage::snapshot_staging::SnapshotStagingError::StateRootMismatch {
                    expected: [1; 32],
                    actual: [2; 32],
                }
            ),
        ...
        assert!(matches!(
            classify_snapshot_finalization_error(
                elide_chain::storage::snapshot_staging::SnapshotStagingError::StagedFileLength {
                    segment_id: 7,
                    expected: 10,
                    actual: 9,
                }
    ...
    fn snapshot_header_pipeline_uses_one_ordered_same_peer_window() {
        ...
        let headers = |start: u64, count: u16| {
            (start..start + u64::from(count))
                .map(|height| {
                    let mut header = elide_chain::consensus::genesis::genesis_header();
    ...
    fn snapshot_header_transport_failure_retries_only_its_exact_range() {
        ...
        let retry = pipeline
            .failure_plan(
                ...
                SNAPSHOT_HEADER_BATCH as u16,
                failed.token,
                elide_p2p::RequestFailureKind::Io,
                &peers,
            )
    ...
    fn snapshot_header_timeout_reduces_only_the_slow_range() {
        ...
        let retry = pipeline
            .failure_plan(
                ...
                initial.count,
                initial.token,
                elide_p2p::RequestFailureKind::Timeout,
                &peers,
            )
            .unwrap();
    ...
    fn snapshot_header_failures_try_every_connected_peer_before_reuse() {
        ...
        let retry_one = pipeline
            .failure_plan(
                ...
                initial.count,
                initial.token,
                elide_p2p::RequestFailureKind::InvalidResponse,
                &peers,
            )
            .unwrap();
    ...
    fn test_coinbase_child(
        parent: &elide_chain::BlockHeader,
        state: &elide_chain::ChainState,
    ) -> elide_chain::block::Block {
        let timestamp = parent.timestamp + elide_chain::consensus::params::BLOCK_TIME;
        ...
        let difficulty_target = elide_chain::consensus::difficulty::next_target(
            0,
            parent.timestamp,
        ...
        let template = elide_chain::consensus::build_block_template(
            ...
            vec![],
            elide_poseidon2b::primitives::Address([0x22; 32]),
            timestamp,
            difficulty_target,
        )
    ...
    fn snapshot_history_boundary_checks_staged_header_chainwork() {
        let state = elide_chain::ChainState::with_log_slots(
            elide_chain::consensus::params::LOG_SLOTS_GENESIS
                .try_into()
                .expect("genesis log_slots fits usize"),
        );
        let h0 = elide_chain::consensus::genesis_header();
        let h0_hash = elide_chain::hash_block_header(&h0);
        let high_start_work = elide_chain::consensus::block_work(&h0.difficulty_target);

        let block = test_coinbase_child(&h0, &state);
        let h1 = block.header;
        let h1_hash = elide_chain::hash_block_header(&h1);
        let h1_work = elide_chain::consensus::add_work(
            &high_start_work,
            &elide_chain::consensus::block_work(&h1.difficulty_target),
        );
        let mut manifest = elide_p2p::protocol::GetStateManifestResponse {
            tip_height: 1,
            tip_hash: h1_hash,
    ...
    fn snapshot_epoch_anchor_obeys_start_of_block_boundaries() {
        ...
        for (tip_height, expected_epoch_height) in [
            ...
            let tip_hash = elide_chain::hash_block_header(&tip_header);
            ...
            assert!(manifest.seal_manifest_digest());
            let mut epoch_anchor_header = elide_chain::consensus::genesis_header();
        ...
        let tip_height = elide_chain::consensus::params::TX_EPOCH_BLOCKS;
        let mut tip_header = elide_chain::consensus::genesis_header();
        tip_header.height = tip_height;
        let tip_hash = elide_chain::hash_block_header(&tip_header);
        ...
        assert!(manifest.seal_manifest_digest());
        let mut wrong_anchor = elide_chain::consensus::genesis_header();
    ...
    fn snapshot_history_step_tip_obeys_local_future_drift_admission() {
        ...
        let mut tip = elide_chain::consensus::genesis::genesis_header();
        tip.timestamp = local_time + elide_chain::consensus::params::MAX_FUTURE_DRIFT;
        let mut boundary = SnapshotHeaderBoundary {
            tip_header: tip,
            tip_hash: elide_chain::hash_block_header(&tip),
...
async fn handle_p2p_events(
    mut rx: elide_p2p::NetworkEventReceiver,
    chain: Arc<RwLock<MdbxChainContext>>,
    mempool: AsyncMempool,
    wallet: SharedWallet,
    p2p_cmd: elide_p2p::NetworkCommandSender,
    ...
    mut canonical_tip_changes: tokio::sync::watch::Receiver<elide_p2p::object_protocol::ChainPoint>,
) -> anyhow::Result<()> {
    use elide_chain::consensus::params::CONSENSUS_FINALITY_DEPTH;
    ...
    struct PendingManifest {
        ...
        rebase_base: Option<elide_node::networking::ChainPoint>,
        manifest: elide_p2p::protocol::VerifiedStateManifest,
        offer: elide_node::networking::snapshot_sync::SnapshotOffer,
    ...
    struct ExactSuffixApplyCompletion {
        plan_id: elide_node::networking::PlanId,
        target: elide_node::networking::ChainPoint,
        tip_announcement: elide_p2p::header_protocol::HeaderAnnouncement,
    ...
    let mut active_snapshot_sync: Option<elide_node::networking::snapshot_sync::SnapshotSync> = None;
    ...
    let mut active_suffix_sync: Option<elide_node::networking::suffix_sync::SuffixSync> = None;
    ...
    let mut exact_suffix_apply_inflight: Option<elide_node::networking::PlanId> = None;
    let mut peer_failure_domains: std::collections::HashMap<
        libp2p::PeerId,
        elide_node::networking::FailureDomain,
    > = std::collections::HashMap::new();
    ...
    let (mut header_dag, mut header_dag_canonical_tip) = {
        let ctx = chain.read().await;
        let canonical_tip =
            elide_node::networking::ChainPoint::new(ctx.tip_height(), ctx.tip_hash());
    ...
    let mut queued_segment_response: Option<(
        libp2p::PeerId,
        elide_p2p::protocol::GetStateSegmentResponse,
    )> = None;
    ...
    macro_rules! dispatch_snapshot_generation_advance {
        () => {{
            if p2p_snapshot_generation_announced != snapshot_sync_generation
                && p2p_cmd
                    .try_send(elide_p2p::NetworkCommand::AdvanceSnapshotGeneration {
    ...
    macro_rules! try_request_manifest {
        ($peer:expr, $requester_height:expr, $digest:expr) => {{
            ...
            if manifest_requested_peers.contains(&peer) {
                ...
                .try_send(elide_p2p::NetworkCommand::RequestStateManifest {
                    generation: snapshot_sync_generation,
                    peer,
    ...
    macro_rules! dispatch_pending_boundary_terminal_requests {
        () => {{
            if let Some(pending) = snapshot_boundary_terminal_inflight.as_mut() {
                ...
                for request in requests {
                    if p2p_cmd
                        .try_send(elide_p2p::NetworkCommand::RequestHistoryStepTerminal {
                            token: request.token,
                            peer: request.peer,
    ...
    macro_rules! retire_snapshot_plan {
        () => {{
            ...
            if let Some(request) = snapshot_boundary_terminal_inflight.take() {
                let _ = p2p_cmd
                    .send(elide_p2p::NetworkCommand::CancelHistoryStepTerminalRace {
    ...
    macro_rules! begin_snapshot_header_staging {
        ($from:expr, $manifest:expr) => {{
            ...
            let snapshot_offer = match elide_node::networking::snapshot_sync::SnapshotOffer::from_verified_manifest(
                manifest.clone(),
            ) {
                Ok(offer) => offer,
                Err(error) => {
            ...
            let rebase_base = snapshot_rebase_hint.and_then(|hint| {
                (hint.ancestor_height < terminal_height).then_some(
                    elide_node::networking::ChainPoint::new(
    ...
    macro_rules! mark_bootstrap_complete_if_caught_up {
        ($local_height:expr) => {{
            ...
            {
                if p2p_cmd
                    .send(elide_p2p::NetworkCommand::BootstrapComplete)
                    .await
                    .is_ok()
                {
    ...
    loop {
        ...
        match rx_item {
            Ok(NetworkEvent::HeaderAnnouncement {
                ...
                let exact_inventory = (source_has_objects && manifest_peers.contains(&from))
                    .then(|| {
                        elide_p2p::header_protocol::HeaderInventoryRecord::from_announcement(
                            announcement,
                        )
                    });
                ...
                let announced_hash = elide_chain::block_id(&announced_header);
                let (our_height, our_hash, finalized_height, canonical_hash_at_height) = {
                    ...
                    (
                        ...
                        ctx.header(height).map(elide_chain::block_id),
                    )
                };
                ...
                if gap_requires_snapshot_sync(our_height, height) {
                    ...
                    let precheck = {
                        ...
                        match ctx.finalized_active_counts() {
                            Ok(finalized_active_counts) => {
                                Some((
                                    elide_chain::consensus::validate_header(
                                        &announced_header,
                                        &parent,
                                        &prev_timestamps,
                    ...
                    if let Err(e) = precheck {
                        if e == elide_chain::consensus::ConsensusError::BadParentHash {
                    ...
                    let cumulative_work = elide_chain::add_work(
                        &base_work,
                        &elide_chain::block_work(&announced_header.difficulty_target),
                    );
                    let validated = elide_node::networking::header_dag::ValidatedHeader::new_after_consensus_checks(
                        announced_header,
                        cumulative_work,
                    );
                    ...
                    {
                        match elide_node::networking::suffix_sync::SuffixOffer::live(
                            base,
                            vec![validated],
                            &[record],
                        ) {
                            Ok(offer) => {
                                let domain = peer_failure_domains.get(&from).copied().unwrap_or(
                                    elide_node::networking::FailureDomain(u64::MAX),
                                );
            ...
            Ok(NetworkEvent::ObjectsResponse {
                ...
                match sync.accept_response(
                    ...
                    Err(
                        elide_node::networking::suffix_sync::SuffixSyncError::UnknownToken,
                    ) => {
                        tracing::debug!(
                            token,
                            peer = %from,
                    ...
                    Err(
                        elide_node::networking::suffix_sync::SuffixSyncError::ContentMismatch,
                    ) => {
            ...
            Ok(NetworkEvent::ObjectsRequestFailed {
                ...
                match result {
                    Ok(()) => {
                        let rejected = matches!(kind, elide_p2p::RequestFailureKind::InvalidResponse);
            ...
            Ok(NetworkEvent::ObjectsRequestBusy {
                ...
                match sync.request_busy(
                    ...
                    Err(elide_node::networking::suffix_sync::SuffixSyncError::UnknownToken) => {}
                    Err(error) => {
            ...
            Ok(NetworkEvent::MempoolSyncResponse {
                ...
                tokio::spawn(async move {
                    ...
                    for intent_bytes in txs {
                        ...
                        if let Ok(intent) = elide_tx::PagedSpendIntent::from_bytes(&intent_bytes) {
            ...
            Ok(NetworkEvent::PeerConnected {
                ...
                peer_failure_domains.insert(
                    peer,
                    elide_node::networking::FailureDomain(failure_domain),
                );
                ...
                if request_mempool
                    ...
                    && p2p_cmd
                        .try_send(elide_p2p::NetworkCommand::RequestMempoolSync { peer })
                        .is_ok()
                {
            ...
            Ok(NetworkEvent::SnapshotHeadersBatch {
                ...
                if let Some(boundary) = snapshot_boundary.filter(|boundary| {
                    boundary.height > 0
                        && boundary.height % elide_p2p::protocol::SNAPSHOT_BOUNDARY_INTERVAL == 0
                        && boundary.hash != [0; 32]
                }) {
            ...
            Ok(NetworkEvent::SnapshotHeadersRequestFailed {
                ...
                if kind == elide_p2p::RequestFailureKind::LocalCapacity {
            ...
            Ok(NetworkEvent::HeaderInventoryBatch {
                ...
                if !rejected_suffix_object_peers.contains(&from) {
                    let domain = peer_failure_domains
                        .get(&from)
                        .copied()
                        .unwrap_or(elide_node::networking::FailureDomain(u64::MAX));
                    ...
                    Ok(HeaderInventoryPlan::Candidate {
                        ...
                        let needs_snapshot = if base == old_tip {
                            ...
                            old_tip.height.saturating_sub(base.height)
                                > elide_chain::consensus::params::CONSENSUS_FINALITY_DEPTH
                                || headers.len()
                                    > elide_chain::consensus::params::CONSENSUS_FINALITY_DEPTH
                                        as usize
                                        * 2
                        ...
                        match source_independent_suffix_offer(
                            ...
                            Err(
                                elide_node::networking::suffix_sync::SuffixSyncError::MissingTipTerminal,
                            ) => {
                                let domain = peer_failure_domains.get(&from).copied().unwrap_or(
                                    elide_node::networking::FailureDomain(u64::MAX),
                                );
                ...
                if manifest.tip_height > 0 {
                        ...
                        .is_some_and(|pending| pending.manifest.as_ref() == manifest.as_ref())
                    {
                        ...
                        if let Some(sync) = active_snapshot_sync.as_mut() {
                            ...
                            let offer = elide_node::networking::snapshot_sync::SnapshotOffer::from_verified_manifest(
                                manifest.clone(),
                            );
            ...
            Ok(NetworkEvent::StateSegment { from, response }) => {
                ...
                if snapshot_staging_inflight.is_some() && queued_segment_response.is_some() {
                    ...
                    let request =
                        elide_node::networking::snapshot_sync::SnapshotSegmentRequest {
                            peer: from,
                            segment,
                        };
                ...
                if let Err(error) = sync.accept_response(from, segment, data_len) {
                    ...
                    if matches!(
                        error,
                        elide_node::networking::snapshot_sync::SnapshotSyncError::ResponseLengthMismatch
                    ) {
            ...
            Ok(NetworkEvent::HistoryStepTerminal {
                ...
                if let Some(maintenance_key) = maintenance_key {
                    ...
                    let _ = p2p_cmd.try_send(
                        elide_p2p::NetworkCommand::CancelHistoryStepTerminalRace { token },
                    );
            ...
            Ok(NetworkEvent::HistoryStepTerminalRequestFailed {
                ...
                let source_exhausted = match kind {
                    ...
                    elide_p2p::RequestFailureKind::Unavailable
                    | elide_p2p::RequestFailureKind::UnsupportedProtocol => {
                        manifest_terminal_capabilities.remove(&from);
                        snapshot_terminal_exhausted.insert(source_key);
                        true
                    }
                    elide_p2p::RequestFailureKind::Timeout
                    | elide_p2p::RequestFailureKind::Io
                    | elide_p2p::RequestFailureKind::Dial
                    | elide_p2p::RequestFailureKind::ConnectionClosed => {
            ...
            Err(elide_p2p::NetworkEventRecvError::Lagged(n)) => {
                tracing::warn!(n, "P2P gossip receiver lagged — recoverable gossip events dropped");
            }
            Err(elide_p2p::NetworkEventRecvError::Closed) => {
            ...
            match completed.result {
                Ok(AppliedExactSuffix::Live(mut applied)) => {
                    ...
                    if complete {
                        ...
                        tokio::spawn(async move {
                            if relay
                                .send(elide_p2p::NetworkCommand::AnnounceAvailability {
                                    announcement,
                                })
                                .await
                                .is_err()
        ...
        completed = snapshot_install_completion_rx.recv() => {
            ...
            if let Some(token) = completed.key.terminal_request_token {
                let _ = p2p_cmd
                    .send(elide_p2p::NetworkCommand::CancelHistoryStepTerminalRace { token })
                    .await;
            }
            match completed.result {
                Ok(applied) => {
        ...
        completed = history_step_verification_rx.recv() => {
            ...
            sync_phase_telemetry.observe_header_scale(
                ...
                completed
                    .staged_header_count
                    .saturating_mul(elide_chain::BLOCK_HEADER_WIRE_SIZE as u64),
            );
            ...
            let semantic_header_id = elide_chain::block_header::semantic_header_id(&boundary_header);
            let mut snapshot_sync = match elide_node::networking::snapshot_sync::SnapshotSync::new(
                initial_peer,
                initial_domain,
        ...
        _ = heartbeat.tick() => {
            ...
            let (our_height, our_hash, our_prev_hash, our_work, reconciled_header_dag) = {
                let ctx = chain.read().await;
                let canonical_tip = elide_node::networking::ChainPoint::new(
                    ctx.tip_height(),
                    ctx.tip_hash(),
                );
            ...
            if let Some(reconciled) = reconciled_header_dag {
                ...
                match reconciled {
                    Ok(()) => {
                        header_dag_canonical_tip = elide_node::networking::ChainPoint::new(
                            our_height,
                            our_hash,
                        );
            ...
            let unresolved_header_waiting_for_inventory = active_suffix_sync.is_none()
                ...
                && header_dag_has_unresolved_better_tip(
                    &header_dag,
                    elide_node::networking::ChainPoint::new(our_height, our_hash),
                    our_work,
                );
                    ...
                    >= EXACT_INVENTORY_PROBE_INTERVAL
            {
                ...
                let committed_tip = elide_node::networking::ChainPoint::new(our_height, our_hash);
                ...
                .is_some_and(|pending| pending.requests.deadline_due(now))
            {
                ...
                let _ = p2p_cmd.try_send(
                    elide_p2p::NetworkCommand::CancelHistoryStepTerminalRace {
...
fn create_snapshot_staging_session(
    ...
    if elide_chain::block_header::block_id(&header) != manifest.tip_hash
...
fn dispatch_exact_snapshot_segments(
    sync: &mut elide_node::networking::snapshot_sync::SnapshotSync,
    p2p_cmd: &elide_p2p::NetworkCommandSender,
    ...
    for request in sync.schedule(now_ms, 1) {
        let command = elide_p2p::NetworkCommand::RequestStateSegment {
            peer: request.peer,
...
fn update_wallet_for_block(wallet: &SharedWallet, block: &elide_chain::block::Block) {
...
fn print_startup_banner(
    ...
    let bar: String = (0..bar_w)
        ...
        .collect();
    let effective_log = log_slots.min(elide_chain::fri_state::LOG_SEGMENT_SIZE as u32) as u8;
    let max_segment_bytes = elide_chain::storage::max_encoded_segment_len_for_eff_log(effective_log)
...
fn parse_address(s: &str) -> anyhow::Result<elide_poseidon2b::primitives::Address> {
    if s.is_empty() {
