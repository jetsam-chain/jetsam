...
use elide_p2p::header_protocol::HeaderInventoryRecord;
use thiserror::Error;
...
impl HeaderDag {
    ...
    ) -> Option<(PeerId, elide_p2p::object_protocol::TerminalObjectId)> {
                    ...
                    terminal.claim.height == target.height
                        && self.nodes.get(&target.hash).is_some_and(|header| {
                            terminal.claim.semantic_header_id
                                == elide_chain::block_header::semantic_header_id(&header.header)
                        })
                })
...
mod tests {
    use super::*;
    use elide_chain::consensus::genesis_header;
    use elide_p2p::{
        ...
        HeaderInventoryRecord {
            ...
            terminal: terminal.then_some(TerminalObjectId {
                claim: TerminalClaimId {
                    height: header.header.height,
                    semantic_header_id: elide_chain::block_header::semantic_header_id(
                        &header.header,
                    ),
                    proof_class: 0,
                },
