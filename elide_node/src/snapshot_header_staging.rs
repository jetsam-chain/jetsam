...
use elide_chain::consensus::header::validate_header_timeless_prehashed_parent;
...
use elide_chain::storage::{MdbxStore, SnapshotHeaderInstallSource, VerifiedHeaderBatchRecord};
...
pub const MAX_STAGED_HEADER_BATCH: usize = elide_p2p::header_sync_codec::MAX_HEADERS_PER_BATCH;
...
impl SnapshotHeaderStaging {
    ...
    pub fn exact_boundary(
        ...
        let epoch_height = elide_chain::consensus::tx_epoch_anchor_height_for_child(expected_height);
...
fn validate_next_header(
    ...
    .map_err(|error| match error {
        elide_chain::consensus::ConsensusError::BadParentHash => {
...
pub fn validate_bounded_header_extension(
    ...
    for header in headers {
        validate_next_header(header, expected_height, previous_hash, &window)?;
        elide_chain::consensus::validate_future_drift(header.timestamp, local_time).map_err(
            |error| SnapshotHeaderStagingError::InvalidCandidate {
                height: header.height,
                reason: error.to_string(),
            },
...
mod tests {
    ...
    use elide_chain::consensus::next_target;
    use elide_chain::consensus::params::BLOCK_TIME;
    ...
    fn snapshot_validation_uses_only_the_depth_finalized_window() {
        use elide_chain::consensus::params::CONSENSUS_FINALITY_DEPTH as DEPTH;
    ...
    fn snapshot_validation_requires_the_complete_finalized_window() {
        use elide_chain::consensus::params::EXPANSION_HEADER_LOOKBACK as LOOKBACK;
    ...
    /// changes. Run with:
    /// `cargo test --release -p elide_node print_new_fixture_nonce -- --ignored --nocapture`
    #[test]
    #[ignore]
    fn print_new_fixture_nonce() {
        let mut header = fixture_chain()[1];
        header.nonce = 0;
        let nonce = elide_chain::consensus::pow::search_pow(&header, 0, 2_000_000_000)
    ...
    fn native_coinbase_child(chain: &elide_chain::storage::MdbxChainContext) -> elide_chain::Block {
        ...
        elide_chain::consensus::build_block_template(
            &parent,
            &chain.state,
    ...
    fn print_new_native_coinbase_child_nonce() {
        ...
        let chain = elide_chain::storage::MdbxChainContext::open_or_create(db.path()).unwrap();
        let child = native_coinbase_child(&chain);
        let nonce = elide_chain::consensus::pow::search_pow(&child.header, 0, 2_000_000_000)
            .expect("fixture target is satisfiable");
        println!("\nNew native coinbase child nonce: {nonce}");
    }

    fn fixture_terminal(height: u64, hash: [u8; 32]) -> Vec<u8> {
        let mut bytes = elide_chain::HistoryStepTerminalMetadata::new(height, hash, 0)
            .unwrap()
            .encode_prefix()
            .to_vec();
        bytes.push(1);
    ...
    fn empty_exact_target_uses_typestate_even_with_canonical_child() {
        let db = tempfile::tempdir().unwrap();
        let stage_dir = tempfile::tempdir().unwrap();
        let mut chain = elide_chain::storage::MdbxChainContext::open_or_create(db.path()).unwrap();
        let genesis_base = canonical_base(&chain.store);
        let block = native_coinbase_child(&chain);
        let child = block.header;
        let bundle = elide_chain::AcceptedBlockBundle::try_from_parts(
            block.to_bytes(),
            fixture_terminal(
                child.height,
                elide_chain::block_header::semantic_header_id(&child),
            ),
        )
        .unwrap();
        chain
