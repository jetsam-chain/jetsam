...
use elide_tx::{validate_paged_spend, PagedSpendError, PagedSpendFacts, Transaction, TxPage};
...
trait PageBody {
    ...
    fn page_body(&self) -> &elide_tx::TxBody;
}
...
fn validate_stream_for_class<T: PageBody>(
    ...
    while cursor < pages.len() {
        ...
        let Some(relative_end) = pages[start..]
            .iter()
            .position(|page| page.page_body().validity_bitmap & elide_tx::PAGED_SPEND_END_BIT != 0)
        else {
...
mod tests {
        ...
        (0..MAX_PAGED_SPEND_PAGES)
            .map(|page_index| {
                ...
                if page_index + 1 == MAX_PAGED_SPEND_PAGES {
                    bitmap |= elide_tx::PAGED_SPEND_END_BIT;
                }
                TxPage::new(TxBody {
    ...
    fn partial_groups_and_cross_group_conflicts_reject() {
        ...
        partial[0].body.validity_bitmap &= !elide_tx::PAGED_SPEND_END_BIT;
        assert_eq!(
