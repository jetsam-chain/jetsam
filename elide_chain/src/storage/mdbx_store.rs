...
use elide_tx::unpack_amount_creation_id;
...
#[inline]
fn owner_key_from_fields(owner_hi: elide_core::Block128, owner_lo: elide_core::Block128) -> [u8; 32] {
    let mut key = [0u8; 32];
...
mod tests {
    ...
    fn compact_restart_defers_raw_segment_check_until_hydration() {
        ...
        tampered.values[7] = elide_tx::pack_amount_creation_id(12, 1);
        let txn = store.db.begin_rw_txn().unwrap();
