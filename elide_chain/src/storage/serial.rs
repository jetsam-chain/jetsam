...
use elide_core::field::{CanonicalDeserialize, CanonicalSerialize, TowerField};
...
pub fn decode_header_chain_anchor(bytes: &[u8]) -> Option<HeaderChainAnchor> {
    ...
    pos += 32;
    let miner_address = elide_poseidon2b::primitives::Address(bytes[pos..pos + 32].try_into().ok()?);
    pos += 32;
...
mod tests {
    ...
    fn header_chain_anchor_roundtrip_uses_only_direct_tip_fields() {
        let anchor = HeaderChainAnchor {
            ...
            state_root: [2u8; 32],
            tx_root: [3u8; 32],
            miner_address: elide_poseidon2b::primitives::Address([4u8; 32]),
            log_slots: 24,
            active_slot_count: 100,
            alloc_counter: 200,
