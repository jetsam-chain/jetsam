...
use jetsam_mempool::{AsyncMempool, MempoolEvent};
...
pub type BlockAppliedHook = Arc<dyn Fn(&jetsam_chain::block::Block) + Send + Sync>;
    ...
    async fn apply_found_block(
        ...
        let (new_view, committed) = {
            ...
            tokio::task::spawn_blocking(move || {
                ...
                Ok::<_, jetsam_chain::storage::MdbxContextError>((view, committed))
            })
            .await
...
mod tests {
    ...
    fn payout_resolver_is_dynamic_while_configured_address_is_fixed() {
        let configured = jetsam_poseidon2b::primitives::Address([0x11; 32]);
        ...
        let resolver: MiningPayoutResolver = Arc::new(move || {
            jetsam_poseidon2b::primitives::Address([resolver_marker.load(Ordering::Relaxed); 32])
        });
        assert_eq!(
        ...
        assert_eq!(
            resolve_mining_payout(configured, Some(&resolver)),
            jetsam_poseidon2b::primitives::Address([0x33; 32])
        );
    }
    ...
    async fn previous_height_confirmation_does_not_cancel_fresh_pow() {
        ...
        sender
            .send(MempoolEvent::TxConfirmed {
                hash: jetsam_poseidon2b::primitives::TxBodyHash([0x11; 32]),
                block_height: 1,
            })
            .unwrap();
        ...
        sender
            .send(MempoolEvent::TxAdmitted {
                hash: jetsam_poseidon2b::primitives::TxBodyHash([0x22; 32]),
                fee: 1,
