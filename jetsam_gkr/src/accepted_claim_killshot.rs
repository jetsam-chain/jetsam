...
//! Accepted-block claim Poseidon2b KillShot.
//!
//! This component proves the fixed field-native accepted-block claim schedule:
//!
//! ```text
//! accepted_claim = Poseidon2b(ACCBLK__, fields[0..80])
//! ```

use jetsam_core::transcript::FiatShamir;
use jetsam_core::{Block128, TowerField};
use jetsam_poseidon2b::native::domain::{capacity_iv, TAG_ACCBLK};
use jetsam_poseidon2b::native::permutation::{Poseidon2bPermutation, MDS_FULL, N_ROUNDS, STATE_SIZE};
...
pub struct AcceptedClaimHashInputs {
    pub fields: [Block128; ACCEPTED_CLAIM_FIELDS],
...
impl AcceptedClaimHashProofKillShot {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct AcceptedClaimHashReductions {
    pub state: BatchEvalReduction,
    pub sin: BatchEvalReduction,
    pub sout: BatchEvalReduction,
}

fn claim_blocks(input: &AcceptedClaimHashInputs) -> [[Block128; 2]; ACCEPTED_CLAIM_BLOCKS] {
...
fn absorb_public_batch<T: FiatShamir<Block128>>(
    ...
    channel.absorb(Block128::from(TAG_ACCBLK.as_u64() as u128));
    channel.absorb(Block128::from(ACCEPTED_CLAIM_FIELDS as u128));
    for input in inputs {
...
fn evaluate_claims(inputs: &[AcceptedClaimHashInputs]) -> Option<Vec<[Block128; STATE_SIZE]>> {
    if inputs.is_empty() {
        return None;
    }
    let perm = Poseidon2bPermutation;
    let iv = capacity_iv(TAG_ACCBLK);
    let mut slot_state_ins = Vec::with_capacity(inputs.len() * ACCEPTED_CLAIM_BLOCKS);
    for input in inputs {
        ...
        if state[..ACCEPTED_CLAIM_PIN_LANES] != input.expected_claim {
            return None;
        }
    }
    Some(slot_state_ins)
...
fn claim_chain_claims(inputs: &[AcceptedClaimHashInputs], num_vars: usize) -> Vec<LinearEvalClaim> {
    let iv = capacity_iv(TAG_ACCBLK);
    ...
    for (idx, input) in inputs.iter().enumerate() {
        ...
        let last_slot = base + ACCEPTED_CLAIM_BLOCKS - 1;
...
pub fn prove_accepted_claim_hash_killshot<T: FiatShamir<Block128>>(
    inputs: &[AcceptedClaimHashInputs],
    channel: &mut T,
) -> (AcceptedClaimHashProofKillShot, AcceptedClaimHashReductions) {
    let slot_state_ins =
        evaluate_claims(inputs).expect("prover asked to prove wrong accepted-claim hash");
    ...
    let proof = AcceptedClaimHashProofKillShot {
    ...
    let reductions = AcceptedClaimHashReductions {
        state: state_red,
        sin: sin_red,
        sout: sout_red,
    };
...
pub fn verify_accepted_claim_hash_killshot<T: FiatShamir<Block128>>(
    proof: &AcceptedClaimHashProofKillShot,
    inputs: &[AcceptedClaimHashInputs],
    channel: &mut T,
) -> Option<AcceptedClaimHashReductions> {
    if inputs.is_empty()
        || proof.n_claims != inputs.len()
        || proof.live_slots != inputs.len() * ACCEPTED_CLAIM_BLOCKS
    {
        return None;
    }
    ...
    Some(AcceptedClaimHashReductions {
        state: state_red,
        sin: sin_red,
        sout: sout_red,
    })
...
pub fn discharge_accepted_claim_hash_reductions_native(
    inputs: &[AcceptedClaimHashInputs],
    reductions: &AcceptedClaimHashReductions,
) -> bool {
...
mod tests {
    use super::*;
    use jetsam_poseidon2b::channel::Poseidon2bChannel;
    use jetsam_poseidon2b::native::compression::Poseidon2bSponge;

    fn input(seed: u128) -> AcceptedClaimHashInputs {
    ...
    #[test]
    fn accepted_claim_hash_roundtrip() {
        let inputs = vec![input(1), input(2)];
        let mut ch_p = Poseidon2bChannel::new();
        let (proof, reductions) = prove_accepted_claim_hash_killshot(&inputs, &mut ch_p);

        let mut ch_v = Poseidon2bChannel::new();
        let verified = verify_accepted_claim_hash_killshot(&proof, &inputs, &mut ch_v)
            .expect("accepted claim hash proof verifies");
        assert_eq!(verified, reductions);
    }

    #[test]
    fn accepted_claim_hash_rejects_tamper() {
        ...
        let mut ch_v = Poseidon2bChannel::new();
        assert!(verify_accepted_claim_hash_killshot(&proof, &bad, &mut ch_v).is_none());
    }
}
