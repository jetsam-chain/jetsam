...
fn build_w_table_flat(claims: &[EvalClaim], alphas: &[Block128], n: usize) -> Vec<u128> {
    use jetsam_core::hardware::{clmul_gcm, tower_to_flat_u128};
...
fn eval_round_flat(w: &[u128], b: &[u128], half: usize) -> [u128; 3] {
    use jetsam_core::hardware::clmul_gcm;
    let f2_flat = jetsam_core::hardware::tower_to_flat_u128(jetsam_core::Block128::from(2u128).0);
...
// Public: the in-circuit trace transliterations (`jetsam_recursive::acceptance::trace`)
...
pub fn prove_batch_eval<T: FiatShamir<Block128>>(
    ...
    for _round in 0..n {
        ...
        let evals: [Block128; 3] =
            evals_flat.map(|v| Block128::from(jetsam_core::hardware::flat_to_tower_u128(v)));
        ...
        let r_i_flat = jetsam_core::hardware::tower_to_flat_u128(r_i.0);
    ...
    let b_final = Block128::from(jetsam_core::hardware::flat_to_tower_u128(b_flat[0]));
    let w_final = Block128::from(jetsam_core::hardware::flat_to_tower_u128(w_flat[0]));
...
pub fn prove_multi_batch_eval<T: FiatShamir<Block128>>(
    ...
    let b_finals: Vec<Block128> = b_tables
        .iter()
        .map(|b| Block128::from(jetsam_core::hardware::flat_to_tower_u128(b[0])))
        .collect();
    let w_finals: Vec<Block128> = w_tables
        .iter()
        .map(|w| Block128::from(jetsam_core::hardware::flat_to_tower_u128(w[0])))
        .collect();
