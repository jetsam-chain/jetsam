...
BASE = Path(
    os.environ.get(
        "JETSAM_LIVE_WALLET_RECEIVE_OFFLINE_SNAPSHOT_DIR",
        str(RUN_PARENT / f"wallet-receive-offline-snapshot-clean-{STAMP}"),
    )
)
BASE_PORT = int(
    os.environ.get("JETSAM_LIVE_WALLET_RECEIVE_OFFLINE_SNAPSHOT_BASE_PORT", "22300")
)
FUNDING_HEIGHT = 4
RETAINED_DEPTH = 18
SNAPSHOT_BOUNDARY_INTERVAL = 6
PAYMENT_MICRO_JTM = 350_000
...
def main():
    ...
    try:
        ...
        require(
            re.search(
                rf"wallet active address reloaded .*height={snapshot_boundary_height}.*utxos=1.*balance={PAYMENT_MICRO_JTM}.*reason=\"snapshot sync\"",
                recipient_log,
            )
            is not None,
