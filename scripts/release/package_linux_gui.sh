...
TEMPORARY=$(mktemp -d "${TMPDIR:-/tmp}/elide-linux-gui.XXXXXX")
cleanup() {
  local status=$?
  if [[ -d $TEMPORARY && $TEMPORARY == "${TMPDIR:-/tmp}"/elide-linux-gui.* ]]; then
    rm -r -- "$TEMPORARY" || true
  fi
  exit "$status"
}
...
PACKAGE_ROOT="$TEMPORARY/elide-gui_${VERSION}_${DEBIAN_ARCHITECTURE}"
install -d \
  "$PACKAGE_ROOT/DEBIAN" \
  "$PACKAGE_ROOT/usr/bin" \
  "$PACKAGE_ROOT/usr/lib/elide" \
  "$PACKAGE_ROOT/usr/share/applications" \
  "$PACKAGE_ROOT/usr/share/doc/elide-gui" \
  "$PACKAGE_ROOT/usr/share/metainfo"
for size in 16 32 48 64 128 256 512; do
  install -d "$PACKAGE_ROOT/usr/share/icons/hicolor/${size}x${size}/apps"
done

install -m 0755 "$BIN_DIR/elide-gui" "$PACKAGE_ROOT/usr/lib/elide/elide-gui"
install -m 0755 "$BIN_DIR/elide" "$PACKAGE_ROOT/usr/lib/elide/elide"
ln -s ../lib/elide/elide-gui "$PACKAGE_ROOT/usr/bin/elide-gui"
install -m 0644 "$RELEASE_ROOT_DIR/LICENSE" \
  "$PACKAGE_ROOT/usr/share/doc/elide-gui/LICENSE"
install -m 0644 "$RELEASE_ROOT_DIR/NOTICE" \
  "$PACKAGE_ROOT/usr/share/doc/elide-gui/NOTICE"
...
for size in 16 32 48 64 128 256 512; do
  install -m 0644 \
    "$RELEASE_ROOT_DIR/elide_gui/assets/app-icons/Parano1d-${size}.png" \
...
ARTIFACT="$OUTPUT_DIR/elide-gui-v${VERSION}-${PLATFORM}.deb"
...
[[ -L $EXTRACTED/usr/bin/elide-gui ]]
[[ -s $EXTRACTED/usr/share/doc/elide-gui/LICENSE ]]
[[ -s $EXTRACTED/usr/share/doc/elide-gui/NOTICE ]]
[[ -s $EXTRACTED/usr/share/metainfo/org.elide.wallet.metainfo.xml ]]
[[ ! -e $EXTRACTED/usr/lib/elide/elide-cli ]]
[[ ! -e $EXTRACTED/usr/lib/elide/elide-miner ]]
"$EXTRACTED/usr/lib/elide/elide-gui" --release-self-check >/dev/null
printf '%s\n' "$ARTIFACT"
