...
use elide_core::mle::evaluate::evaluate_flat_with_scratch;
use elide_core::{AdditiveNTT, Block128, TowerField};
use elide_fri::code::{Code, LOG_RATE, RATE};
use elide_fri::hasher::CryptographicHasher;
use elide_fri::merkle::{compute_leaf_hashes, MerkleTree, VectorCommitment};
...
pub fn prove_mixed_opening(
    ...
    use elide_core::hardware::{clmul_gcm, flat_to_tower_u128, tower_to_flat_u128};
...
///
/// Public so the in-circuit trace twin (`elide_recursive::acceptance::trace`)
...
/// (`elide_ivc_core::deep_chain::leaf_hash::flat_high_pair_leaf_hash`) can be
...
mod tests {
    ...
    fn valid_one_col_fixture_with_log(
        ...
        let ntt = AdditiveNTT::<Block128>::new(log_rows + elide_fri::code::LOG_RATE);
    ...
    fn source_pair_reduction_matches_batched_code_symbols() {
        ...
        let col_refs: Vec<&[Block128]> = cols.iter().map(Vec::as_slice).collect();
        let ntt = AdditiveNTT::<Block128>::new(TEST_LOG_ROWS + elide_fri::code::LOG_RATE);
        let hasher = Poseidon2bSponge::new();
