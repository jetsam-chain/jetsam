...
//!
//! - **Rate 1/32** (`CAPSULE_LOG_RATE = 5`, decoupled from `elide_fri`'s
...
use elide_core::mle::eq::eq_ind_partial_eval;
use elide_core::mle::evaluate::evaluate_slice;
use elide_core::{AdditiveNTT, Block128, Block256, TowerField};
use elide_fri::hasher::{CryptographicHasher, HashOutput};
use elide_fri::Channel;
use elide_poseidon2b::native::compress_flat_feed_forward_with_tag;
use elide_poseidon2b::native::compression::Poseidon2bFlatSponge;
use elide_poseidon2b::native::domain::{TAG_CAPS256, TAG_CAPSLEAF, TAG_CAPSMIX, TAG_CAPSNODE};
...
/// log2 of the capsule code's inverse rate. Decoupled from `elide_fri`'s
...
impl CryptographicHasher for CapsuleNodeHasher {
    ...
    fn batch_compress(&self, pairs: &[HashOutput], out: &mut [HashOutput]) {
        elide_poseidon2b::batch::compress_flat_ff_batch_interleaved_with_tag_into(
            TAG_CAPSNODE,
            pairs,
            out,
        );
...
/// construction as `elide_fri::code::Code::new`, at the capsule's own rate.
