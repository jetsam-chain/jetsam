...
pub struct BlockR1cs {
    ...
    /// Drives the lincheck constant-wire pin for matrix-based circuits. Custom
    /// circuit walkers can set this `None` and carry the pin on their own
    /// `LincheckCircuit`. See `docs/const-wire-pin.md`.
    pub const_pin: Option<usize>,
    /// Lazily-cached Poseidon2b digest of the R1CS instance — see
...
impl BlockR1cs {
    ...
    /// whose `BlockR1cs` carries empty matrix stubs — those must keep their
    /// circuit walkers.
    ...
    /// Poseidon2b hash of the R1CS instance (parameters + sparse matrices).
    ...
    /// (matters for dense matrices), subsequent calls are essentially free.
    pub fn statement_digest(&self) -> [u8; 32] {
        *self.digest_cache.get_or_init(|| {
            ...
            elide_poseidon2b::native::poseidon2b_hash_byte_slices(b"ELD/IVC/R1CS-STMT", &[&bytes])
        })
    }
...
/// Length-prefixed absorption of a sparse matrix into statement bytes.
...
fn absorb_matrix(out: &mut Vec<u8>, m: &SparseBinaryMatrix) {
...
/// ~5x the 8-wide kernel on dense matrices in single-threaded runs, where it
/// is purely compute-bound.
