...
use elide_poseidon2b::native::domain::{TAG_COMPRESS, TAG_TX8X2, capacity_iv_flat};
use elide_poseidon2b::native::permutation::STATE_SIZE;

/// Number of raw, two-lane body leaves.
pub const SPINE_TREE_LEAVES: usize = elide_poseidon2b::primitives::TX8X2_LEAF_COUNT;
...
fn iv_flat(tag: elide_poseidon2b::native::domain::DomainTag) -> [F128; 2] {
...
/// is exactly `elide_tx::body_hash::body_hash_leaves`:
///
...
mod tests {
    ...
    use elide_poseidon2b::native::domain::capacity_iv;
    ...
    fn phi(value: Block128) -> F128 {
        let flat = elide_core::hardware::tower_to_flat_u128(value.0);
        F128 {
            lo: flat as u64,
            hi: (flat >> 64) as u64,
        }
    ...
    fn final_spine_matches_independent_tower_reference() {
        ...
        for _ in 0..4 {
            ...
            let native = elide_poseidon2b::primitives::hash_tx8x2_leaves(&leaves);
            let native_hash = [
