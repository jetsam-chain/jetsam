...
use elide_poseidon2b::native::domain::{TAG_COMPRESS, capacity_iv_flat};
...
/// output lanes `(0, 1)`. Mirrors `elide_poseidon2b`'s tower `hash_pair`
...
/// ```
///
/// Mirrors `elide_poseidon2b::native::compress` lane-for-lane under φ.
...
mod tests {
    use super::*;
    use elide_core::{Block128, CanonicalSerialize, TowerField};
    ...
    fn ref_compress(l: [Block128; 2], r: [Block128; 2]) -> [Block128; 2] {
        let out =
            elide_poseidon2b::native::compress(&digest_bytes(l[0], l[1]), &digest_bytes(r[0], r[1]));
        [
    ...
    fn flat_compress_matches_native_tower_compress() {
        ...
        for _ in 0..16 {
            ...
            let native =
                elide_poseidon2b::native::compress(&digest_bytes(a0, a1), &digest_bytes(b0, b1));
