...
use elide_poseidon2b::native::domain::{TAG_EXSTSLT, capacity_iv_flat};
use elide_poseidon2b::native::permutation::STATE_SIZE;

/// Domain tag for encoded-source leaf hashes — the flat image of
/// `elide_fri_binius::interleaved_commit::SOURCE_LEAF_DOMAIN`. Kept in sync
...
// codeword symbols `s0, s1` (see `elide_fri_binius::mixed_open`). Its hash
...
/// `elide_fri_binius::mixed_open::HIGH_PAIR_DOMAIN`. Kept in sync with the
...
// `elide_chain::exact_state_hash`, whose per-permutation form is
// `elide_gkr::state_leaf_killshot::evaluate_slot_leaf`):
//
// ```text
...
mod tests {
    ...
    /// `elide_chain::exact_state_hash::slot_leaf_hash` /
    ...
    fn sponge_leaf_digest_matches_native_slot_leaf_hash() {
        ...
        use elide_poseidon2b::native::domain::{TAG_EXSTSLT, capacity_iv};

        // φ: tower Block128 → flat F128 lane (per 16-byte lane).
        let tower_flat = |b: Block128| -> F128 {
            let f = elide_core::hardware::tower_to_flat_u128(b.0);
            F128 {
                lo: f as u64,
                hi: (f >> 64) as u64,
            }
