...
/// proof pool. This is a low-level executor hook used by `elide_miner` worker
...
/// `elide_ivc_prover::prover::prove_fast_core`; relation wrappers reuse this then call
...
/// mirror of `elide_ivc_prover::prover::open_claims`. Relation wrappers (e.g. the hash
...
///
/// This mirrors
/// `elide_ivc_prover::field_prover::prove_field_with_public_io_and_post_commit`:
...
/// `elide_ivc_prover::field_prover::prove_field_with_public_io_and_post_commit_context`.
...
mod tests {
    ...
    /// in `elide-ivc-prover`'s `tests/verifier_roundtrip.rs`, since they need the
    /// prove path.)
    #[test]
