...
impl RpcHandler {
    ...
    ///
    /// Opt-in par env `NOID_TEMPLATE_NOGATE=1` ; sans lui, comportement
    ...
    fn require_template_gate(&self) -> RpcResult<()> {
        static TEMPLATE_NOGATE: std::sync::LazyLock<bool> = std::sync::LazyLock::new(|| {
            std::env::var("NOID_TEMPLATE_NOGATE").is_ok_and(|v| v == "1")
        });
        if !*TEMPLATE_NOGATE {
    ...
    async fn prepare_external_mining_attempt(
        ...
        //
        // Regles de validite (noid_chain/consensus/timestamps.rs) :
