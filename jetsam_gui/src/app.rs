...
fn parse_jetsam_amount(input: &str) -> Result<u64, String> {
...
mod tests {
    use super::{
        normalize_utxo_page_after_refresh, parse_jetsam_amount, record_snapshot_refresh_failure,
        utxo_page_count_for, BackendState,
    };

    #[test]
    fn parses_jetsam_without_floating_point() {
        assert_eq!(parse_jetsam_amount("1").unwrap(), 1_000_000);
        assert_eq!(parse_jetsam_amount("0.000001").unwrap(), 1);
        assert_eq!(parse_jetsam_amount("12,5").unwrap(), 12_500_000);
        assert!(parse_jetsam_amount("0").is_err());
        assert!(parse_jetsam_amount("1.0000001").is_err());
        assert!(parse_jetsam_amount("1.2.3").is_err());
    }
