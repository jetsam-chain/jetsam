...
fn exact_translation(source: &str) -> Option<(&'static str, &'static str)> {
    Some(match source {
        ...
        "Written locally to jetsam-node.log" => (
            "Записываются локально в jetsam-node.log",
            "在本机写入 jetsam-node.log",
        ),
        "ERROR" => ("ОШИБКИ", "错误"),
        "WARN" => ("ПРЕДУПР.", "警告"),
        "INFO" => ("ИНФО", "信息"),
        ...
        "No jetsam-node.log output yet…" => (
            "В jetsam-node.log пока нет данных…",
            "jetsam-node.log 暂无输出…",
        ),
