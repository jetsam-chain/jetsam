...
//! `AsyncMempool` — async wrapper around the synchronous `elide_chain::Mempool`.
//!
//! ## Architecture
//!
//! ```text
...
use elide_chain::consensus::params::BLOCK_MAX_USER_PAGES;
use elide_chain::consensus::wire_limits::{MAX_AUTHORIZATION_BYTES, MAX_TX_INTENT_BYTES_GLOBAL};
use elide_chain::consensus::{fee_breakdown, tx_epoch_anchor_height_for_child};
use elide_chain::fri_state::SlotValue;
use elide_chain::Mempool;
use elide_poseidon2b::primitives::{Address, TxBodyHash};
use elide_tx::{
...
fn entry_metadata(
    hash: TxBodyHash,
    entry: &elide_chain::mempool::MempoolEntry,
...
impl AsyncMempool {
    ...
    pub async fn submit(
        ...
        match st.pool.admit(intent.pages, tip_height) {
            ...
            Err(elide_chain::mempool::MempoolError::Full) => {
                return Err(SubmitError::Full {
                    capacity: self.config.capacity,
                });
            }
            Err(elide_chain::mempool::MempoolError::AlreadyAdmitted) => {
                return Err(SubmitError::AlreadyAdmitted(hash));
            }
            Err(e) => {
    ...
    pub async fn verified_authorization_proof_bytes(
        ...
        use elide_gkr::WalletAuthorizationBundle;
...
fn run_admission_checks(
    ...
    if actual < required {
        return Err(SubmitError::Consensus(
            elide_chain::consensus::ConsensusError::BelowMinFee { required, actual },
        ));
    }
    ...
    if spend.epoch_anchor == [0u8; 32] {
        return Err(SubmitError::Consensus(
            elide_chain::consensus::ConsensusError::BadEpochAnchor,
        ));
    }
    ...
    for page in pages {
        for (_, inp) in page.body.live_inputs() {
            if pool_inputs.contains(&inp.slot_index) {
                return Err(SubmitError::Consensus(
                    elide_chain::consensus::ConsensusError::SlotConflict,
                ));
            }
        }
        for (_, out) in page.body.live_outputs() {
    ...
    for page in pages {
        for (_, inp) in page.body.live_inputs() {
            ...
            if (idx as u64) >= view.num_slots {
                return Err(SubmitError::Consensus(
                    elide_chain::consensus::ConsensusError::ShapeMismatch(format!(
            ...
            if actual != expected {
                ...
                return Err(SubmitError::Consensus(
                    elide_chain::consensus::ConsensusError::BadStateRoot,
                ));
            }
        }
    }
...
fn verify_intent_authorization(pages: &[TxPage], authorization_bytes: &[u8]) -> Result<(), String> {
    use elide_gkr::{verify_paged_spend_authorization, WalletAuthorizationBundle};
...
mod tests {
    ...
    fn retained_intent(auth_byte: u8, auth_len: usize) -> Vec<u8> {
        let mut bytes = vec![0; elide_tx::paged_spend_authorization_wire_offset(1).unwrap()];
    ...
    async fn metadata_snapshot_never_carries_retained_intent_or_authorization_bytes() {
        ...
        assert_eq!(
            ...
            elide_chain::consensus::wire_limits::MAX_MEMPOOL_BYTES
        );
        assert_eq!(
    ...
    fn admission_accepts_tip_reward_for_the_next_block() {
        use elide_chain::consensus::params::coinbase_creation_id;
        ...
        let mempool_state = |view: ChainView| MempoolState {
            pool: elide_chain::Mempool::new(16),
            view,
        ...
        let required = elide_chain::consensus::fee_breakdown(
            1,
            1,
