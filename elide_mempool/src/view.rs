...
use elide_chain::consensus::{pow::block_id, tx_epoch_anchor_height_for_child};
use elide_chain::fri_state::SlotValue;
use elide_chain::segmented_state::SegmentedFriState;
use elide_chain::storage::MdbxStore;
...
struct SegmentReadCache {
    entries: VecDeque<(u16, Arc<elide_chain::segmented_state::SegmentColumns>)>,
}
...
impl ChainView {
    ...
    pub fn from_mdbx(ctx: &elide_chain::storage::MdbxChainContext) -> Self {
...
mod tests {
    ...
    fn next_child_anchor_is_exact_around_the_epoch_boundary() {
        ...
        let genesis = elide_chain::consensus::genesis::genesis_header();
        ...
        let view = |tip_height| {
            ChainView::new(
                ...
                headers.clone(),
                0,
                elide_chain::state::ChainState::with_log_slots(8).state,
            )
        };
