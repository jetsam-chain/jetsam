...
use elide_poseidon2b::primitives::hash_tx8x2_leaves;
...
fn killshot_reductions_consistent_with_native_mle() {
    ...
    assert_eq!(
        elide_core::mle::evaluate::evaluate_slice(&mle.state, &reductions.state.point),
        reductions.state.value
    );
    assert_eq!(
        elide_core::mle::evaluate::evaluate_slice(&mle.s_in, &reductions.sin.point),
        reductions.sin.value
    );
    assert_eq!(
        elide_core::mle::evaluate::evaluate_slice(&mle.s_out, &reductions.sout.point),
