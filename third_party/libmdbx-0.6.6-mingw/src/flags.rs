...
// The crate assumed every Windows build is MSVC. bindgen maps a C enum to the
// underlying type the target's C compiler uses: `int` (i32) under MSVC, but
// `unsigned int` (u32) under mingw's GNU toolchain. Keying the shim on `windows`
// alone therefore breaks the x86_64-pc-windows-gnu target, which is the one this
// project cross-compiles from Linux.
