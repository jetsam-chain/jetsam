...
impl<const TIER: usize> PreparedHistoryStepWitness<TIER> {
    ...
    ) -> Result<(Block, jetsam_recursive::BuiltHistoryStep, ChainAccumulator), HistoryStepWitnessError>
    {
        ...
        jetsam_chain::consensus::validation::validate_block_checks_template(
            &template,
            &parent_header,
            &previous_timestamps,
            &finalized_active_counts,
...
fn validate_nonce_independent_block(
    ...
    if block.header.prev_block_hash != jetsam_chain::hash_block_header(context.parent_header)
        || context
            .start_accumulator
            .height
            .checked_add(1)
...
fn build_history_step_components(
    ...
    for (group_index, group) in stream.groups.iter().enumerate() {
        ...
        let public =
            jetsam_gkr::OwnerAuthPublicInputs::new(tx_body_hash, group.spend.input_owner.as_fields());
...
fn slot_leaf_input(slot: jetsam_chain::SlotValue) -> SlotLeafInputs {
    SlotLeafInputs {
...
fn tx_root_merkle_inputs(
    ...
    let leaf_capacity = jetsam_chain::tx_tree::TX_TREE_LEAVES;
    let depth = jetsam_chain::tx_tree::TX_TREE_DEPTH;
    ...
    if jetsam_chain::tx_tree::bind_tx_count(root, logical_hashes.len()) != block.header.tx_root {
        return Err(HistoryStepWitnessError::TransactionRootMismatch);
    }
...
mod tests {
    ...
    use jetsam_chain::consensus::template::build_block_template;
    ...
    fn expansion_frontier_binds_parent_as_left_child_of_the_new_domain() {
        use jetsam_chain::block_id;
        use jetsam_chain::consensus::fees::required_fee_for_tx_body;
        use jetsam_chain::exact_state_hash::{state_node_hash, zero_slot_roots};
    ...
    fn tx_root_paths_bind_every_real_body_to_the_count_wrapped_root() {
        ...
        assert_eq!(paths[0].active_depth, jetsam_chain::tx_tree::TX_TREE_DEPTH);
    }
}
