...
impl FsChallenger {
    ...
    fn with_profile(domain: &[u8], tag: jetsam_poseidon2b::native::DomainTag, c1: bool) -> Self {
        let mut c = Self {
...
// `Poseidon2bPermutation` via the `jetsam_core::hardware` basis isomorphism.
...
pub fn capacity_iv_flat_lanes(tag: jetsam_poseidon2b::native::DomainTag) -> [F128; 2] {
    let [hi, lo] = capacity_iv(tag);
    [
        f128_of_u128(jetsam_core::hardware::tower_to_flat_u128(hi.0)),
        f128_of_u128(jetsam_core::hardware::tower_to_flat_u128(lo.0)),
    ]
}
...
pub fn ks_channel_iv_flat() -> [F128; 2] {
    capacity_iv_flat_lanes(jetsam_poseidon2b::native::TAG_KSCHANNL)
}
...
pub fn fs_pad_lane_flat() -> F128 {
    ...
    f128_of_u128(jetsam_core::hardware::tower_to_flat_u128(tower))
}
...
fn permute_flat(state: &mut [F128; 4]) {
    ...
    jetsam_poseidon2b::native::permutation::Poseidon2bPermutation.permute_mut(&mut tower);
    for i in 0..4 {
