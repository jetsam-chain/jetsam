...
//!
//! Ported from bolt-rs (`ligerito_recursive.rs`) onto HistoryProof primitives:
//! `F128` (GHASH irreducible), [`AdditiveNttF128`] (LCH novel basis,
//! byte-identical to bolt-rs's FFT), Poseidon2b Merkle from [`crate::merkle`],
...
/// Build a sensible default Ligerito config from the caller PCS shape.
...
/// L0 = the caller `pcs::commit` output (reused, not re-committed);
...
pub struct LigeritoSecurityConfig {
    ...
    pub log_n: usize,
    /// L0 lane fold. Must equal the caller `PcsParams::log_batch_size` so
    ...
    pub field: String,
    /// Hash function used by Merkle + FS challenger. Production value:
    /// `"poseidon2b"`.
    pub hash: String,
...
pub struct LigeritoProof {
    ...
    /// the config takes no OOD samples.
    #[serde(default)]
...
///
/// Matches HistoryProof's [`build_eq_table`] LSB-first convention (and bolt-rs's
...
// as HistoryProof's `AdditiveNttF128`, but we expose the evaluation at an arbitrary
...
/// `alpha`. `Ŵ_k = s_k / s_k(v_k)` is normalized to match HistoryProof's NTT twiddles.
///
...
pub(crate) fn induce_sumcheck_evaluate_at_residual(
    ...
    // indices >= n_queries. Replaces the older alpha^i Vandermonde scheme;
...
pub(crate) fn induce_sumcheck_poly(
    ...
    // `n_queries` indices. Replaces the older alpha^i Vandermonde scheme;
...
// ===================================================================
// Stateful sumcheck — HistoryProof (u_0, u_2) convention
// ===================================================================
//
...
impl SumcheckProver {
    ...
    /// building `b1` (saves a large read pass at high `m`). Used by
...
pub fn recursive_prover<Ch: Challenger>(
    ...
    challenger.observe_label(b"history-ligerito-v0");
...
/// (the codeword + merkle tree). This is what HistoryProof's `pcs::open_batch` will
...
fn recursive_prover_with_basis_impl<Ch: Challenger>(
    ...
    challenger.observe_label(b"history-ligerito-basis-v0");
    challenger.observe_f128(target);

    // L0 codeword + tree are borrowed (reused from caller `pcs::commit`).
...
/// `target`. Also supplies the L0 root (from the caller `Commitment`).
#[allow(clippy::too_many_arguments)]
...
fn recursive_prover_inner<Ch: Challenger>(
    ...
    // The non-basis path predates OOD binding and fold grinding;
    ...
    for i in 0..r {
        ...
        if i == r - 1 {
            ...
            return LigeritoProof {
                ...
                sumcheck_transcript: sc_prover.transcript().to_vec(),
                grinding_nonces: Vec::new(), // recursive_prover_inner: no grinding plumbed
                ood_values: Vec::new(),
...
mod tests {
    ...
    fn ligerito_security_config_rejects_paper_inconsistent_eps_pg() {
        let mut cfg = poseidon_m29_udr_example();
    ...
    fn ligerito_security_config_toml_roundtrip() {
        let cfg = poseidon_m29_udr_example();
    ...
    fn ligerito_security_config_drives_roundtrip_with_grinding() {
        ...
        // the transcript PoW search without blowing up test time).
        let queries: Vec<usize> = log_inv_rates.iter().map(|&r| udr_queries(r)).collect();
    ...
    fn induce_sumcheck_poly_consistent_with_codeword() {
        ...
        // Encode via HistoryProof's NTT (zero-pad to block_len).
    ...
    /// HistoryProof's `pcs::commit`) produces a byte-identical proof to
