...
use jetsam_poseidon2b::native::domain::{TAG_EXSTSLT, capacity_iv_flat};
use jetsam_poseidon2b::native::permutation::STATE_SIZE;

/// Domain tag for encoded-source leaf hashes — the flat image of
/// `jetsam_fri_binius::interleaved_commit::SOURCE_LEAF_DOMAIN`. Kept in sync
...
// codeword symbols `s0, s1` (see `jetsam_fri_binius::mixed_open`). Its hash
...
/// `jetsam_fri_binius::mixed_open::HIGH_PAIR_DOMAIN`. Kept in sync with the
...
// `jetsam_chain::exact_state_hash`, whose per-permutation form is
// `jetsam_gkr::state_leaf_killshot::evaluate_slot_leaf`):
//
// ```text
...
mod tests {
    ...
    /// `jetsam_chain::exact_state_hash::slot_leaf_hash` /
    ...
    fn sponge_leaf_digest_matches_native_slot_leaf_hash() {
        ...
        use jetsam_poseidon2b::native::domain::{TAG_EXSTSLT, capacity_iv};
