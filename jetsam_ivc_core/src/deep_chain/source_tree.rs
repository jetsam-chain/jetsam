...
use jetsam_poseidon2b::native::domain::{TAG_COMPRESS, capacity_iv_flat};
use jetsam_poseidon2b::native::permutation::{MDS_FULL, N_ROUNDS, STATE_SIZE};
...
/// output lanes `(0, 1)`. Mirrors `jetsam_poseidon2b`'s tower `hash_pair`
...
/// ```
///
/// Mirrors `jetsam_poseidon2b::native::compress` lane-for-lane under φ.
...
mod tests {
    use super::*;
    use jetsam_core::{Block128, CanonicalSerialize, TowerField};
    ...
    fn tower_flat(b: Block128) -> F128 {
        let f = jetsam_core::hardware::tower_to_flat_u128(b.0);
        F128 {
            lo: f as u64,
            hi: (f >> 64) as u64,
        }
    ...
    fn ref_compress(l: [Block128; 2], r: [Block128; 2]) -> [Block128; 2] {
        let out =
            jetsam_poseidon2b::native::compress(&digest_bytes(l[0], l[1]), &digest_bytes(r[0], r[1]));
        [
    ...
    fn flat_compress_matches_native_tower_compress() {
        ...
        for _ in 0..16 {
            ...
            let native =
                jetsam_poseidon2b::native::compress(&digest_bytes(a0, a1), &digest_bytes(b0, b1));
