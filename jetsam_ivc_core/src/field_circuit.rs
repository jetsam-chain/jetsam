...
//! is bit-for-bit the `jetsam_ivc_core` [`F128`] field (`clmul_gcm` ==
...
//! isomorphism `jetsam_core::hardware::tower_to_flat_u128` and back via
...
//! evaluates in the flat basis internally (`jetsam_poseidon2b`'s
//! `permute_mut`).
//!
...
/// sponge — the LaneExpr pattern of `jetsam_recursive::fs_transcript`, lifted
...
impl LayoutRecordingChallenger {
    ...
    fn record_squeeze(&mut self, value: Option<F128>) {
        if self.filled == 1 {
            ...
            let pad_tower =
                jetsam_core::hardware::flat_to_tower_u128(f128_to_u128(fs_pad_lane_flat()));
...
impl FsChannelUnionRecorder {
    ...
    fn permute(&mut self) {
        ...
        jetsam_poseidon2b::native::permutation::permute_flat_u128(&mut lanes);
    ...
    fn absorb_const(&mut self, c: F128) {
        self.cur_absorb
            .push(Some(jetsam_core::hardware::flat_to_tower_u128(
                Self::flat_bits(c),
            )));
...
/// (`jetsam_poseidon2b::channel::Poseidon2bChannel` — the `FiatShamir<Block128>`
/// implementation every `jetsam_gkr` verifier runs on). Unlike
...
mod tests {
    ...
    use jetsam_core::hardware::clmul_gcm;
    use jetsam_core::{Block128, TowerField};
    use jetsam_poseidon2b::native::permutation::Poseidon2bPermutation;
    ...
    /// the jetsam_ivc F128 multiplication on the same u128 bit layout, and the
    ...
    fn pow7_matches_native_sbox() {
        use jetsam_poseidon2b::native::permutation::sbox_x7;
