...
impl PaddingSpec {
    ...
    /// to the dense URM path with no skipping.
...
fn prove_packed_padded_inner<C: Challenger>(
    ...
    challenger.observe_label(b"history-zerocheck-v0");
    ...
    let zc_timing = std::env::var_os("NOIDH_ZC_TIMING").is_some();
...
mod tests {
    ...
    fn prove_runs_end_to_end() {
        for &m in &[13usize, 14, 15, 16] {
            ...
            let mut challenger = FsChallenger::new(b"history-test-v0");
    ...
    fn prove_verify_roundtrip_honest() {
        for &m in &[13usize, 14, 15, 16] {
            ...
            let (a_p, b_p, c_p) = pack_abc(&a, &b, &c);
            let mut ch_prove = FsChallenger::new(b"history-test-v0");
            let (proof, claim_p) = prove_packed(&a_p, &b_p, &c_p, m, &mut ch_prove);

            let mut ch_verify = FsChallenger::new(b"history-test-v0");
    ...
    fn audit_final_ab_claims_bound_to_transcript() {
        ...
        let mut ch_honest = FsChallenger::new(b"history-test-v0");
        assert!(
        ...
        let mut ch_tampered = FsChallenger::new(b"history-test-v0");
        assert!(
    ...
    fn prove_deterministic() {
        ...
        let (a_p, b_p, c_p) = pack_abc(&a, &b, &c);
        let mut ch1 = FsChallenger::new(b"history-test-v0");
        let mut ch2 = FsChallenger::new(b"history-test-v0");
