...
//!
//! - **Rate 1/32** (`CAPSULE_LOG_RATE = 5`, decoupled from `jetsam_fri`'s
...
use jetsam_core::{AdditiveNTT, Block128, Block256, TowerField};
use jetsam_fri::hasher::{CryptographicHasher, HashOutput};
use jetsam_fri::Channel;
use jetsam_poseidon2b::native::compress_flat_feed_forward_with_tag;
use jetsam_poseidon2b::native::compression::Poseidon2bFlatSponge;
use jetsam_poseidon2b::native::domain::{TAG_CAPS256, TAG_CAPSLEAF, TAG_CAPSMIX, TAG_CAPSNODE};
...
/// log2 of the capsule code's inverse rate. Decoupled from `jetsam_fri`'s
...
impl CryptographicHasher for CapsuleNodeHasher {
    ...
    fn batch_compress(&self, pairs: &[HashOutput], out: &mut [HashOutput]) {
        jetsam_poseidon2b::batch::compress_flat_ff_batch_interleaved_with_tag_into(
            TAG_CAPSNODE,
            pairs,
            out,
        );
...
/// construction as `jetsam_fri::code::Code::new`, at the capsule's own rate.
