...
use jetsam_core::mle::eq::eq_ind_partial_eval;
use jetsam_core::mle::evaluate::evaluate_flat_with_scratch;
use jetsam_core::{AdditiveNTT, Block128, TowerField};
use jetsam_fri::code::{Code, LOG_RATE, RATE};
use jetsam_fri::hasher::CryptographicHasher;
use jetsam_fri::merkle::{compute_leaf_hashes, MerkleTree, VectorCommitment};
use jetsam_fri::Channel;
use rayon::prelude::*;
use std::borrow::Cow;
...
pub fn prove_mixed_opening(
    ...
    use jetsam_core::hardware::{clmul_gcm, flat_to_tower_u128, tower_to_flat_u128};
...
///
/// Public so the in-circuit trace twin (`jetsam_recursive::acceptance::trace`)
...
/// (`jetsam_ivc_core::deep_chain::leaf_hash::flat_high_pair_leaf_hash`) can be
...
mod tests {
    ...
    fn valid_one_col_fixture_with_log(
        ...
        let ntt = AdditiveNTT::<Block128>::new(log_rows + jetsam_fri::code::LOG_RATE);
    ...
    fn source_pair_reduction_matches_batched_code_symbols() {
        ...
        let col_refs: Vec<&[Block128]> = cols.iter().map(Vec::as_slice).collect();
        let ntt = AdditiveNTT::<Block128>::new(TEST_LOG_ROWS + jetsam_fri::code::LOG_RATE);
        let hasher = Poseidon2bSponge::new();
