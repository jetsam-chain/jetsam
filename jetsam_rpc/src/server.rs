...
fn mined_record_is_canonical(
    ...
    canonical_header: &jetsam_chain::block_header::BlockHeader,
) -> bool {
...
fn block_header_info(header: &jetsam_chain::BlockHeader) -> BlockHeaderInfo {
...
fn retained_transaction_summaries(
    ...
    bundle_bytes: &[u8],
    address: Option<&jetsam_poseidon2b::primitives::Address>,
) -> Result<Vec<RecentTransactionInfo>, String> {
    let bundle = jetsam_chain::AcceptedBlockBundle::decode(bundle_bytes)
        .map_err(|error| format!("decode retained block bundle: {error}"))?;
    let block = jetsam_chain::Block::from_bytes(bundle.block_bytes())
    ...
    let logical_txids = jetsam_chain::try_compute_logical_txids(&block.transactions)
...
fn read_canonical_slot(
    ...
    slot_index: u32,
    segment_cache: &mut HashMap<u16, jetsam_chain::segmented_state::SegmentColumns>,
) -> Result<jetsam_chain::fri_state::SlotValue, String> {
...
fn encode_pow_fields_hex(header: &jetsam_chain::BlockHeader) -> String {
...
fn mix_slot_hint_seed(tip_seed: u64, salt: u64) -> u64 {
    ...
    jetsam_chain::consensus::allocator::splitmix64(&mut s)
}

fn fee_breakdown_info(
    breakdown: jetsam_chain::consensus::FeeBreakdown,
    relay_floor: u64,
    paid_total: u64,
) -> FeeBreakdownInfo {
...
fn validate_tx_counts(n_inputs: usize, n_outputs: usize) -> Result<(), ErrorObject<'static>> {
    if (1..=jetsam_tx::MAX_PAGED_SPEND_INPUTS).contains(&n_inputs)
        && (1..=jetsam_tx::MAX_PAGED_SPEND_OUTPUTS).contains(&n_outputs)
    {
        Ok(())
    } else {
        Err(rpc_err(format!(
            "canonical PagedSpend supports 1..={} inputs and 1..={} outputs, got {n_inputs} and {n_outputs}",
            jetsam_tx::MAX_PAGED_SPEND_INPUTS,
            jetsam_tx::MAX_PAGED_SPEND_OUTPUTS,
        )))
    }
}
...
fn seed_from_salt_hex(salt_hex: &str) -> Result<u64, ErrorObject<'static>> {
    ...
    for chunk in bytes.chunks(8) {
        ...
        acc = jetsam_chain::consensus::allocator::splitmix64(&mut acc);
    }
    Ok(acc)
}
...
pub struct RpcHandler {
    ...
    pub p2p_cmd: jetsam_p2p::NetworkCommandSender,
    ...
    pub p2p_health: tokio::sync::watch::Receiver<jetsam_p2p::P2PHealthSnapshot>,
    ...
    pub canonical_tip_changes: tokio::sync::watch::Sender<jetsam_p2p::object_protocol::ChainPoint>,
    ...
    pub mining_payout_address: Option<jetsam_poseidon2b::primitives::Address>,
...
impl RpcHandler {
    ...
    fn resolved_mining_payout(&self) -> RpcResult<jetsam_poseidon2b::primitives::Address> {
    ...
    async fn submit_wallet_transaction(
        ...
        for attempt in 0..3u32 {
            ...
            let intent = match jetsam_tx::PagedSpendIntent::from_bytes(&intent_bytes) {
                Ok(intent) => intent,
                Err(error) => {
            ...
            let facts = jetsam_tx::validate_paged_spend(&intent.pages)
    ...
    async fn prepare_external_mining_attempt(
        self,
        requested: Option<jetsam_poseidon2b::primitives::Address>,
        ...
        let claimable_fees_micro_jtm = tmpl
            ...
            .iter()
            .map(|tx| {
                jetsam_chain::consensus::claimable_fee_for_tx_body(
                    &tx.body,
        ...
        let (prepared, prepare_elapsed) = tokio::task::spawn_blocking(move || {
            ...
            let result = jetsam_miner::install_history_step_phase_cpu(|| {
    ...
    async fn finalize_node_owned_template_after_nonce(
        ...
        let (committed, new_view) = tokio::task::spawn_blocking(move || {
            ...
            let view = jetsam_mempool::ChainView::from_mdbx(&ctx);
            canonical_tip_changes.send_replace(jetsam_p2p::object_protocol::ChainPoint::new(
        ...
        let confirmed = jetsam_chain::try_compute_logical_txids(&block.transactions)
...
impl JetsamApiServer for RpcHandler {
    ...
    async fn get_state_info(&self) -> RpcResult<StateInfo> {
        use jetsam_chain::consensus::params::{EXPAND_DENOM, EXPAND_NUM, LOG_SLOTS_MAX};
        ...
        let max_encoded_segment_bytes =
            jetsam_chain::storage::max_encoded_segment_len_for_eff_log(LOG_SEGMENT_SIZE as u8)
    ...
    async fn get_block_details(&self, height: u64) -> RpcResult<Option<BlockDetailsInfo>> {
        ...
        let proof_class = match stream.proof_class {
            jetsam_chain::consensus::BlockProofClass::B25 => "B25 / m22",
            jetsam_chain::consensus::BlockProofClass::B255 => "B255 / m24",
        }
        .to_string();
    ...
    async fn estimate_fee(&self, n_outputs: u32) -> RpcResult<u64> {
        ...
        Ok(
            jetsam_chain::consensus::fee_breakdown(1, n_outputs as u64, active_slot_count, log_slots)
                .required_total
                .max(floor),
        )
    }
    ...
    async fn submit_tx_intent(&self, hex_str: String) -> RpcResult<String> {
        ...
        let intent = jetsam_tx::PagedSpendIntent::from_bytes(&bytes)
    ...
    async fn verify_receipt(&self, receipt_hex: String) -> RpcResult<ReceiptVerifyResult> {
        use jetsam_chain::consensus::receipt::{verify_against_header, verify_merkle_inclusion};
...
mod tests {
    ...
    fn mined_block_identity_marks_exact_and_legacy_reorganizations() {
        let header = jetsam_chain::consensus::genesis_header();
    ...
    fn transaction_count_validation_matches_paged_spend() {
        ...
        assert!(validate_tx_counts(jetsam_tx::MAX_PAGED_SPEND_INPUTS + 1, 1).is_err());
...
pub async fn start_rpc_server(
    ...
    p2p_health: tokio::sync::watch::Receiver<jetsam_p2p::P2PHealthSnapshot>,
    canonical_tip_changes: tokio::sync::watch::Sender<jetsam_p2p::object_protocol::ChainPoint>,
    ...
    let wallet_submission_mempool =
        mempool
            ...
            .with_authorization_verification_executor(Arc::new(
                |task: jetsam_mempool::AuthorizationVerificationTask| {
                    jetsam_miner::install_wallet_verifier_cpu(task).map_err(|error| {
