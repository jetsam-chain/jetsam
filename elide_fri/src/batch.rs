...
pub fn prove_batched_mixed(
    ...
    let column_openings: Vec<Block128> = {
        ...
        evals_per_col
            ...
            .map(|(col, &ll)| elide_core::mle::evaluate::evaluate_slice(col, &eval_points[&ll]))
            .collect()
    };
...
pub fn prove_batched(
    ...
    let column_openings: Vec<Block128> = {
        ...
        evals_per_col
            .par_iter()
            .map(|col| elide_core::mle::evaluate::evaluate_slice(col, eval_point))
            .collect()
    };
    channel.observe_field_elems(&column_openings);
...
mod tests {
    ...
    fn prove_verify_batched_roundtrip() {
        ...
        let expected_openings: Vec<Block128> = cols
            .iter()
            .map(|c| elide_core::mle::evaluate::evaluate_slice(c, &eval_point))
            .collect();
    ...
    fn mixed_batched_roundtrip_two_log_lens() {
        ...
        let expected: Vec<Block128> = cols
            .iter()
            .zip(col_log_lens.iter())
            .map(|(c, &ll)| elide_core::mle::evaluate::evaluate_slice(c, &eps[&ll]))
            .collect();
