...
fn mle_evaluate(evals: &[Block128], point: &[Block128]) -> Block128 {
    ...
    if PACKED_LANES > 1 && evals.len() >= PACKED_LANES && evals.len().is_multiple_of(PACKED_LANES) {
        elide_core::mle::evaluate::evaluate_packed(evals, point)
    } else {
...
fn fold_evals(evals: &[Block128], r: Block128) -> Vec<Block128> {
    ...
    if PACKED_LANES > 1 && half >= PACKED_LANES && evals.len().is_multiple_of(PACKED_LANES) {
        ...
        elide_core::sumcheck::prove::fold_highest_var_packed_inplace(&mut data, r, half);
        data
    } else if half >= 1024 {
...
pub fn prove(
    ...
    let left_eq = elide_core::mle::eq::eq_ind_partial_eval(left);
    ...
    let batching_eq = elide_core::mle::eq::eq_ind_partial_eval(&tensor_batching_point);
    ...
    let eq_right = elide_core::mle::eq::eq_ind_partial_eval(right);
