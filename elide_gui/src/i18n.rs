...
fn exact_translation(source: &str) -> Option<(&'static str, &'static str)> {
    Some(match source {
        ...
        "Written locally to elide-node.log" => (
            "Записываются локально в elide-node.log",
            "在本机写入 elide-node.log",
        ),
        "ERROR" => ("ОШИБКИ", "错误"),
        "WARN" => ("ПРЕДУПР.", "警告"),
        "INFO" => ("ИНФО", "信息"),
        ...
        "No elide-node.log output yet…" => (
            "В elide-node.log пока нет данных…",
            "elide-node.log 暂无输出…",
        ),
