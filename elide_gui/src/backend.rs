...
const NETWORK_STORAGE_EPOCH: &[u8] = b"elide/mainnet/network-storage/v1/860e70453390bf815718e933aa4927167a13d098b0151391eefd722ee1add610\n";
...
impl Backend {
    ...
    pub async fn node_log_tail(&self, max_bytes: u64, max_lines: usize) -> Result<String, String> {
        let log_path = self.config_snapshot()?.data_dir.join("elide-node.log");
    ...
    async fn rpc_with_timeout<T: DeserializeOwned>(
        ...
        let body = RpcRequest {
            jsonrpc: "2.0",
            id,
            method: format!("paraelide_{method}"),
            params,
        };
    ...
    fn spawn(&self, mode: NodeMode, selected_threads: usize, genesis: bool) -> Result<(), String> {
        ...
        let config_path = config.data_dir.join("elide-gui.toml");
        let log_path = config.data_dir.join("elide-node.log");
                ...
                if let Some(child) = state.child.as_mut() {
                        ...
                        .map_err(|error| format!("inspect node process: {error}"))?
                    {
                        ...
                        return Err(format!(
                            ...
                            config.data_dir.join("elide-node.log").display()
                        ));
                    }
                }
            }
...
async fn run_node_maintenance(
    ...
    command
        .arg("--config")
        .arg(config.data_dir.join("elide-gui.toml"))
        .arg("--data-dir")
        .arg(&config.data_dir)
        .arg("--log")
        .arg("error")
...
#[cfg(target_os = "windows")]
const SIBLING_NODE_NAMES: &[&str] = &["elide-node.exe", "elide.exe"];
#[cfg(target_os = "macos")]
const SIBLING_NODE_NAMES: &[&str] = &["elide-node", "elide"];
#[cfg(not(any(target_os = "windows", target_os = "macos")))]
const SIBLING_NODE_NAMES: &[&str] = &["elide"];
...
mod tests {
    ...
    async fn node_log_tail_reads_only_the_latest_complete_lines() {
        let directory = tempfile::tempdir().unwrap();
        let path = directory.path().join("elide-node.log");
        let contents = (0..120)
