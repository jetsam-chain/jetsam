    ...
    pub fn update(&mut self, message: Message) -> Task<Message> {
        match message {
            ...
            Message::SubmitSend => {
                ...
                let amount_micro_eld = match parse_elide_amount(&self.send_amount) {
                    Ok(amount) => amount,
                    Err(error) => {
...
fn parse_elide_amount(input: &str) -> Result<u64, String> {
...
mod tests {
    use super::{
        normalize_utxo_page_after_refresh, parse_elide_amount, record_snapshot_refresh_failure,
        utxo_page_count_for, BackendState,
    };

    #[test]
    fn parses_elide_without_floating_point() {
        assert_eq!(parse_elide_amount("1").unwrap(), 1_000_000);
        assert_eq!(parse_elide_amount("0.000001").unwrap(), 1);
        assert_eq!(parse_elide_amount("12,5").unwrap(), 12_500_000);
        assert!(parse_elide_amount("0").is_err());
        assert!(parse_elide_amount("1.0000001").is_err());
        assert!(parse_elide_amount("1.2.3").is_err());
    }
