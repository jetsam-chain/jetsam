...
use jetsam_soundness::calculate;
...
fn main() -> Result<(), String> {
    ...
    for argument in std::env::args().skip(1) {
        match argument.as_str() {
            "--exact" => exact = true,
            "-h" | "--help" => {
                println!("Usage: jetsam_soundness [--exact]");
    ...
    if exact {
        println!(
            ...
            jetsam_soundness::poseidon2b_cryptanalysis::SKIPPING_CLASS_REVIEWED_VERSION,
            jetsam_soundness::poseidon2b_cryptanalysis::SKIPPING_CLASS_PDF_SHA256
        );
    }
    println!(
