...
use elide_chain::consensus::allocator::{generate_zone_segment_hints, splitmix64};
...
pub(crate) fn collect_empty_slot_hints(
    ...
    for segment_id in partial_segments {
        ...
        hints = collect_empty_slot_hints_streaming(
            ...
            |index| state.slot(index) == elide_chain::fri_state::SlotValue::EMPTY,
    ...
    for segment_id in generate_zone_segment_hints(
        ...
        hints = collect_empty_slot_hints_streaming(
            ...
            |_| false,
            |index| state.slot(index) == elide_chain::fri_state::SlotValue::EMPTY,
            |_| -> Result<elide_chain::segmented_state::SegmentColumns, String> {
...
fn load_durable_segment(
    ...
    segment_id: u16,
    expected_log: usize,
) -> Result<elide_chain::segmented_state::SegmentColumns, String> {
...
impl ExactSegmentSlots for elide_chain::segmented_state::SegmentColumns {
    fn slot_is_empty(&self, segment_id: u16, local_index: usize) -> Result<bool, String> {
...
pub fn next_user_epoch_anchor(chain: &MdbxChainContext) -> Result<[u8; 32], String> {
    ...
    let anchor_height = elide_chain::consensus::tx_epoch_anchor_height_for_child(child_height);
    let header = chain
...
mod tests {
    ...
    fn fallback_preserves_too_short_segment_failure_order() {
        ...
        let error = collect_empty_slot_hints_streaming(
            ...
            |segment_id| {
                if segment_id == 3 {
                    let mut columns = elide_chain::segmented_state::SegmentColumns::new_zero(1);
                    columns.values[0] = 1u64.into();
                    Ok(columns)
                } else {
                    Ok(elide_chain::segmented_state::SegmentColumns::new_zero(8))
                }
            },
        )
        .unwrap_err();
    ...
    fn production_allocator_returns_the_freed_slot_before_opening_a_new_segment() {
        ...
        use elide_tx::{TxBody, TxInput, TxOutput, TX_INPUTS, TX_OUTPUTS};
        ...
        let full_segment = (0..SEGMENT_SIZE)
            ...
            .collect::<Vec<_>>();
        chain.state =
            elide_chain::ChainState::from_sparse_utxos(24, &full_segment, u64::from(SEGMENT_SIZE))
        ...
        chain.state = elide_chain::ChainState::from_sparse_utxos(24, &[(7, sole_slot)], 1)
            .expect("one-slot production segment");
        elide_chain::apply_tx(&mut chain.state, &spend_only(7, 1)).unwrap();
        ...
        let first_zone_for_segment_zero = (0u64..256)
            .find(|zone| {
                generate_zone_segment_hints(
                    zone * elide_chain::consensus::allocator::ZONE_CAPACITY,
                    24,
                    1,
                ) == vec![0]
            })
        ...
        chain.state.alloc_counter =
            (first_zone_for_segment_zero + 256) * elide_chain::consensus::allocator::ZONE_CAPACITY;
