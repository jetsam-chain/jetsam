...
use elide_ivc_core::deep_chain::relations::{claimed_refs, ColRef, RELATION_DEGREE};
use elide_ivc_core::deep_chain::schedule::{
    carry_selection_terms, duplex_substitution_terms, DuplexFamilyRefs,
};
use elide_ivc_core::deep_chain::WALK_DEGREE;
...
/// Exact shape of [`elide_ivc_core::deep_chain::MultiDeepChainWalkProof`].
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
pub(super) fn duplex_like_proof_shape(
    ...
    let shifts = substitution_refs
        ...
        .filter(|reference| {
            matches!(
                reference,
                elide_ivc_core::deep_chain::relations::ColRef::CommittedShift(_)
                    | elide_ivc_core::deep_chain::relations::ColRef::CommittedShift2(_)
            )
        })
        .count();
    FixedProofShape {
