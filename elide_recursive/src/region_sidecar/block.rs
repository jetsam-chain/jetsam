...
use elide_ivc_core::deep_chain::capsule_leaf::raw_flat_lane;
...
use elide_ivc_core::pcs::{C1QuirkyDirectClaim, PcsParams};
use elide_ivc_core::public_io::{PublicIoSpec, WitnessSlice};
use elide_poseidon2b::native::domain::{
...
pub(crate) fn selected_zk_block_geometry_for_auth_tiles(
    ...
    elide_chain::consensus::params::BLOCK_PAGE_CLASS_TIERS
        .into_iter()
...
impl BlockRegionSidecarVk {
    ...
    pub(crate) fn validate_selected_zk_roles(&self) -> Result<(), RegionSidecarError> {
        ...
        let geometry = elide_chain::consensus::params::BLOCK_PAGE_CLASS_TIERS
            .into_iter()
            .filter_map(selected_zk_block_geometry)
...
fn selected_duplex_vk_matches(
    vk: &DuplexRegionVk,
    layout: &elide_ivc_core::deep_chain::schedule::DuplexLayout,
    purpose: [u8; 32],
    w_log: usize,
    tile_log: usize,
) -> bool {
...
pub(crate) fn shape_only_c1_block_region_walk_deferred_proof(
    ...
    use elide_ivc_core::deep_chain::relations::c1::{C1ColumnRelationProof, C1ShiftDischargeProof};
    ...
    let relation = |rounds: usize, values: usize| C1ColumnRelationProof {
        rounds: vec![[F256::ZERO; elide_ivc_core::deep_chain::relations::RELATION_DEGREE]; rounds],
