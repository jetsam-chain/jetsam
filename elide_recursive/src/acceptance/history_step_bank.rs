...
use elide_chain::consensus::params::BLOCK_PAGE_CLASS_TIERS;
use elide_core::Block128;
use elide_ivc_core::challenger::{Challenger, FsLaneChallenger};
use elide_ivc_core::field::{F128, F256};
use elide_ivc_core::field_circuit::{f128_from_u128, f128_to_u128, FieldR1csBuilder, FsChannelOps};
use std::sync::Arc;

use elide_ivc_core::field_r1cs::{CompactFieldR1cs, FieldR1cs};
...
use elide_ivc_core::pcs::{PcsParams, BASEFOLD_RATE_QUARTER_C1_QUERIES, LOG_PACKING};
use elide_ivc_core::proof::{pcs_params_statement_bytes, FieldShape};
...
const _: () = assert!(
    HISTORY_STEP_CURRENT_CLASS_MS[0]
        == elide_chain::consensus::paged_spend::BlockProofClass::B25.outer_m()
        && HISTORY_STEP_CURRENT_CLASS_MS[1]
            == elide_chain::consensus::paged_spend::BlockProofClass::B255.outer_m(),
...
pub fn canonical_history_step_shape(class_id: CanonicalHistoryStepClassId) -> FieldShape {
    ...
    FieldShape {
        m,
        k_log: m,
        k_skip: elide_ivc_core::zerocheck::K_SKIP,
        const_pin: Some(0),
    }
}
...
pub fn history_step_bank_block_accumulator(
    ...
    let lanes = parsed.block_accumulator.map(|lane| {
        ...
        Block128::from(elide_core::hardware::flat_to_tower_u128(flat))
    });
...
pub(crate) fn observe_history_step_bank_fold_route_trace(
    ...
    selected_parent_class: &elide_ivc_core::field_circuit::LinExpr,
) {
...
mod tests {
    ...
    use elide_ivc_core::field_circuit::{FsChannelOps, LinExpr};
    ...
    fn history_step_soundness_ledger_matches_basefold_target() {
        assert_eq!(
            elide_gkr::zk_auth_qrom::HISTORY_STEP_CLASSICAL_BITS,
            elide_ivc_core::pcs::BASEFOLD_UDR_TARGET_BITS
        );
        assert_eq!(elide_gkr::zk_auth_qrom::HISTORY_STEP_QROM_BITS, 83);
    }
    ...
    fn route_selection_requires_the_authenticated_parent_class() {
        ...
        let matrix = FieldR1cs {
            ...
            k_skip: 6,
            useful_rows: 1,
            a_0: elide_ivc_core::field_r1cs::SparseFieldMatrix::zero(k),
            b_0: elide_ivc_core::field_r1cs::SparseFieldMatrix::zero(k),
            const_pin: Some(0),
