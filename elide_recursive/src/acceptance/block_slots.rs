...
use elide_poseidon2b::native::compression::compress;
use elide_poseidon2b::native::domain::{capacity_iv, TAG_BLOCKHDR, TAG_SEMHDR, TAG_TXROOT};
...
use elide_gkr::SpineInputs;
use elide_ivc_core::deep_chain::spine::SpineInstanceFlat;
use elide_ivc_core::field_circuit::f128_to_u128;
...
mod selected_zk_capability_tests {
    ...
    fn b255_capability_is_zero_row_exact_and_pad_is_constant_metadata() {
        let ghost_body = elide_gkr::ghost_tx::ghost_tx_body();
        let ghost_statement = elide_gkr::zk_authorization::ZkAuthCapsuleOwnerStatement {
            tx_body_hash: elide_gkr::ghost_tx::ghost_tx_body_hash(),
...
const _: () =
    assert!(header_fields::FIELDS + 1 == elide_chain::consensus::pow::POW_HEADER_FIELD_COUNT);
...
fn tx_root_region_capacity_handoff(
    b: &mut FieldR1csBuilder,
    tx_root_inputs: &[elide_gkr::merkle_circuit::MerklePathInputs],
...
fn tx_root_region_capacity_data_from_wires(
    ...
    assert_eq!(depth, elide_chain::tx_tree::TX_TREE_DEPTH);
...
fn exact_state_region_capacities(
    ...
    let segments = structural
        .touched_indices
        .iter()
        .map(|slot| slot >> elide_chain::consensus::params::LOG_SEGMENT_SIZE)
...
fn bind_tx_epoch_anchors(
    ...
    const L0: usize = elide_tx::body_hash::TX8X2_LEAF_EPOCH_ANCHOR;
    ...
    let ghost =
        elide_gkr::spine_statement::spine_inputs_from_body(&elide_gkr::ghost_tx::ghost_tx_body());
    for (spine, live) in user_spines.iter().zip(user_live_bits) {
        ...
        for leaf in 0..elide_tx::body_hash::BODY_HASH_LEAVES {
            for lane in 0..2 {
                let ghost_diff =
...
pub(in crate::acceptance) struct CanonicalSelectedZkAuthorizationSlot {
    ...
    liveness: LinExpr,
    native_statement: elide_gkr::zk_authorization::ZkAuthCapsuleOwnerStatement,
...
impl CanonicalSelectedZkAuthorizationSlot {
    ...
    pub(in crate::acceptance) fn native_statement(
        &self,
    ) -> elide_gkr::zk_authorization::ZkAuthCapsuleOwnerStatement {
...
fn mint_canonical_selected_zk_authorization_capability(
    ...
    let ghost_body = elide_gkr::ghost_tx::ghost_tx_body();
    let ghost_hash = elide_gkr::ghost_tx::ghost_tx_body_hash();
    ...
    for index in 0..body_auth_slots {
        ...
        let native_statement = elide_gkr::zk_authorization::ZkAuthCapsuleOwnerStatement {
...
impl DirectHeaderTrace {
    fn alloc(b: &mut FieldR1csBuilder, header: &BlockHeader) -> Self {
        let fields = elide_chain::consensus::pow::pow_header_fields(header)
            .into_iter()
            .enumerate()
            .filter(|(index, _)| *index != elide_chain::consensus::pow::POW_NONCE_FIELD_INDEX)
            .map(|(_, field)| alloc_block(b, field))
            .collect();
        let semantic_id = digest_lanes(&elide_chain::block_header::semantic_header_id(header));
        Self {
            fields,
...
fn sponge_lanes_trace(
    b: &mut FieldR1csBuilder,
    tag: elide_poseidon2b::native::domain::DomainTag,
    fields: &[LinExpr],
) -> [LinExpr; 2] {
...
impl ParentSealTrace {
    pub(in crate::acceptance) fn alloc(b: &mut FieldR1csBuilder, parent: &BlockHeader) -> Self {
        let fields: Vec<LinExpr> = elide_chain::consensus::pow::pow_header_fields(parent)
            .into_iter()
...
fn build_selected_zk_block_slots_core(
    ...
    assert_eq!(
        elide_chain::consensus::paged_spend::BlockProofClass::for_page_count(effective_page_count)
    ...
    if cap_txs > n_real_txs {
        let ghost_body = elide_gkr::ghost_tx::ghost_tx_body();
        let ghost_spine = elide_gkr::spine_statement::spine_inputs_from_body(&ghost_body);
        let ghost_hash = elide_gkr::ghost_tx::ghost_tx_body_hash();
    ...
    let user_action_slots = tier.saturating_mul(elide_tx::TX_ACTIONS);
    let mut action_candidates = Vec::with_capacity(user_action_slots + 3);
    let mut selected_input_bits = Vec::with_capacity(tier.saturating_mul(elide_tx::TX_INPUTS));
    let mut selected_output_bits = Vec::with_capacity(tier.saturating_mul(elide_tx::TX_OUTPUTS) + 3);
    ...
    let allocation = bind_development_allocation(
        ...
        &spine_inputs[1].leaves[elide_tx::body_hash::TX8X2_LEAF_OUTPUT0_DATA][1],
    );
    ...
    let user_spine_inputs = (0..tier)
        ...
        .collect::<Vec<_>>();
    let ghost_hash_native = elide_gkr::ghost_tx::ghost_tx_body_hash();
    let ghost_hash = [
...
mod tx_epoch_anchor_tests {
    ...
    fn tail_parent_header(height: u64) -> BlockHeader {
        BlockHeader {
            ...
            timestamp: height * 10,
            height,
            miner_address: elide_poseidon2b::primitives::Address([0x66; 32]),
    ...
    fn tail_start(parent: &BlockHeader) -> ChainAccumulator {
        ChainAccumulator {
            ...
            tip_semantic_id: elide_chain::block_header::semantic_header_id(parent),
    ...
    fn tail_child(parent: &BlockHeader, nonce: u128) -> BlockHeader {
        BlockHeader {
            prev_block_hash: elide_chain::hash_block_header(parent),
            state_root: [0x44; 32],
            tx_root: [0x55; 32],
    ...
    fn direct_tail_fixture(
    ...
    ) -> (elide_ivc_core::field_r1cs::FieldR1cs, Vec<F128>, usize) {
        ...
        let parent_block_id = digest_lanes(&elide_chain::hash_block_header(parent))
    ...
    const MARKER_LEAF: usize = elide_tx::body_hash::TX8X2_LEAF_FEE;

    fn selected_suffix_case(
        payout_due: bool,
    ) -> (elide_ivc_core::field_r1cs::FieldR1cs, Vec<F128>, [F128; 3]) {
        let mut coinbase = SpineInputs {
            leaves: [[Block128::ZERO; 2]; elide_tx::body_hash::BODY_HASH_LEAVES],
        };
    ...
    fn bodies(start: &ChainAccumulator) -> Vec<SpineInputs> {
        ...
        coinbase.leaves[elide_tx::body_hash::TX8X2_LEAF_EPOCH_ANCHOR] =
            digest_lanes(&TEST_PARENT_BLOCK_ID);
        let mut user = SpineInputs {
            leaves: [[Block128::ZERO; 2]; elide_tx::body_hash::BODY_HASH_LEAVES],
        };
        user.leaves[elide_tx::body_hash::TX8X2_LEAF_EPOCH_ANCHOR] =
    ...
    fn coinbase_user_and_ghost_anchor_recombination() {
        ...
        for (body, leaf, lane) in [
            (0usize, elide_tx::body_hash::TX8X2_LEAF_EPOCH_ANCHOR, 0usize),
            (1, elide_tx::body_hash::TX8X2_LEAF_EPOCH_ANCHOR, 1),
            (2, elide_tx::body_hash::TX8X2_LEAF_FEE, 0),
            (2, elide_tx::body_hash::TX8X2_LEAF_EPOCH_ANCHOR, 1),
        ] {
    ...
    fn capacity_matrix_is_identical_across_real_user_counts() {
        ...
        two_users[2].leaves[elide_tx::body_hash::TX8X2_LEAF_EPOCH_ANCHOR] =
            digest_lanes(&start.epoch_anchor_id);
    ...
    fn scheduled_system_anchor_satisfies(payout_anchor: [u8; 32]) -> bool {
        ...
        payout.leaves[elide_tx::body_hash::TX8X2_LEAF_EPOCH_ANCHOR] = digest_lanes(&payout_anchor);
        let ghost =
            elide_gkr::spine_statement::spine_inputs_from_body(&elide_gkr::ghost_tx::ghost_tx_body());
    ...
    fn b255_body_liveness_excludes_the_256th_authorization_pad() {
        ...
        while natives.len() < 1 + elide_chain::consensus::params::BLOCK_MAX_USER_PAGES {
        ...
        let auth_capacity = super::tier_auth_slot_count(
            Some(elide_chain::consensus::params::BLOCK_MAX_USER_PAGES),
            1,
        );
