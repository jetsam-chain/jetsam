...
//!
//! Trace twins of `elide_gkr::merkle_batch_killshot`:
...
use elide_gkr::merkle_batch_killshot::{
    BatchedMerkleProofKillShot, MERKLE_CHAIN_LINEAR_RELATION_TAG, MERKLE_PIN_LANES,
};
use elide_gkr::merkle_circuit::{MerkleCircuit, MerklePathInputs, MAX_MERKLE_DEPTH};
...
impl BatchedMerkleProofTrace {
    pub fn alloc(
        ...
        let num_vars = elide_gkr::block_spine::num_vars_for(live_slots);
...
pub fn verify_batched_merkle_killshot_trace(
    ...
    assert_eq!(
        proof.num_vars,
        elide_gkr::block_spine::num_vars_for(proof.live_slots)
    );
...
mod tests {
    ...
    use elide_gkr::merkle_oracle::compute_merkle_root_with_directions;
