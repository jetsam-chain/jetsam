...
use elide_chain::consensus::fees::{PRESSURE_EXTREME_BPS, PRESSURE_HIGH_BPS, PRESSURE_LOW_BPS};
...
use elide_core::{hardware::flat_to_tower_u128, Block128};
...
const MAX_CONSERVATIVE_MIN_FEE: u128 = MIN_FEE_BASE as u128
    + FEE_PER_INPUT as u128 * elide_tx::MAX_PAGED_SPEND_INPUTS as u128
    + FEE_PER_OUTPUT as u128 * elide_tx::MAX_PAGED_SPEND_OUTPUTS as u128
    + STATE_GROWTH_FEE_BASE as u128 * 8 * elide_tx::MAX_PAGED_SPEND_OUTPUTS as u128;
...
fn bind_pressure(
    ...
    let native = elide_chain::consensus::fees::pressure_multiplier(native_active, native_depth);
...
fn bind_group_fee(
    ...
    let input_cap = range_check_bits(
        ...
        &const_block(Block128::from(
            (elide_tx::MAX_PAGED_SPEND_INPUTS + 1) as u128,
        )),
        11,
    );
    let output_cap = range_check_bits(
        b,
        &const_block(Block128::from(
            (elide_tx::MAX_PAGED_SPEND_OUTPUTS + 1) as u128,
        )),
        9,
    );
...
mod tests {
    ...
    fn required(body: &TxBody, active: u64, depth: u32) -> u64 {
        elide_chain::consensus::fees::required_fee_for_tx_body(body, active, depth)
    }
    ...
    fn exact_pressure_boundaries_match_native_integer_basis_points() {
        for depth in MIN_EXACT_STATE_DEPTH..=MAX_EXACT_STATE_DEPTH {
            for (bps, before, at) in [
                ...
                assert_eq!(
                    elide_chain::consensus::fees::pressure_multiplier(threshold - 1, depth as u32),
                    before
                );
                assert_eq!(
                    elide_chain::consensus::fees::pressure_multiplier(threshold, depth as u32),
                    at
                );
            }
        }
