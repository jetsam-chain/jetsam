...
//! (`elide_gkr::block_spine`): `verify_block_spine_unified` and
...
use elide_gkr::block_spine::{
    BlockSpineShiftProof, BlockSpineUnifiedProof, BLOCK_SPINE_ROUND_DEGREE,
    BLOCK_SPINE_SHIFT_DEGREE,
};
use elide_gkr::spine_mle::{sigma_at, N_SPINE_ELEM_VARS, N_SPINE_ROUND_VARS};
...
pub fn spine_point_index(num_vars: usize, slot: usize, round: usize, lane: usize) -> usize {
    let point = elide_gkr::block_spine::block_spine_state_point(num_vars, slot, round, lane);
...
impl ColumnAccumulator {
    ...
    pub fn new(b: &mut FieldR1csBuilder, point: &[LinExpr], n_slots: usize) -> Self {
        assert!(n_slots > 0);
        let num_vars = elide_gkr::block_spine::num_vars_for(n_slots);
...
mod tests {
    ...
    fn column_evaluation_trace_matches_native() {
        ...
        use elide_gkr::batch_eval::BatchEvalReduction;
        assert!(
