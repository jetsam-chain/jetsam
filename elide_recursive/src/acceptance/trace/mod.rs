...
//! `elide_poseidon2b::channel::Poseidon2bChannel`), same checks, no
...
use elide_core::{Block128, Block256};

pub use elide_ivc_core::field::{F128, F256};
pub use elide_ivc_core::field_circuit::{
...
/// Trace twin of `elide_core::mle::eq::eq_ind` for two expression points:
/// `Π (1 + x_i + y_i)` (the char-2 form of `x·y + (1−x)(1−y)`).
pub fn eq_ind_trace(b: &mut FieldR1csBuilder, x: &[LinExpr], y: &[LinExpr]) -> LinExpr {
    b.eq_eval_trace(x, y)
}

/// Trace twin of `elide_core::mle::eq::eq_ind_partial_eval`: the tensor
...
/// Trace twin of `elide_core::mle::evaluate::evaluate_slice` for an
...
/// `CompressedRoundPolynomial<Block128>` (`elide_core::sumcheck`): the linear
...
impl RoundPolyTrace {
    pub fn alloc(
        b: &mut FieldR1csBuilder,
        native: &elide_core::sumcheck::CompressedRoundPolynomial<Block128>,
...
/// `elide_gkr::batch_eval::BatchEvalReduction`.
#[derive(Clone)]
...
mod tests {
    use super::*;
    use elide_core::mle::eq::{eq_ind, eq_ind_partial_eval};
    ...
    fn evaluate_slice_trace_matches_native() {
        ...
        let native = elide_core::mle::evaluate::evaluate_slice(&tab, &point);
        test_support::assert_expr_is(&b, &got, native, "evaluate_slice");
        let (r1cs, z) = b.build();
        assert!(r1cs.satisfies(&z));
    }

    fn gated_equality_matrix(active: bool) -> (elide_ivc_core::field_r1cs::FieldR1cs, Vec<F128>) {
        let mut b = FieldR1csBuilder::new();
