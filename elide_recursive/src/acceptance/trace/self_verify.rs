...
//! The self-verification slot: the `elide_ivc_core` FieldR1cs verifier
...
//! the verified proof's field elements are `elide_ivc_core::F128` values
...
//! primitives are the flat-basis constructions of `elide_ivc_core::merkle`.
...
use elide_ivc_core::field::PHI_8_TABLE;
use elide_ivc_core::field_circuit::{f128_from_u128, FsChannelOps};
use elide_ivc_core::field_r1cs::FieldR1cs;
use elide_ivc_core::merkle::{self, Hash};
use elide_ivc_core::ntt::AdditiveNttF128;
use elide_ivc_core::pcs::{self, compute_fri_arities, default_fri_queries, PcsParams, LOG_PACKING};
use elide_ivc_core::public_io::POST_COMMIT_CLASS_BINDING_LABEL;
use elide_ivc_core::zerocheck::{self, K_SKIP};
use elide_poseidon2b::native::{capacity_iv_flat, DomainTag};
...
/// The `IVCPCSL_` / `IVCPCSN_` tags of `elide_ivc_core::merkle`, duplicated
...
// ---------------------------------------------------------------------------
// Merkle primitives (trace twins of elide_ivc_core::merkle)
// ---------------------------------------------------------------------------
...
/// Trace twin of `elide_ivc_core::merkle::hash_pair` — ONE feed-forward
...
/// Trace twin of `elide_ivc_core::merkle::hash_leaf` for a lane-aligned leaf
...
/// Trace twin of `elide_ivc_core::proof::bind_statement_field`. The instance
...
pub fn bind_statement_field_trace(
    ...
    ch.observe_bytes_const(
        b,
        &elide_ivc_core::proof::pcs_params_statement_bytes(pcs_params),
    );
...
impl LincheckProofTrace {
    pub fn alloc(
        b: &mut FieldR1csBuilder,
        native: &elide_ivc_core::lincheck::LincheckProof,
        k_log: usize,
        k_skip: usize,
    ) -> Self {
...
impl C1LincheckProofTrace {
    pub fn alloc(
        b: &mut FieldR1csBuilder,
        native: &elide_ivc_core::lincheck::c1::C1LincheckProof,
        k_log: usize,
        k_skip: usize,
    ) -> Self {
        assert_eq!(native.rounds.len(), k_log - k_skip, "rounds off shape");
...
impl FieldR1csProofTrace {
    pub fn alloc(
        b: &mut FieldR1csBuilder,
        native: &elide_ivc_core::proof::FieldR1csProof,
        r1cs: &FieldR1cs,
        pcs_params: &PcsParams,
    ) -> Self {
        Self::alloc_shape(
            b,
            native,
            &elide_ivc_core::proof::FieldShape::of(r1cs),
            pcs_params,
        )
    }
    ...
    pub fn alloc_shape(
        b: &mut FieldR1csBuilder,
        native: &elide_ivc_core::proof::FieldR1csProof,
        shape: &elide_ivc_core::proof::FieldShape,
        pcs_params: &PcsParams,
    ) -> Self {
...
impl C1FieldR1csProofTrace {
    pub fn alloc(
        b: &mut FieldR1csBuilder,
        native: &elide_ivc_core::proof::C1FieldR1csProof,
        r1cs: &FieldR1cs,
        pcs_params: &PcsParams,
    ) -> Self {
...
pub fn bind_public_io_trace(
    b: &mut FieldR1csBuilder,
    ch: &mut impl FsChannelOps,
    spec: &elide_ivc_core::public_io::PublicIoSpec,
    io: &[LinExpr],
    total_vars: usize,
    ...
    let prefix = |slice: &elide_ivc_core::public_io::WitnessSlice| -> Vec<LinExpr> {
        slice
...
pub fn bind_public_io_c1_trace(
    ...
    let prefix = |slice: &elide_ivc_core::public_io::WitnessSlice| {
        slice
            .prefix_coords(total_vars)
            .into_iter()
...
fn verify_field_trace_inner(
    ...
    commitment_root: &FlatDigestExpr,
    proof: &FieldR1csProofTrace,
    public_io: Option<(&elide_ivc_core::public_io::PublicIoSpec, &[LinExpr])>,
) -> R1csClaimTrace {
    assert_eq!(
        pcs_params.m,
        r1cs.m + LOG_PACKING,
...
pub(crate) fn shape_only_field_r1cs_proof_c1(
    shape: &elide_ivc_core::proof::FieldShape,
    pcs_params: &PcsParams,
) -> (elide_ivc_core::proof::C1FieldR1csProof, Hash) {
    ...
    let proof = elide_ivc_core::proof::C1FieldR1csProof {
        ...
        lincheck: elide_ivc_core::lincheck::c1::C1LincheckProof {
...
pub(crate) fn patch_shape_only_query_positions_c1(
    proof: &mut elide_ivc_core::proof::C1FieldR1csProof,
    pcs_params: &PcsParams,
    ...
    for lane in query_lane_values {
        let raw = elide_ivc_core::field_circuit::f128_to_u128(*lane);
...
mod tests {
    use super::*;
    use elide_ivc_core::challenger::{fs_pack_bytes_lanes, Challenger, FsLaneChallenger};
    use elide_ivc_core::deep_chain::schedule::{compile_duplex, DuplexLayout};
    use elide_ivc_core::field_circuit::{f128_to_u128, FsChannelUnionRecorder};
    ...
    use elide_ivc_core::field_circuit::FsChannelTrace;
    ...
    /// The module's private tag copies match the ones `elide_ivc_core::merkle`
    ...
    fn duplicated_tags_match_native_merkle() {
        use elide_poseidon2b::native::{compress_flat_feed_forward_with_tag, Poseidon2bFlatSponge};
    ...
    fn lagrange_windows_match_native() {
        use elide_ivc_core::zerocheck::multilinear::{
    ...
    fn lincheck_fixture(
        ...
        elide_ivc_core::lincheck::LincheckProof,
    ) {
        use elide_ivc_core::field_r1cs::synthetic_satisfiable;
        use elide_ivc_core::lincheck::{self, QuirkyPoint};
    ...
    fn c1_lincheck_fixture(
        ...
        elide_ivc_core::lincheck::c1::C1LincheckProof,
    ) {
        use elide_ivc_core::field_r1cs::synthetic_satisfiable;
        use elide_ivc_core::lincheck::c1::{self, C1QuirkyPoint};
    ...
    fn pcs_fixture(
        ...
        use elide_ivc_core::lincheck::build_eq_table;
        use elide_ivc_core::zerocheck::multilinear::lagrange_weights_naive;
    ...
    fn field_proof_fixture(
        ...
        PcsParams,
        pcs::Commitment,
        elide_ivc_core::proof::FieldR1csProof,
        elide_ivc_core::proof::R1csClaim,
    ) {
        use elide_ivc_core::field_r1cs::synthetic_satisfiable;
    ...
    fn c1_field_proof_fixture(
        ...
        PcsParams,
        pcs::Commitment,
        elide_ivc_core::proof::C1FieldR1csProof,
        elide_ivc_core::proof::C1R1csClaim,
    ) {
        use elide_ivc_core::field_r1cs::synthetic_satisfiable;
        use elide_ivc_prover::field_prover::prove_field_c1;
    ...
    fn build_field_verify_trace(
        ...
        params: &PcsParams,
        commitment: &pcs::Commitment,
        proof: &elide_ivc_core::proof::FieldR1csProof,
    ) -> (
        FieldR1cs,
        Vec<F128>,
        std::ops::Range<usize>,
        ...
        elide_ivc_core::verifier::verify_field(r1cs, commitment, proof, &mut ch_native)
    ...
    fn verify_field_c1_replay_lockstep_e2e() {
        ...
        elide_ivc_core::verifier::verify_field_c1(&r1cs, &commitment, &proof, &mut native)
    ...
    fn verify_field_c1_deferred_capture_lockstep_e2e() {
        use elide_ivc_core::field_r1cs::synthetic_satisfiable;
        use elide_ivc_prover::field_prover::prove_field_c1_capturing_fresh;
        ...
        let shape = elide_ivc_core::proof::FieldShape::of(&r1cs);
    ...
    fn verify_field_c1_production_envelope_lockstep_e2e() {
        use elide_ivc_core::field_r1cs::synthetic_satisfiable;
        use elide_ivc_core::public_io::{PublicIoSpec, WitnessSlice};
        use elide_ivc_core::verifier::VerifyError;
        use elide_ivc_prover::field_prover::prove_field_c1_with_public_io_and_post_commit_context;
            ...
            prove_field_c1_with_public_io_and_post_commit_context(
                ...
                |context| {
                    ...
                    let value = context
                        .witness()
                        .iter()
                        .zip(elide_ivc_core::lincheck::build_eq_table(&point))
        ...
        let (_, native_fresh) =
            elide_ivc_core::verifier::verify_field_c1_deferred_matrix_with_post_commit_context(
                &shape,
    ...
    fn verify_field_replay_with_public_io_lockstep_and_mutations() {
        use elide_ivc_core::field_r1cs::SparseFieldMatrix;
        use elide_ivc_core::public_io::{IoClaimSpec, PublicIoSpec, WitnessSlice};
        use elide_ivc_prover::field_prover::prove_field_with_public_io;
        ...
        let eq = elide_ivc_core::lincheck::build_eq_table(&p);
        let mut v = F128::ZERO;
        ...
        elide_ivc_core::verifier::verify_field_with_public_io(
            &inner,
            &commitment,
            &proof,
            &spec,
    ...
    fn verify_field_deferred_lockstep_and_mutations() {
        use elide_ivc_core::field_r1cs::SparseFieldMatrix;
        use elide_ivc_core::matrix_claim::{
            prove_matrix_claim_fold, stacked_matrix_mle_eval, MatrixAccClaim,
        };
        use elide_ivc_core::proof::FieldShape;
        use elide_ivc_core::public_io::{PublicIoSpec, WitnessSlice};
        use elide_ivc_prover::field_prover::prove_field_with_public_io;
        ...
        let (_, fresh_native) = elide_ivc_core::verifier::verify_field_deferred_matrix(
            &shape,
            &digest,
            &commitment,
            &proof,
    ...
    fn verify_field_deferred_post_commit_auxiliary_lockstep() {
        ...
        use elide_ivc_core::proof::FieldShape;
        use elide_ivc_core::public_io::{PublicIoSpec, WitnessSlice};
        use elide_ivc_core::verifier::{verify_field_deferred_matrix_with_post_commit, VerifyError};
        use elide_ivc_prover::field_prover::prove_field_with_public_io_and_post_commit;
    ...
    fn verify_field_deferred_post_commit_context_lockstep() {
        ...
        use elide_ivc_core::proof::FieldShape;
        use elide_ivc_core::public_io::{PublicIoSpec, WitnessSlice};
        use elide_ivc_core::verifier::{
            verify_field_deferred_matrix_with_post_commit_context, VerifyError,
        };
        use elide_ivc_prover::field_prover::prove_field_with_public_io_and_post_commit_context;
