...
use elide_ivc_core::matrix_claim::c1::{C1FreshLincheckClaim, C1MatrixFoldProof};
use elide_ivc_core::pcs::{Commitment, PcsParams};
use elide_ivc_core::proof::{pcs_params_statement_bytes, C1FieldR1csProof, FieldShape};
use elide_ivc_core::public_io::PublicIoSpec;
use elide_ivc_core::verifier::{
    verify_field_c1_deferred_matrix_with_post_commit_context, VerifyError,
};
use elide_ivc_prover::field_prover::{
...
pub struct AuthorizationComponentInput {
    ...
    pub tx_index: usize,
    pub tx_body_hash: [elide_core::Block128; 2],
    pub public: elide_gkr::OwnerAuthPublicInputs,
}
...
pub struct ExactStateStructuralFrontierInputs {
    ...
    pub old_slot_leaves: Vec<elide_gkr::SlotLeafInputs>,
    pub new_slot_leaves: Vec<elide_gkr::SlotLeafInputs>,
    pub live_sibling_digests: Vec<elide_chain::exact_state_hash::StateHash>,
    pub old_combine_digests: Vec<elide_chain::exact_state_hash::StateHash>,
    pub new_combine_digests: Vec<elide_chain::exact_state_hash::StateHash>,
    pub old_root: elide_chain::exact_state_hash::StateHash,
    pub new_root: elide_chain::exact_state_hash::StateHash,
}
...
pub struct HistoryStepBlockComponents {
    ...
    pub tx_body_inputs: Vec<elide_gkr::SpineInputs>,
    pub tx_body_hashes: Vec<[elide_core::Block128; 2]>,
    pub tx_root_inputs: Vec<elide_gkr::MerklePathInputs>,
...
pub fn prepare_history_step_ghost_authorization(
    proof: elide_gkr::zk_authorization::ZkAuthorizationProof,
...
pub fn prepare_history_step_authorizations<const TIER: usize>(
    ...
    proofs: Vec<elide_gkr::zk_authorization::ZkAuthorizationProof>,
    ...
    if inputs.iter().any(|input| {
        input.public.layout != elide_gkr::OwnerAuthLayout::FIXED
    ...
    let statements = inputs
        .iter()
        .map(
            |input| elide_gkr::zk_authorization::ZkAuthCapsuleOwnerStatement {
...
impl<const TIER: usize> HistoryStepBlockInput<TIER> {
    pub fn try_new(
        ...
        let actual_tier = elide_chain::consensus::paged_spend::BlockProofClass::for_page_count(
        ...
        let semantic_id = elide_chain::block_header::semantic_header_id(sealed_header);
