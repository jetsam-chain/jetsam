...
use elide_ivc_core::field_circuit::{f128_from_u128, ExtExpr};
pub const HISTORY_STEP_WIRE_VERSION: u8 = 4;

const _: () = assert!(
    HISTORY_STEP_WIRE_VERSION == elide_chain::history_step::HISTORY_STEP_TERMINAL_VERSION,
    ...
    pub fn new(
        ...
        for (slot, vk) in direct_block_vks.iter().enumerate() {
            if crate::region_sidecar::selected_zk_block_geometry(
                elide_chain::consensus::params::BLOCK_PAGE_CLASS_TIERS[slot],
            )
            .is_none()
...
fn history_step_query_lane_count(params: &PcsParams) -> usize {
    let log_dim = params.m - elide_ivc_core::pcs::LOG_PACKING - params.log_batch_size;
    let k_code = log_dim + params.log_inv_rate;
    let per_lane = 128 / k_code;
    elide_ivc_core::pcs::default_fri_queries(params.log_dim(), params.log_inv_rate)
        .div_ceil(per_lane)
}
...
fn prepare_history_step_assembly<const TIER: usize>(
    ...
    if elide_chain::consensus::paged_spend::BlockProofClass::for_page_count(effective_pages)
    ...
    // header, pinned through both derived ids.
    {
        ...
        let genesis_id = digest_lanes(&elide_chain::hash_block_header(&genesis));
        let genesis_semantic =
            digest_lanes(&elide_chain::block_header::semantic_header_id(&genesis));
        for lane in 0..2 {
    ...
    Ok(match r1cs {
        ...
        None => {
            ...
            let built = BuiltHistoryStep {
                ...
                useful_rows: used,
                class_id: current_class,
                semantic_id: elide_chain::block_header::semantic_header_id(sealed_header),
...
mod tests {
    ...
    fn seal_fixture_parent(height: u64) -> elide_chain::BlockHeader {
    ...
    fn parent_seal_replay_rejects_wrong_prev_block_hash() {
        ...
        let link = elide_chain::hash_block_header(&parent);
        let tip = elide_chain::block_header::semantic_header_id(&parent);
        ...
        assert!(seal_glue_satisfies(
            &renonced,
            elide_chain::hash_block_header(&renonced),
            tip,
        ));
        ...
        assert!(!seal_glue_satisfies(
            &tampered,
            elide_chain::hash_block_header(&tampered),
            tip,
        ));
    }
    ...
    fn epoch_lane_updates_from_derived_parent_id_at_boundary() {
        ...
        for parent_height in [elide_chain::consensus::params::TX_EPOCH_BLOCKS - 1, elide_chain::consensus::params::TX_EPOCH_BLOCKS] {
            ...
            let child = elide_chain::BlockHeader {
            ...
            let satisfies = |end: &ChainAccumulator| {
                ...
                let child_wires = DirectChildWires {
                    semantic_id: digest_lanes(&elide_chain::block_header::semantic_header_id(
                        &child,
                    ))
                    .map(|lane| {
                        LinExpr::from_wire(
                    ...
                    height: LinExpr::from_wire(builder.alloc_f128(
                        crate::acceptance::trace::flat_of(elide_core::Block128::from(
    ...
    fn base_case_selector_rejects_non_genesis_parent() {
        ...
        let base_pins_satisfy = |parent: &elide_chain::BlockHeader| {
            let genesis = elide_chain::consensus::genesis_header();
            let genesis_id = digest_lanes(&elide_chain::hash_block_header(&genesis));
            let genesis_semantic =
                digest_lanes(&elide_chain::block_header::semantic_header_id(&genesis));
        ...
        assert!(base_pins_satisfy(&elide_chain::consensus::genesis_header()));
        assert!(!base_pins_satisfy(&seal_fixture_parent(0)));
        let mut renonced_genesis = elide_chain::consensus::genesis_header();
